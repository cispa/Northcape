...
`define AXI_INTERFACE_MODULE_MONITOR(name)    \
    /* AW Channel  */ \
    input wire [AXI_ID_WIDTH - 1 : 0] ``name``_awid, \
    input wire [AXI_ADDR_WIDTH - 1 : 0] ``name``_awaddr, \
    input wire [7:0] ``name``_awlen, \
    ...
    (* X_INTERFACE_PARAMETER = "XIL_INTERFACENAME S_AXI, MODE Monitor" *) \
    (* X_INTERFACE_MODE = "monitor" *) \
    input wire ``name``_rready

`define AXI_INTERFACE_MODULE_OUTPUT_CUSTOM(name, AXI_ADDR_WIDTH, AXI_DATA_WIDTH, AXI_ID_WIDTH, AXI_USER_WIDTH)    \
    /* AW Channel  */ \
...
`define AXI_INTERFACE_MODULE_OUTPUT(name)    \
    `AXI_INTERFACE_MODULE_OUTPUT_CUSTOM(name, AXI_ADDR_WIDTH, AXI_DATA_WIDTH, AXI_ID_WIDTH, AXI_USER_WIDTH)
...
`define AXIS_MODULE_DECL(prefix) \
    wire ``prefix``_tvalid; \
    wire ``prefix``_tready; \
    wire [TDATA_WIDTH-1:0] ``prefix``_tdata; \
    wire [TDATA_WIDTH / 8-1:0] ``prefix``_tstrb; \
    wire [TDATA_WIDTH / 8-1:0] ``prefix``_tkeep; \
    wire ``prefix``_tlast;\
    wire [TID_WIDTH-1:0] ``prefix``_tid;\
    wire [TDEST_WIDTH-1:0] ``prefix``_tdest;\
    wire [TUSER_WIDTH-1:0] ``prefix``_tuser;\
    wire ``prefix``_twakeup
