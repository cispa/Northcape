...
module northcape_cap_ops_wrapper#(
    // AXI parameters
    ...
    parameter CSR_REQ_WIDTH=-1,
    parameter CSR_RSP_WIDTH=-1,

    parameter bit MMIO_INTERFACE_SUPPORTED = 1'b1,
    parameter bit CSR_INTERFACE_SUPPORTED  = 1'b1,
    parameter bit OPS_TAG_METHOD_USE_CTR   = 1'b1,
...
`endif
    ...
    // cache interface
    output logic ops_interface_request_valid,
    output logic [CAPABILITY_ID_WIDTH-1:0] ops_interface_request_capability_id,
    output logic [TAG_ID_WIDTH-1:0] ops_interface_request_capability_tag,
    output logic ops_interface_is_write,
    output logic [CMT_ENTRY_WIDTH-1:0] ops_interface_write_request_capability,
    output logic ops_interface_write_request_flush,
    output logic ops_interface_request_is_uncacheable,
    input logic ops_interface_response_valid,
    input logic ops_interface_response_err,
    input logic [CMT_ENTRY_WIDTH-1:0] ops_interface_response_cmt_entry,

    input logic memory_ready_i,

    // CSR interface
    input logic [CSR_REQ_WIDTH-1:0] csr_req_i,
    output logic [CSR_RSP_WIDTH-1:0] csr_rsp_o,
    ...
    output logic [4:0] debug_capability_operation_o,
...
assign ops_interface_request_valid = ops_interface.request_valid;
assign ops_interface_request_capability_id = ops_interface.request_capability_id;
assign ops_interface_request_capability_tag = ops_interface.request_capability_tag;
assign ops_interface_is_write = ops_interface.is_write;
assign ops_interface_write_request_capability = ops_interface.write_request_capability;
assign ops_interface_write_request_flush = ops_interface.write_request_flush;
assign ops_interface_request_is_uncacheable = ops_interface.request_is_uncacheable;

assign ops_interface.response_cmt_entry = ops_interface_response_cmt_entry;
assign ops_interface.response_err = ops_interface_response_err;
assign ops_interface.response_valid = ops_interface_response_valid;
...
assign csr_req = csr_req_i;
assign csr_rsp_o = csr_rsp;
...
northcape_capability_ops#(
    ...
    .OPS_TAG_METHOD(OPS_TAG_METHOD_USE_CTR ? northcape_capability_ops_common::NORTHCAPE_CAPABILITY_OPS_CTR : northcape_capability_ops_common::NORTHCAPE_CAPABILITY_OPS_CBC),
...
`NORTHCAPE_UNREAD(csr_req_i);

generate
    if(CAPABILITY_ID_WIDTH < northcape_types::NORTHCAPE_CAPABILITY_ID_WIDTH || TAG_ID_WIDTH < northcape_types::NORTHCAPE_CAPABILITY_TAG_WIDTH || CMT_ENTRY_WIDTH < $bits(northcape_types::northcape_cmt_entry_t) || CSR_REQ_WIDTH < $bits(northcape_types::northcape_cap_ops_rcsr_req_t) || CSR_RSP_WIDTH < $bits(northcape_types::northcape_cap_ops_rcsr_resp_t))
    begin
