/**
  ...
  */
module northcape_sram_dport_wenable_indclock #(
  ...
  generate
    if (INIT_TO_ZERO) begin : gen_init
      initial begin
        for (int init_index = 0; init_index < DATA_DEPTH; init_index++) begin
          bram[init_index] = {DATA_WIDTH{1'b0}};
        end
  ...
  generate
    if (WRITE_FIRST) begin : gen_logic_write_first
      for (genvar i = 0; i < DATA_BYTES; i++) begin : gen_byte_access
        ...
        always_ff @(posedge (clk_a_i)) begin : a_port_access
          if (a_enable_i) begin
        ...
        always_ff @(posedge (clk_b_i)) begin : b_port_access
          if (b_enable_i) begin
      ...
      always_ff @(posedge (clk_a_i)) begin : a_port_read
        if (a_enable_i) begin
          a_rdata_o <= bram[a_addr_i];
        end
      end : a_port_read

      always_ff @(posedge (clk_b_i)) begin : b_port_read
        if (b_enable_i) begin
      ...
      for (genvar i = 0; i < DATA_BYTES; i++) begin : gen_byte_access
        ...
        always_ff @(posedge (clk_a_i)) begin : a_port_write
          if (a_enable_i) begin
            if (a_wenable_i[i]) begin
              bram[a_addr_i][BIT_START-1:BIT_END] <= a_wdata_i[BIT_START-1:BIT_END];
            end
        ...
        always_ff @(posedge (clk_b_i)) begin : b_port_write
          if (b_enable_i) begin
            if (b_wenable_i[i]) begin
              bram[b_addr_i][BIT_START-1:BIT_END] <= b_wdata_i[BIT_START-1:BIT_END];
            end
