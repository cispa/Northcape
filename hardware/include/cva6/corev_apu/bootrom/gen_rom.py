...
memory_initialization_vector=$content;
"""
...
def generate_coe(filename, rom):
    """ Generate Vivado coe (Block RAM init file) for FPGA """
    with open(filename + ".coe", "w") as f:
        rom_str = ""
        # process in junks of 64 bit (8 byte)
        for i in range(int(len(rom)/8)):
            rom_str += bytes(reversed(rom[i*8:i*8+8])).hex() + ", "
        # remove the trailing comma
        rom_str = rom_str[:-2]

        s = Template(COE_TEMPLATE)
        f.write(s.substitute(content=rom_str))

def generate_mem(filename, rom):
    """ Generate Verilog .mem file """
...
def main():
    ...
    generate_coe(filename, rom)
    generate_mem(filename, rom)
