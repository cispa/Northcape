...
#(
    ...
    output northcape_types::task_id_t miss_task_id_o, // task ID provided by Northcape MMU stage
    output northcape_types::northcape_device_interpreted_restriction_t miss_device_interpreted_restriction_o,
  ...
  logic is_non_cacheable_d, is_non_cacheable_q;

  northcape_types::task_id_t task_id_q, task_id_d;
  northcape_types::northcape_device_interpreted_restriction_t device_interpreted_restriction_q, device_interpreted_restriction_d;
  ...
  assign is_irq_d = (req_port_o.data_gnt) ? req_port_i.is_irq : is_irq_q;
  assign is_non_cacheable_d = (req_port_o.data_gnt) ? req_port_i.is_non_cacheable : is_non_cacheable_q;
  assign task_id_d = (req_port_o.data_gnt) ? req_port_i.task_id : task_id_q;
  assign device_interpreted_restriction_d = (req_port_o.data_gnt) ? req_port_i.device_interpreted_restriction : device_interpreted_restriction_q;
  ...
  assign req_port_o.data_rerror= rd_error_i;
  ...
  assign miss_irq_o = is_irq_q;
  assign miss_task_id_o = task_id_q;
  assign miss_device_interpreted_restriction_o = device_interpreted_restriction_q;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin : p_regs
    if (!rst_ni) begin
      ...
      rd_ack_q      <= '0;
      is_irq_q      <= 1'b0;
      is_non_cacheable_q <= 1'b0;
      task_id_q     <= '0;
      device_interpreted_restriction_q <= '0;
    end else begin
      state_q       <= state_d;
      ...
      rd_ack_q      <= rd_ack_d;
      is_irq_q      <= is_irq_d;
      is_non_cacheable_q <= is_non_cacheable_d;
      task_id_q     <= task_id_d;
      device_interpreted_restriction_q <= device_interpreted_restriction_d;
    end
  end
