...
(
    ...
    output logic [HPDcacheCfg.axiUserWidth-1:0] wbuf_user_o,
...
//  {{{
    ...
    logic               [N-1:0][HPDcacheCfg.axiUserWidth-1:0] user;
...
//  {{{
    for (gen_i = 0; gen_i < N; gen_i++) begin : gen_check
        ...
        assign user[gen_i] = req_q[gen_i].req.user;
...
//  {{{
    ...
    hpdcache_mux #(
        .NINPUT         (N),
        .DATA_WIDTH     ($bits(wbuf_user_o)),
        .ONE_HOT_SEL    (1'b1)
    ) wbuf_pending_user_mux_i (
        .data_i         (user),
        .sel_i          (wbuf_sel),
        .data_o         (wbuf_user_o)
    );
