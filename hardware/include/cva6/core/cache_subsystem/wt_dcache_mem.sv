...
#(
    ...
    output logic rd_error_o, // bus error for this read?
    ...
    input logic                                      wr_cl_data_error_i, // bus error when trying to get data
  ...
  logic [DCACHE_NUM_BANKS-1:0][CVA6Cfg.DCACHE_SET_ASSOC-1:0]                   bank_rerror; // read OK or error
  logic [DCACHE_NUM_BANKS-1:0][CVA6Cfg.DCACHE_SET_ASSOC-1:0]                   bank_rerror_w; // read OK or error - input
  logic [CVA6Cfg.DCACHE_SET_ASSOC-1:0][CVA6Cfg.XLEN-1:0] rdata_cl;  // selected word from each cacheline
  logic [CVA6Cfg.DCACHE_SET_ASSOC-1:0]                   rdata_cl_error; // selected word had a bus error
  ...
  for (genvar k = 0; k < DCACHE_NUM_BANKS; k++) begin : gen_bank
    for (genvar j = 0; j < CVA6Cfg.DCACHE_SET_ASSOC; j++) begin : gen_bank_way
      ...
      assign bank_wdata[k][j] = (wr_cl_we_i[j] & wr_cl_vld_i) ?  wr_cl_data_i[k*CVA6Cfg.XLEN +: CVA6Cfg.XLEN] :
                                                                 wr_data_i;
      assign bank_rerror_w[k][j] = (wr_cl_we_i[j] & wr_cl_vld_i) ? wr_cl_data_error_i : 1'b0; // on read, copy error from missunit; clear on store (cannot error)
  ...
  for (genvar i = 0; i < CVA6Cfg.DCACHE_SET_ASSOC; i++) begin : gen_tag_cmpsel
    ...
    assign rdata_cl_error[i] = bank_rerror[bank_off_q[CVA6Cfg.DCACHE_OFFSET_WIDTH-1:CVA6Cfg.XLEN_ALIGN_BYTES]][i];
  ...
  generate
    ...
    begin: gen_ram_with_error_bit
      for (genvar k = 0; k < DCACHE_NUM_BANKS; k++) begin : gen_data_banks
        // Data RAM with extra bit for OK / bus error
        sram #(
            .USER_WIDTH(CVA6Cfg.DCACHE_SET_ASSOC * CVA6Cfg.DCACHE_USER_WIDTH),
            .DATA_WIDTH(CVA6Cfg.DCACHE_SET_ASSOC * (CVA6Cfg.XLEN + 1)),
            ...
            .wdata_i({bank_rerror_w[k], bank_wdata[k]}),
            .be_i   (bank_be[k]),
            .ruser_o(bank_ruser[k]),
            .rdata_o({bank_rerror[k],bank_rdata[k]})
        );
      end: gen_data_banks
    end: gen_ram_with_error_bit
    else
    begin: gen_ram_without_error_bit
      for (genvar k = 0; k < DCACHE_NUM_BANKS; k++) begin : gen_data_banks
        // Data RAM
        sram #(
            .USER_WIDTH(CVA6Cfg.DCACHE_SET_ASSOC * CVA6Cfg.DCACHE_USER_WIDTH),
        ...
        assign bank_rerror[k] = 1'b0;
      end: gen_data_banks
    end: gen_ram_without_error_bit
