...
#(
    ...
    // iret has been committed --> no longer in interrupt context
    input logic eret_i,
    input logic v_i,
    // bus error on store write back
  ...
  localparam type dcache_rtrn_t = struct packed {
    ...
    logic [CVA6Cfg.MEM_TID_WIDTH-1:0] tid;  // threadi id (used as transaction id in Ariane)
    logic bus_error; // bus error occured during load
  };
