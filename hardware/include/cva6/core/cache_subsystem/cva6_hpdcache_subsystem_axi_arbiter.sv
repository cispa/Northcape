...
(
  ...
  northcape_types::northcape_axi_user_t axi_user_icache;

  generate
    if(CVA6Cfg.NORTHCAPE_STAGE_ENABLED)
    begin: gen_icache_users_northcape
      assign axi_user_icache.current_device_id = (CVA6Cfg.NORTHCAPE_MMU_INSTR_DEVICE_ID) >> 1;
      assign axi_user_icache.current_task_id = icache_miss_i.task_id;
      assign axi_user_icache.device_interpreted_restriction = icache_miss_i.device_interpreted_restriction;
    end
    else
    begin: gen_no_icache_users
      assign axi_user_icache = '0;
    end
  endgenerate
  ...
  assign icache_miss_req_wdata.mem_req_user = axi_user_icache;
