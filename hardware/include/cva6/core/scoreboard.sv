...
module scoreboard #(
  ...
  always_comb begin : clobber_assign
    ...
    for (int unsigned i = 0; i < CVA6Cfg.NR_SB_ENTRIES; i++) begin
      ...
      begin
        if(mem_q[i].sbe.op == ariane_pkg::REG_ZERO && mem_q[i].issued)
        begin
          for(int k = 0; k < 2 ** ariane_pkg::REG_ADDR_SIZE; k++)
          begin
            // ALL registers are clobbered until the instruction commits...
            gpr_clobber_vld[k][i] = 1'b1;
            fpr_clobber_vld[k][i] = 1'b1;
          end
        end
      end
    end
