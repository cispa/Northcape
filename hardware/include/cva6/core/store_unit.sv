...
#(
    ...
    // last bits of the physical address, one clock before paddr - MMU
  ...
  always_comb begin
    ...
    begin
      ...
      // also, need to base this on the PHYSICAL address, and compute it one cycle early
      st_data_n = (CVA6Cfg.RVA && instr_is_amo) ? lsu_ctrl_i.data[CVA6Cfg.XLEN-1:0] :
          data_align(dtlb_full_index_i[2:0], {{64 - CVA6Cfg.XLEN{1'b0}}, lsu_ctrl_i.data});
      // northcape can change offset with translation, common for 1-byte transfers
      // so we have to generate the store mask based on the final address
      st_be_n = be_gen(dtlb_full_index_i[2:0], extract_transfer_size(lsu_ctrl_i.operation));
    end
    else
    begin
      // don't shift the data if we are going to perform an AMO as we still need to operate on this data
  ...
  store_buffer #(
      ...
      .cbo_op_i             (cbo_op_q),
      .is_irq_i             (is_irq_q),
      .lsu_current_task_i   (lsu_current_task_i),
      .lsu_device_restriction_i(lsu_device_restriction_i),
      .lsu_is_non_cacheable_i(lsu_is_non_cacheable_i),
      .be_i                 (st_be_q),
