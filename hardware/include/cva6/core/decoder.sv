...
#(
    ...
    // bus error during store - CACHE
    input logic st_error_i,
    ...
    // Is in interrupt context - FRONTEND
    input logic is_irq_i,
    // task ID that triggered the exception - FRONTEND
    input northcape_types::task_id_t task_id_i,
    // is this a valid subsystem call? - FRONTEND
    input logic is_valid_scall_i,
    // is this a valid subsystem call if allowing self calls? - FRONTEND
    input logic is_valid_scall_s_i
);
  import ariane_pkg::*;
  ...
  always_comb begin : decoder
    ...
    instruction_o.is_irq                   = is_irq_i;
    instruction_o.task_id                  = task_id_i;
    instruction_o.northcape_is_valid_scall = is_valid_scall_i;
    instruction_o.northcape_is_valid_scall_s = is_valid_scall_s_i;
    instruction_o.scall_type               = ariane_pkg::SKADI_NO_SCALL;
    tinst                                  = '0;
    ecall                                  = 1'b0;
    ebreak                                 = 1'b0;
    check_fprm                             = 1'b0;
    ...
    if (~ex_i.valid) begin
      case (instr.rtype.opcode)
        ...
        // Memory ordering and cache block instructions
          ...
          if (instr.rvftype.funct2 == 2'b10) begin  // Prefix 10 for all Xfvec ops
            ...
            unique case ({
              ...
              begin
                ...
                begin
                  instruction_o.op = ariane_pkg::REG_ZERO;
                end
                else
                begin
        ...
        riscv::OpcodeJalr: begin
          ...
          unique case(instr.itype.funct3)
            ...
            3'b1:
            begin
              // Skadi scall - functionally jalr, but triggers exception if not valid subsystem call target
              if(CVA6Cfg.NORTHCAPE_SCALL_EXTENSION)
              begin
                instruction_o.scall_type = SKADI_SCALL;
              end
              else
              begin
                illegal_instr = 1'b1;
            ...
            3'b10:
            begin
              // Skadi scalls - like scall but allows subsystem calls to self
              if(CVA6Cfg.NORTHCAPE_SCALL_EXTENSION)
              begin
                instruction_o.scall_type = SKADI_SCALL_S;
              end
              else
              begin
                illegal_instr = 1'b1;
        ...
        riscv::OpcodeCustom0: begin
          ...
          unique case({
            ...
            begin
              // Skadi scall - functionally jalr, but triggers exception if not valid subsystem call target
              // this flavour expects a target capability (rs1) and expected subsystem ID (rs2) and throws an exception if target is not subsystem call to that ID
              if(CVA6Cfg.NORTHCAPE_SCALL_EXTENSION)
              begin
                instruction_o.scall_type = SKADI_SCALL_ID;
              end
              else
              begin
                illegal_instr = 1'b1;
  ...
  always_comb begin : exception_handling
    ...
    if (~ex_i.valid) begin
      ...
      end else if (CVA6Cfg.EXCEPTION_ON_BUS_ERROR && st_error_i)
      begin
        instruction_o.ex.valid = 1'b1;
        // TODO we just leave this as 0, because there is no good way to get to the offending address...
        instruction_o.ex.tval = '0;
        // we got an access fault during a preceding store
        instruction_o.ex.cause = riscv::ST_ACCESS_FAULT;
      end
      // -----------------
      // Interrupt Control
      // -----------------
      ...
      if (interrupt_cause[CVA6Cfg.XLEN-1] && (irq_ctrl_i.global_enable | (CVA6Cfg.NMI_ENABLE && ((irq_ctrl_i.mip & irq_ctrl_i.nmi) != '0) && is_irq_i == 1'b0) )) begin
