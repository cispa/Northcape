...
#(
    ...
    // Flush and Invalidate DCache - CACHE
    output logic flush_inval_dcache_o,
    ...
    // Northcape ops wants us to flush the data cache - NORTHCAPE OPS
    input logic nc_flush_dcache_i,
    // Dcache was disabled via custom CSR - CSR_REGFILE
    input logic dcache_flush_inval_all_i,
    // instruction is being comitted - COMMIT_STAGE
    input logic pc_commit_valid_i,
    // Icache flush complete - ICACHE
    input logic icache_flush_done_i
);

  // active fence - high if we are currently flushing the dcache
  logic fence_active_d, fence_active_q;
  logic flush_dcache, flush_inval_dcache;
  // Added fence_i_active state to track fence.i progress
  logic fence_i_active_d, fence_i_active_q;

  logic dcache_ack_seen_q, dcache_ack_seen_d;
  logic icache_ack_seen_q, icache_ack_seen_d;

  logic nc_flush_dcache_d, nc_flush_dcache_q;
  ...
  always_comb begin : flush_ctrl
    ...
    flush_ex_o             = 1'b0;
    flush_dcache           = 1'b0;
    flush_inval_dcache     = flush_dcache_o ? flush_inval_dcache_o : 1'b0;
    flush_icache_o         = 1'b0;
    flush_tlb_o            = 1'b0;
    flush_tlb_vvma_o       = 1'b0;
    flush_tlb_gvma_o       = 1'b0;
    flush_bp_o             = 1'b0;
    dcache_ack_seen_d      = dcache_ack_seen_q;
    icache_ack_seen_d      = icache_ack_seen_q;
    nc_flush_dcache_d      = nc_flush_dcache_q;
    // ------------
    // Mis-predict
    // ------------
    // flush on mispredict
    ...
    if (fence_i) begin
      ...
      // TODO this would break on a multiprocessor
      if (CVA6Cfg.DcacheFlushOnFence && !CVA6Cfg.RVZiCbom) begin
        flush_dcache   = 1'b1;
        fence_active_d = 1'b1;
      end
    end
    ...
    if (fence_i_i) begin
      ...
      if (CVA6Cfg.DcacheFlushOnFence) begin
        ...
        dcache_ack_seen_d = 1'b0;
        icache_ack_seen_d = 1'b0;
      end
    end
    ...
    // have a write-through cache in this case
    if (CVA6Cfg.DcacheFlushOnFence) begin
      if(flush_dcache_ack_i)
      begin
        dcache_ack_seen_d = 1'b1;
      end
      if(icache_flush_done_i)
      begin
        icache_ack_seen_d = 1'b0;
      end
      ...
      if (dcache_ack_seen_d && icache_ack_seen_d && fence_i_active_q) begin
    ...
    begin
      // Deassert after icache indicates completion
      if(icache_flush_done_i)
      begin
        fence_i_active_d = 1'b0;
      end
    ...
    // ---------------------------------
    // Software needs us to flush the data cache
    // ---------------------------------
    if(dcache_flush_inval_all_i)
    begin
      // no need to stop the instruction pipeline - treat this as flush + fence
      flush_dcache = 1'b1;
      flush_inval_dcache = 1'b1;
      fence_active_d = 1'b1;
    end

    // ---------------------------------
    // Northcape needs us to flush the data cache
    // ---------------------------------
    if(CVA6Cfg.NORTHCAPE_STAGE_ENABLED && nc_flush_dcache_i)
    begin
      nc_flush_dcache_d = 1'b1;
    end
    // Set PC to commit stage and flush pipeline when we are procedding a valid instruction
    if(CVA6Cfg.NORTHCAPE_STAGE_ENABLED && nc_flush_dcache_q && pc_commit_valid_i)
    begin
      // have to flush the icache too, could be affected
      // at the same time: need to get rid of pipeline state to avoid following illegal instructions
      set_pc_commit_o        = 1'b1;
      flush_if_o             = 1'b1;
      flush_unissued_instr_o = 1'b1;
      flush_id_o             = 1'b1;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (~rst_ni) begin
      ...
      flush_dcache_o   <= 1'b0;
      flush_inval_dcache_o <= 1'b0;
      dcache_ack_seen_q <= 1'b0;
      icache_ack_seen_q <= 1'b0;
      nc_flush_dcache_q <= 1'b0;
    end else begin
      fence_active_q    <= fence_active_d;
      fence_i_active_q <= fence_i_active_d;
      // register on the flush signal, this signal might be critical
      flush_dcache_o    <= flush_dcache;
      flush_inval_dcache_o <= flush_inval_dcache;
      dcache_ack_seen_q <= dcache_ack_seen_d;
      icache_ack_seen_q <= dcache_ack_seen_d;
      nc_flush_dcache_q <= nc_flush_dcache_d;
    end
  end
endmodule
