...
module id_stage #(
    ...
    // bus error encountered when writing back a store
  ...
  decoder #(
      ...
      .is_irq_i                  (fetch_entry_i[0].is_irq),
      .task_id_i                 (fetch_entry_i[0].task_id),
      .is_valid_scall_i          (fetch_entry_i[0].northcape_is_valid_scall),
      .is_valid_scall_s_i        (fetch_entry_i[0].northcape_is_valid_scall_s),
      .fs_i,
      .vfs_i,
      .frm_i,
      .vs_i,
      ...
      .st_error_i                (st_error_d || st_error_i)
  );
  ...
  always_comb begin
    ...
    // we have to latch the error until we managed to get it into the issue stage
    st_error_d = issue_instr_ack_i ? 1'b0 : st_error_q || st_error_i;
  end
  ...
  generate
    if(CVA6Cfg.EXCEPTION_ON_BUS_ERROR == 1'b1)
    begin
      always_ff @(posedge clk_i or negedge rst_ni) begin
      if (~rst_ni) begin
        st_error_q <= '0;
