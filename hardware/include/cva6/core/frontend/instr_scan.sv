...
module instr_scan #(
    ...
    // Return from Interrupt (sret, mret, etc.) - FRONTEND
    output logic return_from_irq_o
);
  ...
  // scall_id has an opcode from the custom range
  assign rvi_jalr_o = (instr_i[6:0] == riscv::OpcodeJalr) || (CVA6Cfg.NORTHCAPE_SCALL_EXTENSION && instr_i[6:0] == riscv::OpcodeCustom0);
  ...
  generate
    if(CVA6Cfg.SUPPORT_NON_STANDARD_TABLE_ISR_MODE) begin: checkIRQRet
      logic is_sret, is_mret;
      // sret/mret are completely static
      assign is_sret = instr_i == 32'b0001000_00010_00000_000_00000_1110011;
      assign is_mret = instr_i == 32'b0011000_00010_00000_000_00000_1110011;
      assign return_from_irq_o = (is_sret || is_mret);
    end: checkIRQRet
    else
    begin: skipIRQRetCheck
      assign return_from_irq_o = 1'b0;
    end: skipIRQRetCheck
