...
#(
    ...
    // Instruction is in IRQ context? - FRONTEND
    input logic is_irq_i,
    // Task ID of the instruction - FRONTEND
    input northcape_types::task_id_t task_id_i,
    // valid subsystem call target? - FRONTEND
    input logic valid_scall_i,
    // valid subsystem call target if self call allowed? - FRONTEND
    input logic valid_scall_s_i,
    // Task ID in case of replay - FRONTEND
    output northcape_types::task_id_t replay_task_id_o
);
  ...
  typedef struct packed {
    ...
    northcape_types::task_id_t       task_id;         // task ID of the instruction
    logic                            valid_scall;     // valid subsystem call target?
    logic                            valid_scall_s;   // valid subsystem call target if self call allowed?
  } instr_data_t;
...
ariane_pkg::FETCH_FIFO_DEPTH
  ...
  if (CVA6Cfg.RVC) begin : gen_multiple_instr_per_fetch_with_C
    ...
    for (genvar i = 0; i < CVA6Cfg.INSTR_PER_FETCH; i++) begin : gen_fifo_input_select
      ...
      assign instr_data_in[i].is_irq = is_irq_i;
      assign instr_data_in[i].task_id = task_id_i;
      assign instr_data_in[i].valid_scall = valid_scall_i;
      assign instr_data_in[i].valid_scall_s = valid_scall_s_i;
    ...
    assign instr_data_in[0].is_irq = is_irq_i;
    assign instr_data_in[0].task_id = task_id_i;
    assign instr_data_in[0].valid_scall = valid_scall_i;
    assign instr_data_in[0].valid_scall_s = valid_scall_s_i;
  ...
  begin: gen_northcape_replay_task_id
    assign replay_task_id_o = task_id_i;
  end else begin: gen_no_replay_task_id
    assign replay_task_id_o = '0;
  end
  ...
  if (CVA6Cfg.RVC) begin : gen_downstream_itf_with_c
    always_comb begin
      ...
      for (int unsigned i = 0; i <= ariane_pkg::SUPERSCALAR; i++) begin
        ...
        fetch_entry_o[i].is_irq = 1'b0;
        fetch_entry_o[i].task_id = '0;
        fetch_entry_o[i].northcape_is_valid_scall = 1'b0;
        fetch_entry_o[i].northcape_is_valid_scall_s = 1'b0;
      ...
      for (int unsigned i = 0; i < CVA6Cfg.INSTR_PER_FETCH; i++) begin
        ...
        if (idx_ds[0][i]) begin
          ...
          fetch_entry_o[0].is_irq = instr_data_out[i].is_irq;
          fetch_entry_o[0].task_id = instr_data_out[i].task_id;
          fetch_entry_o[i].northcape_is_valid_scall = instr_data_out[i].valid_scall;
          fetch_entry_o[i].northcape_is_valid_scall_s = instr_data_out[i].valid_scall_s;
          if (CVA6Cfg.TvalEn)
        ...
        if (ariane_pkg::SUPERSCALAR > 0) begin
          if (idx_ds[1][i]) begin
            ...
            fetch_entry_o[NID].is_irq = instr_data_out[i].is_irq;
            fetch_entry_o[NID].task_id = instr_data_out[i].task_id;
            fetch_entry_o[NID].northcape_is_valid_scall = instr_data_out[i].valid_scall;
            fetch_entry_o[NID].northcape_is_valid_scall_s = instr_data_out[i].valid_scall_s;
    ...
    always_comb begin
      ...
      fetch_entry_o[0].is_irq = instr_data_out[0].is_irq;
      fetch_entry_o[0].task_id = instr_data_out[0].task_id;
      fetch_entry_o[0].northcape_is_valid_scall = instr_data_out[0].valid_scall;
      fetch_entry_o[0].northcape_is_valid_scall_s = instr_data_out[0].valid_scall_s;
