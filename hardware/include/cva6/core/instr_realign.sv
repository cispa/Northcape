...
#(
    ...
    // Instruction task ID (Northcape) - FRONTEND
    input northcape_types::task_id_t task_id_i,
    // valid subsystem call target? - FRONTEND
    input logic valid_scall_i,
    // valid subsystem call target allowing self? - FRONTEND
    input logic valid_scall_s_i,

    // valid subsystem call target? - FRONTEND
    output logic valid_scall_o,
    // valid subsystem call target allowing self? - FRONTEND
    output logic valid_scall_s_o,
  ...
  // simple passthrough - always take from the last instruction
  assign task_id_o = task_id_i;
  assign valid_scall_o = valid_scall_i;
  assign valid_scall_s_o = valid_scall_s_i;
