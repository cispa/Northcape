...
def write_regs(node):
    ...
    for i, reg in enumerate(node.regs):
        ...
        if reg.addr is not None and reg.size is not None:
            externvar = f"""
#if defined(SKADI_SUBSYSTEM) && !defined(_LINKER) && !defined(_ASMLANGUAGE)
/* 
 * What this achieves: the register address is set to the value of an imported symbol of a subsystem's ELF binary
 * The imported symbol encodes an MMIO range we want, which the Skadi loader will resolve to a capability token.
 * GCC does not allow us to read the value of a global here.
 * However, we are allowed to take the address of a *function pointer* and cast it to an integer.
 * So we lie to gcc, telling it that there is a *function* corresponding to the symbol name.
 * We then butcher the function address into the (scalar, integer) register address which we can use like normal.
 * GCC creates a relocation for us that the loader can then fill with the capability token.
 */
extern void __skadi_mmio_{i}_{reg.addr}_{reg.size}();
#define __SKADI_DT_{path_id}_REG_IDX_{i} ((uintptr_t)(void*)__skadi_mmio_{i}_{reg.addr}_{reg.size})
#endif
            """
            print(externvar, file=header_file)
        else:
            externvar = f"""
#if defined(SKADI_SUBSYSTEM) && !defined(_LINKER) && !defined(_ASMLANGUAGE)
/*
 * This is (hopefully!) not "actual" MDIO, but a constant address like an MDIO register address.
 * We just forward the plain value in this case; it will trigger an exception if used like an address.
 */
#define __SKADI_DT_{path_id}_REG_IDX_{i} DT_{path_id}_REG_IDX_{i}_VAL_ADDRESS
#endif
            """
            print(externvar, file=header_file)
