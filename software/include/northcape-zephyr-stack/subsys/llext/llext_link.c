...
extern uintptr_t skadi_loader_resolve_well_known_symbol(const struct llext *ext, const char *symbol_name);
extern void skadi_loader_symbol_resolved_cb(const struct llext *ext, const char *symbol_name, const void *symbol_value);
#endif
 ...
 */
static int llext_lookup_symbol(uintptr_t *link_addr, const elf_rela_t *rel, const elf_sym_t *sym, struct llext *ext, struct llext_loader *ldr, const char *name, const elf_shdr_t *shdr){
	*link_addr = 0;
	...
	if (ELF_R_SYM(rel ->r_info) == 0) {
		...
		*link_addr = 0;
		...
		/* 
		 * for security reasons, skadi forces subsystems to use loader_proxy before calling loader routines
		 * one exception: for integration tests, we need to be able to fail from all subsystems, so we allow a direct jump to the test fail function
		 */

		if(!IS_ENABLED(CONFIG_SKADI_OS) || strcmp(ext->name, "loader_proxy") == 0 || (IS_ENABLED(CONFIG_SOC_SERIES_CV64A6_PROVIDE_TEST_POWEROFF) && strcmp(name, "__skadi_z_sys_poweroff_callee_trampoline") == 0)){
			...
			*link_addr = (uintptr_t)llext_find_sym(NULL,
			...
			LOG_DBG("Skipping main binary symbol lookup for extension %s", ext->name);
			*link_addr = 0;
		}
		...
		if (*link_addr == 0) {
			/* Try loaded tables */
			struct llext *dep;
			/* crucial for the "TID 0 is gone" security argument */
			if(strcmp(name, "__skadi_nuuk_callee_trampoline") == 0){
				LOG_WRN("Nuuk is NOT available to other extensions!");
			}
			else{
		...
		if (*link_addr == 0) {
#ifdef CONFIG_SKADI_OS
			*link_addr = skadi_loader_resolve_well_known_symbol(ext, name);
			/*
				*  TODO find a proper solution
				*  In cases where a caller trampoline is declared in a header, but never used in the actual subsystem, 
				*  there remains an init function which prepares a caller trampoline. 
				*  The init function needs to set the subsystem call epilogue address, i.e., the code that restores the caller's registers.
				*  However, as the trampoline is never called, the compiler can optimize this code out and remove the symbol.
				*  Thus, in this case, we provide the symbol's value as 0.
				*/
			if(strstr(name,"skadi_subsystem_caller_trampoline_restore_context_") == name){
				LOG_WRN("Ignoring unresolved trampoline restore context symbol.");
			}
			else if(!*link_addr){
#endif
...
static inline int strendswith(const char *s, const char *t){
 ...
 *
 */
static int llext_riscv_find_sym_pcrel(struct llext *ext, struct llext_loader *ldr,
	...
	int *last_rel_idx)
{
	...
	ret = llext_lookup_symbol(&link_addr, &candidate, &candidate_sym, ext, ldr,
	symbol_name, shdr);
...
int llext_link(struct llext_loader *ldr, struct llext *ext, bool do_local)
{
	...
	for (i = 0; i < ldr->sect_cnt; ++i) {
		...
		if (mem_idx == LLEXT_MEM_COUNT) {
			/* FIXME sections such as debug_info might not actually be loaded */
		...
		for (int j = 0; j < rel_cnt; j++) {
			...
			ret = llext_lookup_symbol(&link_addr, &rel, &sym, ext, ldr, name, shdr);
			...
			if(IS_ENABLED(CONFIG_RISCV)){
				/* 
				 * on RISC-V, for certain relocations, the arch_elf_relocate function expects
				 * a value that depends on other relocations; we resolve this in a helper function
				 */
...
#ifdef CONFIG_SKADI_OS
			if(strendswith(name, "callee_trampoline") == 0){
				skadi_loader_symbol_resolved_cb(ext, name, (void*)link_addr);
			}
#endif
	 ...
	 * Skadi loader will synchronize with ifence later - would require write permission for the capability
	 */
	for (i = 0; i < LLEXT_MEM_COUNT; ++i) {
		if (ext->mem[i]) {
