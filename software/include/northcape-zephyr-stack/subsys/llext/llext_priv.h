...
/* we need the initial allocator, as the full allocator has not been loaded yet */
#include <zephyr/skadi/skadi_init_alloc.h>
#include <zephyr/skadi/skadi_allocator.h>

static inline void *llext_alloc(size_t bytes)
{
	const bool lock_to_loader = true;
	return (void *) skadi_init_alloc_allocate((uint32_t) bytes, SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_LOCKABLE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS, lock_to_loader);
}

/* llext_aligned_alloc never used  - not defined */

static inline void llext_free(void *ptr)
{
	skadi_init_alloc_free(ptr);
}
