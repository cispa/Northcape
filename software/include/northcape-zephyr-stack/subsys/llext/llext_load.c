		...
		for (j = i+1; j < LLEXT_MEM_COUNT; j++) {
			...
			if (ldr->hdr.e_type == ET_DYN) {
				...
				if ((x->sh_addr <= y->sh_addr &&
					...
					LOG_WRN("Region %d VMA range (0x%zx +%zd) "
...
#ifdef CONFIG_SKADI_OS
				LOG_WRN("Region %d ELF file range (0x%zx +%zd) "
					"overlaps with %d (0x%zx +%zd)",
...
#ifdef CONFIG_SKADI_OS
		exp_tab->syms[i].size = sym->size;
#endif

		if(IS_ENABLED(CONFIG_SKADI_OS)){
			const size_t buffer_bytes = strlen(exp_tab->syms[i].name) + 1;
			/* FIXME this storage is leaked! */
			// we need the strings to live in "our" memory
			char *name_copy = llext_alloc(buffer_bytes + 1);
			if(!name_copy){
				LOG_ERR("Could not allocate buffer for name!");
			}
			exp_tab->syms[i].name = strcpy(name_copy, exp_tab->syms[i].name);
		}
...
out:
	 ...
	 */
	if (!((IS_ENABLED(CONFIG_LLEXT_LOG_LEVEL_DBG) || IS_ENABLED(CONFIG_SKADI_LOADER)) && ret == 0)) {
		llext_free(ext->sym_tab.syms);
		ext->sym_tab.sym_cnt = 0;
		ext->sym_tab.syms = NULL;
	}
