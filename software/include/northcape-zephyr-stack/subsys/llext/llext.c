...
#ifdef CONFIG_SKADI_OS
static struct k_spinlock llext_lock;
#else
...
{
#ifdef CONFIG_SKADI_OS
	k_spinlock_key_t key = k_spin_lock(&llext_lock);
#else
...
const void *llext_find_sym(const struct llext_symtable *sym_table, const char *sym_name)
{
	if (sym_table == NULL) {
		...
		for (size_t i = 0; i < sym_table->sym_cnt; i++) {
			if (sym_table->syms[i].name && strcmp(sym_table->syms[i].name, sym_name) == 0) {
