...
static void transport_evt_handler(enum shell_transport_evt evt_type, void *ctx)
{
	...
	skadi_poll_signal_raise(signal, 0);
}
...
static void shell_log_process(const struct shell *sh)
{
	...
	do {
		...
		skadi_poll_signal_check(signal, &signaled, &result);
			 ...
			 struct shell_backend_config_flags cfg_flags)
{
	...
	skadi_mutex_init(&sh->ctx->wr_mtx);

	for (int i = 0; i < SHELL_SIGNALS; i++) {
		skadi_poll_signal_init(&sh->ctx->signals[i]);
		skadi_poll_event_init(&sh->ctx->events[i],
				  K_POLL_TYPE_SIGNAL,
...
static int instance_uninit(const struct shell *sh)
{
	...
	for (int i = 0; i < SHELL_SIGNALS; i++) {
		skadi_poll_signal_cleanup(&sh->ctx->signals[i]);
	}

	skadi_mutex_cleanup(&sh->ctx->wr_mtx);
				...
				shell_signal_handler_t handler)
{
	...
	skadi_poll_signal_check(sig, &set, &res);

	if (set) {
		skadi_poll_signal_reset(sig);
		handler(sh);
	}
}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, shell_thread, void *shell_handle, void *arg_log_backend, void *arg_log_level)
{
	struct shell *sh = shell_handle;
	int err;
	struct k_poll_event *events_token = skadi_cap_ops_derive_arg(sh->ctx->events, sizeof(sh->ctx->events));
	...
	while (true) {
		...
		err = skadi_poll(events_token, SHELL_SIGNAL_TXDONE,
			     K_FOREVER);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(shell_thread)
	       ...
	       bool log_backend, uint32_t init_log_level)
{
	...
	params.new_thread = sh->thread;
	params.stack = sh->stack;
	params.stack_size = CONFIG_SHELL_STACK_SIZE;
	params.entry = SKADI_SUBSYSTEM_FUNCTION_POINTER(shell_thread);
	params.p1 = (void*)sh;
	params.p2 = (void*)log_backend;
	params.p3 = UINT_TO_POINTER(init_log_level);
	params.prio = SHELL_THREAD_PRIORITY;
	params.options = 0;
	params.delay = K_NO_WAIT;

	k_tid_t tid = skadi_thread_create(&params);

	sh->ctx->tid = tid;
	skadi_thread_name_set(tid, sh->name);
...
void shell_uninit(const struct shell *sh, shell_uninit_cb_t cb)
{
	...
	if (IS_ENABLED(CONFIG_MULTITHREADING)) {
		...
		sh->ctx->uninit_cb = cb;
		/* signal kill message */
		(void)skadi_poll_signal_raise(signal, 0);
...
{
	__ASSERT_NO_MSG(sh);
	__ASSERT(!skadi_is_in_isr(), "Thread context required.");
	__ASSERT_NO_MSG(sh->ctx);
	__ASSERT_NO_MSG(z_flag_cmd_ctx_get(sh) ||
			(skadi_current_get() != sh->ctx->tid));
...
int shell_prompt_change(const struct shell *sh, const char *prompt)
{
	...
	if (skadi_mutex_lock(&sh->ctx->wr_mtx, K_MSEC(mtx_timeout_ms))) {
		return -EBUSY;
	}
