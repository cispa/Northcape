...
#include <zephyr/skadi/subsystems/uart/skadi_uart.h>
#endif
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, async_callback, const struct device *dev, struct uart_event *evt, void *user_data)
#else
...
#endif
{
	...
	case  UART_RX_BUF_REQUEST:
	{
		...
		if (buf) {
#ifdef CONFIG_SKADI_OS
			int err = skadi_uart_rx_buf_rsp(dev, buf, len);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(async_callback)
#endif
...
static void uart_rx_handle(const struct device *dev, struct shell_uart_int_driven *sh_uart)
{
	...
	do {
		...
		if (len > 0) {
#ifdef CONFIG_SKADI_OS
			rd_len = skadi_uart_fifo_read(dev, data, len);
#else
...
#ifdef CONFIG_SKADI_OS
			rd_len = skadi_uart_fifo_read(dev, &dummy, 1);
#else
...
#ifdef CONFIG_SKADI_OS
		err = skadi_uart_line_ctrl_get(dev, UART_LINE_CTRL_DTR, &dtr);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, dtr_timer_handler, struct k_timer *timer)
{
	struct shell_uart_int_driven *sh_uart = skadi_timer_user_data_get(timer);
...
#ifdef CONFIG_SKADI_OS
	skadi_uart_irq_tx_enable(sh_uart->common.dev);
#else
	uart_irq_tx_enable(sh_uart->common.dev);
#endif
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(dtr_timer_handler)
#else
...
static void uart_tx_handle(const struct device *dev, struct shell_uart_int_driven *sh_uart)
{
...
#ifdef CONFIG_SKADI_OS
		skadi_uart_irq_tx_disable(dev);
		skadi_timer_start(&sh_uart->dtr_timer, K_MSEC(100), K_MSEC(100));
#else
	...
	if (len) {
		int err;
#ifdef CONFIG_SKADI_OS
		len = skadi_uart_fifo_fill(dev, data, len);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, uart_callback, const struct device *dev, void *user_data)
#else
...
#ifdef CONFIG_SKADI_OS
	skadi_uart_irq_update(dev);

	if (skadi_uart_irq_rx_ready(dev)) {
		uart_rx_handle(dev, sh_uart);
	}

	if (skadi_uart_irq_tx_ready(dev)) {
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(uart_callback)
#endif
...
static void irq_init(struct shell_uart_int_driven *sh_uart)
{
	...
	sh_uart->tx_busy = 0;
#ifdef CONFIG_SKADI_OS
	skadi_uart_irq_callback_user_data_set(dev, SKADI_SUBSYSTEM_FUNCTION_POINTER(uart_callback), (void *)sh_uart);
	skadi_uart_irq_rx_enable(dev);
#else
	...
	if (IS_ENABLED(CONFIG_SHELL_BACKEND_SERIAL_CHECK_DTR)) {
#ifdef CONFIG_SKADI_OS
		skadi_timer_init(&sh_uart->dtr_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(dtr_timer_handler), NULL);
		skadi_timer_user_data_set(&sh_uart->dtr_timer, (void *)sh_uart);
#else
...
{
#ifdef CONFIG_SKADI_OS
	return skadi_uart_rx_enable(dev, buf, len, 10000);
#else
...
#ifdef CONFIG_SKADI_OS
	skadi_sem_init(&sh_uart->tx_sem, 0, 1);
#else
...
#ifdef CONFIG_SKADI_OS
	err = skadi_uart_callback_set(dev, SKADI_SUBSYSTEM_FUNCTION_POINTER(async_callback), (void *)sh_uart);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, polling_rx_timeout_handler, struct k_timer *timer)
{
	uint8_t c;
	struct shell_uart_polling *sh_uart = skadi_timer_user_data_get(timer);

#ifdef CONFIG_SKADI_OS
	while (skadi_uart_poll_in(sh_uart->common.dev, &c) == 0) {
#else
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(polling_rx_timeout_handler)
#else
...
{
#ifdef CONFIG_SKADI_OS
	skadi_timer_init(&sh_uart->rx_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(polling_rx_timeout_handler), NULL);
	skadi_timer_user_data_set(&sh_uart->rx_timer, (void *)sh_uart);
	skadi_timer_start(&sh_uart->rx_timer, RX_POLL_PERIOD, RX_POLL_PERIOD);
#else
...
#ifdef CONFIG_SKADI_OS
	skadi_timer_stop(&sh_uart->dtr_timer);
	skadi_timer_cleanup(&sh_uart->dtr_timer);
	skadi_uart_irq_tx_disable(dev);
	skadi_uart_irq_rx_disable(dev);
#else
...
{
#ifdef CONFIG_SKADI_OS
	skadi_timer_stop(&sh_uart->rx_timer);
	skadi_timer_cleanup(&sh_uart->rx_timer);
#else
...
#ifdef CONFIG_SKADI_OS
		skadi_uart_irq_tx_disable(sh_uart->dev);
#else
...
#ifdef CONFIG_SKADI_OS
		skadi_uart_poll_out(sh_uart->dev, data8[i]);
#else
...
#ifdef CONFIG_SKADI_OS
	err = skadi_uart_tx(sh_uart->common.dev, data, length, SYS_FOREVER_US);
#else
...
#ifdef CONFIG_SKADI_OS
	err = skadi_sem_take(&sh_uart->tx_sem, K_FOREVER);
#else
		      ...
		      void *data, size_t length, size_t *cnt)
{
	...
	*cnt = sh_cnt;
	...
	if (sh_uart->pending_rx_req && buf_available) {
		uint8_t *inner_buf = uart_async_rx_buf_req(async_rx);
		size_t len = uart_async_rx_get_buf_len(async_rx);
		int err;

		__ASSERT_NO_MSG(inner_buf != NULL);
		atomic_dec(&sh_uart->pending_rx_req);
#ifdef CONFIG_SKADI_OS
		err = skadi_uart_rx_buf_rsp(sh_uart->common.dev, inner_buf, len);
#else
		err = uart_rx_buf_rsp(sh_uart->common.dev, inner_buf, len);
#endif
		/* If it is too late and RX is disabled then re-enable it. */
		if (err < 0) {
			if (err == -EACCES) {
				sh_uart->pending_rx_req = 0;
				err = rx_enable(sh_uart->common.dev, inner_buf, len);
			} else {
				return err;
			}
		}
