...
#ifdef CONFIG_SKADI_OS
const char *hostname_derived = NULL;

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(const char *, skadi_net_hostname_get)
{
	if(hostname_derived == NULL){
		hostname_derived = skadi_cap_ops_derive_arg_ro(hostname, strlen(hostname)+1);
	}
	return hostname_derived;
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_hostname_get)
#endif
