...
static bool skadi_net_init_spair_slab(void){
	return skadi_mem_slab_init(&spair_slab, spair_slab.buffer, spair_slab.info.block_size, spair_slab.info.num_blocks) == 0;
}

#endif /* CONFIG_NET_SOCKETPAIR_STATIC */

/* forward declaration */
static struct socket_op_vtable *spair_fd_op_vtable;
...
{
	const struct spair *remote = skadi_zvfs_get_fd_obj(spair->remote,
...
{
	struct spair *const remote = skadi_zvfs_get_fd_obj(spair->remote,
		(const struct fd_op_vtable *)spair_fd_op_vtable, 0);
	...
	return skadi_pipe_write_avail(&remote->recv_q);
}
...
{
	return skadi_pipe_read_avail(&spair->recv_q);
}
...
static void spair_delete(struct spair *spair)
{
	...
	if (spair->remote != -1) {
		remote = skadi_zvfs_get_fd_obj(spair->remote,
			(const struct fd_op_vtable *)spair_fd_op_vtable, 0);

		if (remote != NULL) {
			res = skadi_sem_take(&remote->sem, K_FOREVER);
			if (res == 0) {
				have_remote_sem = true;
				remote->remote = -1;
				res = skadi_poll_signal_raise(&remote->readable,
					SPAIR_SIG_CANCEL);
				__ASSERT(res == 0,
					"skadi_poll_signal_raise() failed: %d",
					res);
			}
		}
	}
	...
	res = skadi_poll_signal_raise(&spair->writeable, SPAIR_SIG_CANCEL);
	__ASSERT(res == 0, "skadi_poll_signal_raise() failed: %d", res);

	if (remote != NULL && have_remote_sem) {
		skadi_sem_give(&remote->sem);
	}

	skadi_sem_cleanup(&spair->sem);
	skadi_pipe_cleanup(&spair->recv_q);
	skadi_poll_signal_cleanup(&spair->readable);
	skadi_poll_signal_cleanup(&spair->writeable);

	/* ensure no private information is released to the memory pool */
	memset(spair, 0, sizeof(*spair));
#ifdef CONFIG_NET_SOCKETPAIR_STATIC
	skadi_mem_slab_free(&spair_slab, (void *)spair);
...
static struct spair *spair_new(void)
{
	...
	res = skadi_mem_slab_alloc(&spair_slab, (void **) &spair, K_NO_WAIT);
	if (res != 0) {
		spair = NULL;
	}
	...
	skadi_sem_init(&spair->sem, 1, 1);
	skadi_pipe_init(&spair->recv_q, spair->buf, sizeof(spair->buf));
	skadi_poll_signal_init(&spair->readable);
	skadi_poll_signal_init(&spair->writeable);

	/* A new socket is always writeable after creation */
	res = skadi_poll_signal_raise(&spair->writeable, SPAIR_SIG_DATA);
	__ASSERT(res == 0, "skadi_poll_signal_raise() failed: %d", res);

	spair->remote = skadi_zvfs_reserve_fd();
	if (spair->remote == -1) {
		errno = ENFILE;
		goto cleanup;
	}

	skadi_zvfs_finalize_typed_fd(spair->remote, spair,
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_socketpair, int family, int type, int proto, int *sv)
	return z_impl_zsock_socketpair(family, type, proto, sv);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_socketpair)
...
static ssize_t spair_write(void *obj, const void *buffer, size_t count)
{
	...
	res = skadi_sem_take(&spair->sem, K_NO_WAIT);
	...
	if (res < 0) {
		...
		res = skadi_sem_take(&spair->sem, K_FOREVER);
		if (res < 0) {
			errno = -res;
			res = -1;
			goto out;
	...
	res = skadi_sem_take(&remote->sem, K_NO_WAIT);
	if (res < 0) {
		if (is_nonblock) {
			errno = EAGAIN;
			res = -1;
	...
	if (will_block) {
		...
		for (int signaled = false, result = -1; !signaled;
			...
			res = skadi_poll(events, ARRAY_SIZE(events), K_FOREVER);
			if (res < 0) {
				errno = -res;
				res = -1;
				goto out;
			...
			skadi_poll_signal_check(&remote->writeable, &signaled,
					    &result);
			if (!signaled) {
				continue;
			}
	...
	res = skadi_pipe_put(&remote->recv_q, (void *)buffer, count,
			 &bytes_written, 1, K_NO_WAIT);
	__ASSERT(res == 0, "skadi_pipe_put() failed: %d", res);

	if (spair_write_avail(spair) == 0) {
		skadi_poll_signal_reset(&remote->writeable);
	}

	res = skadi_poll_signal_raise(&remote->readable, SPAIR_SIG_DATA);
	__ASSERT(res == 0, "skadi_poll_signal_raise() failed: %d", res);
...
out:
	...
	if (spair != NULL && have_local_sem) {
		skadi_sem_give(&spair->sem);
	}

	return res;
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, spair_write_ss, void *obj, const void *buffer, size_t count)
	return spair_write(obj, buffer, count);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(spair_write_ss)
...
static ssize_t spair_read(void *obj, void *buffer, size_t count)
{
	...
	if (will_block) {
		...
		for (int signaled = false, result = -1; !signaled;
			...
			res = skadi_poll(events, ARRAY_SIZE(events), K_FOREVER);
			__ASSERT(res == 0, "skadi_poll() failed: %d", res);
			...
			skadi_poll_signal_check(&spair->readable, &signaled,
					    &result);
			if (!signaled) {
				continue;
			}
	...
	res = skadi_pipe_get(&spair->recv_q, (void *)buffer, count, &bytes_read,
			 1, K_NO_WAIT);
	__ASSERT(res == 0, "skadi_pipe_get() failed: %d", res);

	if (spair_read_avail(spair) == 0 && !sock_is_eof(spair)) {
		skadi_poll_signal_reset(&spair->readable);
	}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, spair_read_ss, void *obj, void *buffer, size_t count)
	return spair_read(obj, buffer, count);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(spair_read_ss)
				 ...
				 struct k_poll_event **pev)
{
	...
	if (pfd->events & ZSOCK_POLLOUT) {
		...
		signaled = false;
		skadi_poll_signal_check(&remote->writeable, &signaled, &result);
		if (signaled) {
...
pollout_done:
	...
	if (pfd->events & ZSOCK_POLLIN) {
		...
		/* check to see if op was canceled */
		signaled = false;
		skadi_poll_signal_check(&spair->readable, &signaled, &result);
		if (signaled) {
			/* Cannot be SPAIR_SIG_DATA, because
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, spair_ioctl_ss, void *obj, unsigned int request, va_list args)
	return spair_ioctl(obj, request, args);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(spair_ioctl_ss)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, spair_close_ss, void *obj)
	return spair_close(obj);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(spair_close_ss)

static struct socket_op_vtable *spair_fd_op_vtable;

static bool skadi_posix_subsystem_Init_spair_fd_op_vtable(void){
	spair_fd_op_vtable = skadi_allocator_alloc_rw(sizeof(*spair_fd_op_vtable));

	__ASSERT_NO_MSG(spair_fd_op_vtable);

	if(!spair_fd_op_vtable){
		return false;
	}

	spair_fd_op_vtable->fd_vtable.read = SKADI_SUBSYSTEM_FUNCTION_POINTER(spair_read_ss);
	spair_fd_op_vtable->fd_vtable.write = SKADI_SUBSYSTEM_FUNCTION_POINTER(spair_write_ss);
	spair_fd_op_vtable->fd_vtable.close = SKADI_SUBSYSTEM_FUNCTION_POINTER(spair_close_ss);
	spair_fd_op_vtable->fd_vtable.ioctl = SKADI_SUBSYSTEM_FUNCTION_POINTER(spair_ioctl_ss);
		
	spair_fd_op_vtable->bind = spair_bind;
	spair_fd_op_vtable->connect = spair_connect;
	spair_fd_op_vtable->listen = spair_listen;
	spair_fd_op_vtable->accept = spair_accept;
	spair_fd_op_vtable->sendto = spair_sendto;
	spair_fd_op_vtable->sendmsg = spair_sendmsg;
	spair_fd_op_vtable->recvfrom = spair_recvfrom;
	spair_fd_op_vtable->getsockopt = spair_getsockopt;
	spair_fd_op_vtable->setsockopt = spair_setsockopt;

	return true;
}

#ifdef CONFIG_NET_SOCKETPAIR_STATIC
SKADI_SUBSYSTEM_INIT_FUNCTIONS(skadi_posix_subsystem_Init_spair_fd_op_vtable, skadi_net_init_spair_slab)
#else
SKADI_SUBSYSTEM_INIT_FUNCTIONS(skadi_posix_subsystem_Init_spair_fd_op_vtable)
#endif
