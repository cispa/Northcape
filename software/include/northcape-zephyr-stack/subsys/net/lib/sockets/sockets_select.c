...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_select, int nfds, zsock_fd_set *readfds, zsock_fd_set *writefds, zsock_fd_set *exceptfds, struct zsock_timeval *timeout)
	return z_impl_zsock_select(nfds, readfds, writefds, exceptfds, timeout);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_select)
#endif
