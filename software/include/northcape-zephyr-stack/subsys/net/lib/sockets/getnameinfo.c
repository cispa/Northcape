...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_getnameinfo, const struct sockaddr *addr, socklen_t addrlen, char *host, socklen_t hostlen, char *serv, socklen_t servlen, int flags)
		return zsock_getnameinfo(addr, addrlen, host, hostlen, serv, servlen, flags);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_getnameinfo)
#endif
