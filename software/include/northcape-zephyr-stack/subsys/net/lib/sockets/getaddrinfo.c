...
#ifdef CONFIG_SKADI_OS
	*res = skadi_allocator_calloc_rw(AI_ARR_MAX, sizeof(struct zsock_addrinfo));
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_getaddrinfo, const char *host, const char *service, const struct zsock_addrinfo *hints, struct zsock_addrinfo **res)
	return zsock_getaddrinfo(host, service, hints, res);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_getaddrinfo)
#endif
...
#ifdef CONFIG_SKADI_OS
#define ERR(e) case DNS_ ## e: { 										\
	static const char *token_ ## e = NULL; 								\
	if(!token_ ## e){													\
		token_ ## e = skadi_cap_ops_derive_arg_ro(#e, strlen(#e) + 1);	\
	}																	\
	__ASSERT_NO_MSG(token_ ##e);										\
	return token_ ##e;													\
}
#else
...
#ifdef CONFIG_SKADI_OS
	default:{
		static const char *token_default = NULL;
		if(!token_default){
			token_default = skadi_cap_ops_derive_arg_ro("EAI_UNKNOWN", strlen("EAI_UNKNOWN") + 1);
		}
		__ASSERT_NO_MSG(token_default);
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(const char *, __skadi_zsock_gai_strerror, int errcode)
		return zsock_gai_strerror(errcode);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_gai_strerror)
#endif
