...
static struct socket_op_vtable tls_sock_fd_op_vtable;
static const struct socket_op_tvable *tls_sock_fd_op_vtable_token;
...
__net_socket struct tls_context {
	...
	mbedtls_ssl_cookie_ctx *cookie_alloc;
	...
	mbedtls_ssl_context *ssl_alloc;

	/** mbedTLS configuration. */
	mbedtls_ssl_config *config;
	mbedtls_ssl_config *config_alloc;

#if defined(MBEDTLS_X509_CRT_PARSE_C)
	/** mbedTLS structure for CA chain. */
	mbedtls_x509_crt *ca_chain;
	mbedtls_x509_crt *ca_chain_alloc;

	/** mbedTLS structure for own certificate. */
	mbedtls_x509_crt *own_cert;
	mbedtls_x509_crt *own_cert_alloc;

	/** mbedTLS structure for own private key. */
	mbedtls_pk_context *priv_key;
	mbedtls_pk_context *priv_key_alloc;
...
static void tls_session_cache_reset(void)
{
	for (int i = 0; i < ARRAY_SIZE(client_cache); i++) {
		if (client_cache[i].session != NULL) {
			skadi_mbedtls_free(client_cache[i].session);
		}
	}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, tls_ctr_drbg_random, void *ctx, unsigned char *buf, size_t len)
{
	ARG_UNUSED(ctx);

#if defined(CONFIG_CSPRNG_ENABLED)
	return skadi_sys_csrand_get(buf, len);
#else
	skadi_sys_rand_get(buf, len);

	return 0;
#endif
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tls_ctr_drbg_random)
...
static void dtls_timing_set_delay(void *data, uint32_t int_ms, uint32_t fin_ms)
{
	...
	if (fin_ms != 0U) {
		ctx->snapshot = skadi_uptime_get_32();
	}
}
...
static int dtls_timing_get_delay(void *data)
{
	...
	elapsed_ms = skadi_uptime_get_32() - timing->snapshot;
...
static int tls_init(void)
{
	...
	skadi_mutex_init(&context_lock);

#if defined(MBEDTLS_SSL_CACHE_C)
	skadi_mbedtls_ssl_cache_init(&server_cache);
#endif
...
{
	return skadi_sem_count_get(&ctx->tls_established) != 0;
}
...
static inline void tls_set_max_frag_len(mbedtls_ssl_config *config, enum net_sock_type type)
{
	...
	skadi_mbedtls_ssl_conf_max_frag_len(config, mfl_code);
}
#else
static inline void tls_set_max_frag_len(mbedtls_ssl_config *config, enum net_sock_type type) {}
#endif
...
static struct tls_context *tls_alloc(void)
{
	...
	skadi_mutex_lock(&context_lock, K_FOREVER);
	...
	skadi_mutex_unlock(&context_lock);

	if (tls) {
		skadi_sem_init(&tls->tls_established, 0, 1);
		tls->ssl_alloc = skadi_allocator_alloc_rw(sizeof(*tls->ssl_alloc));
		if(!tls->ssl_alloc){
			return NULL;
		}
		tls->ssl = skadi_cap_ops_derive_arg(tls->ssl_alloc,sizeof(*tls->ssl_alloc));
		if(!tls->ssl){
			return NULL;
		}
		skadi_mbedtls_ssl_init(tls->ssl);
		tls->config_alloc = skadi_allocator_alloc_rw(sizeof(*tls->config_alloc));
		if(!tls->config_alloc){
			return NULL;
		}
		/* needs to be non-task-ID protected */
		tls->config = skadi_cap_ops_derive_arg(tls->config_alloc, sizeof(*tls->config_alloc));
		if(!tls->config){
			return NULL;
		}
		skadi_mbedtls_ssl_config_init(tls->config);
#if defined(CONFIG_NET_SOCKETS_ENABLE_DTLS)
		tls->cookie_alloc = skadi_allocator_alloc_rw(sizeof(*tls->cookie_alloc));
		if(!tls->cookie_alloc){
			return NULL;
		}
		/* needs to be non-task-ID protected */
		tls->cookie = skadi_cap_ops_derive_arg(tls->cookie_alloc, sizeof(*tls->cookie_alloc));
		if(!tls->cookie){
			return NULL;
		}
		skadi_mbedtls_ssl_cookie_init(tls->cookie);
...
#endif
#if defined(MBEDTLS_X509_CRT_PARSE_C)
		tls->ca_chain_alloc = skadi_allocator_alloc_rw(sizeof(*tls->ca_chain_alloc));
		if(!tls->ca_chain_alloc){
			return NULL;
		}
		tls->ca_chain = skadi_cap_ops_derive_arg(tls->ca_chain_alloc, sizeof(*tls->ca_chain_alloc));
		if(!tls->ca_chain){
			return NULL;
		}
		skadi_mbedtls_x509_crt_init(tls->ca_chain);
		tls->own_cert_alloc = skadi_allocator_alloc_rw(sizeof(*tls->own_cert_alloc));
		tls->own_cert = skadi_cap_ops_derive_arg(tls->own_cert_alloc, sizeof(*tls->own_cert_alloc));
		if(!tls->own_cert){
			return NULL;
		}
		skadi_mbedtls_x509_crt_init(tls->own_cert);
		tls->priv_key_alloc = skadi_allocator_alloc_rw(sizeof(*tls->priv_key_alloc));
		if(!tls->priv_key_alloc){
			return NULL;
		}
		tls->priv_key = skadi_cap_ops_derive_arg(tls->priv_key_alloc, sizeof(*tls->priv_key_alloc));
		if(!tls->priv_key){
			return NULL;
		}
		skadi_mbedtls_pk_init(tls->priv_key);
#endif

#if defined(CONFIG_MBEDTLS_DEBUG)
		skadi_mbedtls_ssl_conf_dbg(&tls->config, zephyr_mbedtls_debug, NULL);
#endif
	} else {
...
static struct tls_context *tls_clone(struct tls_context *source_tls)
{
	...
	if (target_tls->options.is_hostname_set) {
		skadi_mbedtls_ssl_set_hostname(target_tls->ssl,
					 source_tls->ssl->private_hostname);
	}
#endif
...
#if defined(CONFIG_NET_SOCKETS_ENABLE_DTLS)
	skadi_mbedtls_ssl_cookie_free(tls->cookie);
	(void)skadi_cap_ops_drop(tls->cookie);
#endif
	skadi_mbedtls_ssl_config_free(tls->config);
	(void)skadi_cap_ops_drop(tls->config);
	skadi_mbedtls_ssl_free(tls->ssl);
	(void)skadi_cap_ops_drop(tls->ssl);
#if defined(MBEDTLS_X509_CRT_PARSE_C)
	skadi_mbedtls_x509_crt_free(tls->ca_chain);
	(void)skadi_cap_ops_drop(tls->ca_chain);
	skadi_mbedtls_x509_crt_free(tls->own_cert);
	(void)skadi_cap_ops_drop(tls->own_cert);
	skadi_mbedtls_pk_free(tls->priv_key);
	(void)skadi_cap_ops_drop(tls->priv_key);
#endif

#if defined(CONFIG_NET_SOCKETS_ENABLE_DTLS)
	skadi_allocator_free(tls->cookie_alloc);
#endif
	skadi_allocator_free(tls->ssl_alloc);
	skadi_allocator_free(tls->ca_chain_alloc);
	skadi_allocator_free(tls->own_cert_alloc);
	skadi_allocator_free(tls->priv_key_alloc);
			    ...
			    mbedtls_ssl_session *session)
{
	...
	if (entry->session != NULL) {
		skadi_mbedtls_free(entry->session);
		entry->session = NULL;
	}

	(void)skadi_mbedtls_ssl_session_save(session, NULL, 0, &session_len);

	entry->session = skadi_allocator_calloc_rw(1, session_len);
	if (entry->session == NULL) {
		NET_ERR("Failed to allocate session buffer.");
		return -ENOMEM;
	}

	ret = skadi_mbedtls_ssl_session_save(session, entry->session, session_len,
				       &session_len);
	if (ret < 0) {
	...
	entry->timestamp = skadi_uptime_get();
			   ...
			   mbedtls_ssl_session *session)
{
	...
	ret = skadi_mbedtls_ssl_session_load(session, entry->session,
				       entry->session_len);
	if (ret < 0) {
			      ...
			      socklen_t addrlen)
{
	...
	skadi_mbedtls_ssl_session_init(&session);

	ret = skadi_mbedtls_ssl_get_session(context->ssl, &session);
	if (ret < 0) {
...
exit:
	skadi_mbedtls_ssl_session_free(&session);
}
				...
				socklen_t addrlen)
{
	...
	ret = skadi_mbedtls_ssl_set_session(context->ssl, &session);
	if (ret < 0) {
...
#if defined(MBEDTLS_SSL_CACHE_C)
	skadi_mbedtls_ssl_cache_free(&server_cache);
	skadi_mbedtls_ssl_cache_init(&server_cache);
#endif
}
...
{
	uint32_t elapsed = skadi_uptime_get_32() - start;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, dtls_tx, void *ctx, const unsigned char *buf, size_t len)
{
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(dtls_tx)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, dtls_rx, void *ctx, unsigned char *buf, size_t len)
{
	struct tls_context *tls_ctx = ctx;
	...
	if (tls_ctx->dtls_peer_addrlen == 0) {
		...
		if (tls_ctx->options.role == MBEDTLS_SSL_IS_SERVER) {
			...
			err = skadi_mbedtls_ssl_set_client_transport_id(
				&tls_ctx->ssl,
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(dtls_rx)
#endif /* CONFIG_NET_SOCKETS_ENABLE_DTLS */

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, tls_tx, void *ctx, const unsigned char *buf, size_t len)
{
	struct tls_context *tls_ctx = ctx;
	ssize_t sent;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tls_tx)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, tls_rx, void *ctx, unsigned char *buf, size_t len)
{
	struct tls_context *tls_ctx = ctx;
	ssize_t received;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tls_rx)
				  ...
				  struct tls_credential *ca_cert)
{
	...
	if (tls->options.cert_nocopy == TLS_CERT_NOCOPY_NONE ||
		...
		err = skadi_mbedtls_x509_crt_parse(tls->ca_chain, ca_cert->buf,
					     ca_cert->len);
	} else {
		err = skadi_mbedtls_x509_crt_parse_der_nocopy(tls->ca_chain,
							ca_cert->buf,
							ca_cert->len);
	}
...
{
#if defined(MBEDTLS_X509_CRT_PARSE_C)
	skadi_mbedtls_ssl_conf_ca_chain(tls->config, tls->ca_chain, NULL);
	skadi_mbedtls_ssl_conf_cert_profile(tls->config,
			    ...
			    struct tls_credential *own_cert)
{
	...
	if (tls->options.cert_nocopy == TLS_CERT_NOCOPY_NONE ||
		...
		err = skadi_mbedtls_x509_crt_parse(tls->own_cert,
					     own_cert->buf, own_cert->len);
	} else {
		err = skadi_mbedtls_x509_crt_parse_der_nocopy(tls->own_cert,
							own_cert->buf,
							own_cert->len);
	}
...
{
#if defined(MBEDTLS_X509_CRT_PARSE_C)
	int err = skadi_mbedtls_ssl_conf_own_cert(tls->config, tls->own_cert,
					    tls->priv_key);
	if (err != 0) {
		err = -ENOMEM;
	}
			       ...
			       struct tls_credential *priv_key)
{
	...
	err = skadi_mbedtls_pk_parse_key(tls->priv_key, priv_key->buf,
				   priv_key->len, NULL, 0,
				   SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_ctr_drbg_random), NULL);
	if (err != 0) {
		return -EINVAL;
	}
		       ...
		       struct tls_credential *psk_id)
{
	...
	int err = skadi_mbedtls_ssl_conf_psk(tls->config,
				       psk->buf, psk->len,
...
static int tls_mbedtls_reset(struct tls_context *context)
{
	...
	ret = skadi_mbedtls_ssl_session_reset(context->ssl);
	if (ret != 0) {
		return ret;
	}

	skadi_sem_reset(&context->tls_established);
				 ...
				 k_timeout_t timeout)
{
	...
	end = skadi_sys_timepoint_calc(timeout);

	while ((ret = skadi_mbedtls_ssl_handshake(context->ssl)) != 0) {
		if (ret == MBEDTLS_ERR_SSL_WANT_READ ||
			...
			/* Blocking timeout. */
			timeout = skadi_sys_timepoint_timeout(end);
			if (K_TIMEOUT_EQ(timeout, K_NO_WAIT)) {
				ret = -EAGAIN;
				break;
			}
	...
	if (ret == 0) {
		skadi_sem_give(&context->tls_established);
	}
...
static int tls_mbedtls_init(struct tls_context *context, bool is_server)
{
	...
	if (type == MBEDTLS_SSL_TRANSPORT_STREAM) {
		skadi_mbedtls_ssl_set_bio(context->ssl, context,
				    SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_tx), SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_rx), NULL);
	} else {
#if defined(CONFIG_NET_SOCKETS_ENABLE_DTLS)
		skadi_mbedtls_ssl_set_bio(context->ssl, context,
					SKADI_SUBSYSTEM_FUNCTION_POINTER(dtls_tx), SKADI_SUBSYSTEM_FUNCTION_POINTER(dtls_rx), NULL);
#else
		return -ENOTSUP;
#endif /* CONFIG_NET_SOCKETS_ENABLE_DTLS */
	}

	ret = skadi_mbedtls_ssl_config_defaults(context->config, role, type,
	...
	skadi_mbedtls_ssl_conf_legacy_renegotiation(context->config,
					   MBEDTLS_SSL_LEGACY_BREAK_HANDSHAKE);
	skadi_mbedtls_ssl_conf_renegotiation(context->config,
	...
	if (type == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
		...
		skadi_mbedtls_ssl_set_timer_cb(context->ssl,
					 &context->dtls_timing,
					 dtls_timing_set_delay,
					 dtls_timing_get_delay);
		skadi_mbedtls_ssl_conf_handshake_timeout(context->config,
		...
		if (context->options.dtls_cid.enabled) {
			ret = skadi_mbedtls_ssl_conf_cid(
					context->config,
		...
		if (role == MBEDTLS_SSL_IS_SERVER) {
			ret = skadi_mbedtls_ssl_cookie_setup(context->cookie,
						       SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_ctr_drbg_random),
						       NULL);
			if (ret != 0) {
				return -ENOMEM;
			}

			skadi_mbedtls_ssl_conf_dtls_cookies(context->config,
						      mbedtls_ssl_cookie_write,
						      mbedtls_ssl_cookie_check,
						      context->cookie);

			skadi_mbedtls_ssl_conf_read_timeout(
					context->config,
	...
	if (!is_server && !context->options.is_hostname_set) {
		skadi_mbedtls_ssl_set_hostname(context->ssl, "");
	}
#endif
	...
	if (context->options.verify_level != -1) {
		skadi_mbedtls_ssl_conf_authmode(context->config,
					  context->options.verify_level);
	}

	skadi_mbedtls_ssl_conf_rng(context->config,
			     SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_ctr_drbg_random),
			     NULL);
	...
	if (is_server && context->options.cache_enabled) {
		skadi_mbedtls_ssl_conf_session_cache(context->config, &server_cache,
	...
	skadi_mbedtls_ssl_conf_early_data(context->config, MBEDTLS_SSL_EARLY_DATA_ENABLED);
#endif

	ret = skadi_mbedtls_ssl_setup(context->ssl,
				context->config);
	if (ret != 0) {
		/* According to mbedTLS API documentation,
	...
	if (type == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
		if (context->options.dtls_cid.enabled) {
			ret = skadi_mbedtls_ssl_set_cid(context->ssl, MBEDTLS_SSL_CID_ENABLED,
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
	if (skadi_mbedtls_ssl_set_hostname(context->ssl, optval) != 0) {
		return -EINVAL;
	}
#else
	return -ENOPROTOOPT;
					...
					void *optval, socklen_t *optlen)
{
	...
	if (context->options.ciphersuites[0] == 0) {
		...
		selected_ciphers = skadi_mbedtls_ssl_list_ciphersuites();
	} else {
					...
					void *optval, socklen_t *optlen)
{
	...
	ciph = skadi_mbedtls_ssl_get_ciphersuite(context->ssl);
	if (ciph == NULL) {
		return -ENOTCONN;
	}

	*(int *)optval = skadi_mbedtls_ssl_get_ciphersuite_id(ciph);

	(void)skadi_cap_ops_drop(ciph);
...
#if defined(CONFIG_CSPRNG_ENABLED)
			skadi_sys_csrand_get(context->options.dtls_cid.cid,
				       MBEDTLS_SSL_CID_OUT_LEN_MAX);
#else
			skadi_sys_rand_get(context->options.dtls_cid.cid,
				     MBEDTLS_SSL_CID_OUT_LEN_MAX);
#endif
						     ...
						     socklen_t *optlen)
{
	...
	ret = skadi_mbedtls_ssl_get_peer_cid(context->ssl, &enabled, optval, optlen);
	if (!enabled) {
		*optlen = 0;
	}
	return ret;
					  ...
					  void *optval, socklen_t *optlen)
{
	...
	ret = skadi_mbedtls_ssl_get_peer_cid(context->ssl, &enabled,
				       cid.cid,
				       &cid.cid_len);
	if (ret) {
...
static int ztls_socket(int family, int type, int proto)
{
	...
	skadi_zvfs_finalize_typed_fd(fd, ctx, (const struct fd_op_vtable *)tls_sock_fd_op_vtable_token,
			    ZVFS_MODE_IFSOCK);
...
int ztls_close_ctx(struct tls_context *ctx)
{
	...
	(void)skadi_mbedtls_ssl_close_notify(ctx->ssl);
		    ...
		    socklen_t *addrlen)
{
	...
	fd = skadi_zvfs_reserve_fd();
	if (fd < 0) {
		return -1;
	}


	skadi_mutex_unlock(parent->lock);
	sock = zsock_accept(parent->sock, addr, addrlen);
	skadi_mutex_lock(parent->lock, K_FOREVER);
	if (sock < 0) {
		ret = -errno;
		goto error;
	}
	...
	skadi_zvfs_finalize_typed_fd(fd, child, (const struct fd_op_vtable *)tls_sock_fd_op_vtable_token,
			    ZVFS_MODE_IFSOCK);
			...
			size_t len, int flags)
{
	...
	do {
		ret = skadi_mbedtls_ssl_write(ctx->ssl, buf, len);
		if (ret >= 0) {
			return ret;
		}
...
static bool sendmsg_lock_initialized;
					   ...
					   int flags)
{
	...
	if(!sendmsg_lock_initialized){
		skadi_mutex_init(&sendmsg_lock);
		sendmsg_lock_initialized=true;
	}

	skadi_mutex_lock(&sendmsg_lock, K_FOREVER);
	...
	for (int i = 0; i < msg->msg_iovlen; i++) {
		...
		if (vec->iov_len > 0) {
			if (len + vec->iov_len > sizeof(sendmsg_buf)) {
				skadi_mutex_unlock(&sendmsg_lock);
				errno = EMSGSIZE;
				return -1;
			}
			...
			size_t max_len, int flags)
{
	...
	do {
		...
		ret = skadi_mbedtls_ssl_read(ctx->ssl, (uint8_t *)buf + recv_len,
				       read_len);
		if (ret < 0) {
			...
			if (ret == MBEDTLS_ERR_SSL_WANT_READ ||
				...
				/* Block. */
				skadi_mutex_unlock(ctx->lock);
				ret = wait_for_reason(ctx->sock, timeout_ms, ret);
				skadi_mutex_lock(ctx->lock, K_FOREVER);
				    ...
				    socklen_t *addrlen)
{
	...
	do {
		...
		ret = skadi_mbedtls_ssl_read(&ctx->ssl, buf, max_len);
		if (ret < 0) {
			if (ret == MBEDTLS_ERR_SSL_WANT_READ ||
			    ret == MBEDTLS_ERR_SSL_WANT_WRITE ||
			    ret == MBEDTLS_ERR_SSL_ASYNC_IN_PROGRESS ||
		 ...
		 */
		remaining = skadi_mbedtls_ssl_get_bytes_avail(&ctx->ssl);
		...
		for (int i = 0; i < remaining; i++) {
			...
			err = skadi_mbedtls_ssl_read(&ctx->ssl, &byte, sizeof(byte));
			if (err <= 0) {
...
static int ztls_poll_prepare_pollin(struct tls_context *ctx)
{
	 ...
	 */
	if (!ctx->is_listening) {
		if (skadi_mbedtls_ssl_get_bytes_avail(ctx->ssl) > 0) {
			return -EALREADY;
		}
	}
				 ...
				 struct k_poll_event *pev_end)
{
	...
	obj = skadi_zvfs_get_fd_obj_and_vtable(
	...
	(void)skadi_mutex_lock(lock, K_FOREVER);

	ret = skadi_zvfs_fdtable_call_ioctl(vtable, obj, ZFD_IOCTL_POLL_PREPARE,
				   pfd, pev, pev_end);
...
static int ztls_socket_data_check(struct tls_context *ctx)
{
	...
	ret = skadi_mbedtls_ssl_read(ctx->ssl, NULL, 0);
	if (ret < 0) {
		if (ret == MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY) {
	...
	return skadi_mbedtls_ssl_get_bytes_avail(ctx->ssl);
}
				...
				struct k_poll_event **pev)
{
	...
	if ((pfd->events & ZSOCK_POLLIN) &&
		...
		if ((*pev)->state != K_POLL_STATE_NOT_READY) {
			ret = skadi_zvfs_fdtable_call_ioctl(vtable, obj,
	...
	ret = skadi_zvfs_fdtable_call_ioctl(vtable, obj, ZFD_IOCTL_POLL_UPDATE,
				   pfd, pev);
...
static int ztls_poll_offload(struct zsock_pollfd *fds, int nfds, int timeout)
{
	...
	ctx = skadi_zvfs_get_fd_obj_and_vtable(fds[0].fd,
				      (const struct fd_op_vtable **)&vtable,
				      NULL);
	if (ctx == NULL) {
		errno = EINVAL;
	...
	do {
		...
		ret = skadi_zvfs_fdtable_call_ioctl(vtable, ctx, ZFD_IOCTL_POLL_OFFLOAD,
					   fds, nfds, remaining);
		if (ret < 0) {
			goto exit;
		}
		...
		for (i = 0; i < nfds; i++) {
			ctx = skadi_zvfs_get_fd_obj(fd_backup[i],
					   (const struct fd_op_vtable *)
							&tls_sock_fd_op_vtable,
					   0);
			if (ctx != NULL) {
...
mbedtls_ssl_context *ztls_get_mbedtls_ssl_context(int fd)
{
	...
	ctx = skadi_zvfs_get_fd_obj(fd, (const struct fd_op_vtable *)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, tls_sock_read_vmeth, void *obj, void *buffer, size_t count)
{
	return ztls_recvfrom_ctx(obj, buffer, count, 0, NULL, 0);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tls_sock_read_vmeth)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, tls_sock_write_vmeth, void *obj, const void *buffer, size_t count)
{
	return ztls_sendto_ctx(obj, buffer, count, 0, NULL, 0);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tls_sock_write_vmeth)


SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_ALLOW_SELF(int, tls_sock_ioctl_vmeth, void *obj, unsigned int request, va_list args)
{
	...
	case F_SETFL: {
		...
		fd_obj = skadi_zvfs_get_fd_obj_and_vtable(ctx->sock,
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tls_sock_ioctl_vmeth)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_ALLOW_SELF(int, tls_sock_close_vmeth, void *obj)
{
	return ztls_close_ctx(obj);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tls_sock_close_vmeth)
...
static bool skadi_init_fd_op_table(void){
	tls_sock_fd_op_vtable_token = skadi_cap_ops_derive_arg_ro(&tls_sock_fd_op_vtable, sizeof(tls_sock_fd_op_vtable));

	__ASSERT_NO_MSG(tls_sock_fd_op_vtable_token);
	if(!tls_sock_fd_op_vtable_token){
		return false;
	}

	tls_sock_fd_op_vtable.fd_vtable.read = SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_sock_read_vmeth);
	tls_sock_fd_op_vtable.fd_vtable.write = SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_sock_write_vmeth);
	tls_sock_fd_op_vtable.fd_vtable.close = SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_sock_close_vmeth);
	tls_sock_fd_op_vtable.fd_vtable.ioctl = SKADI_SUBSYSTEM_FUNCTION_POINTER(tls_sock_ioctl_vmeth);

	tls_sock_fd_op_vtable.shutdown = tls_sock_shutdown_vmeth;
	tls_sock_fd_op_vtable.bind = tls_sock_bind_vmeth;
	tls_sock_fd_op_vtable.connect = tls_sock_connect_vmeth;
	tls_sock_fd_op_vtable.listen = tls_sock_listen_vmeth;
	tls_sock_fd_op_vtable.accept = tls_sock_accept_vmeth;
	tls_sock_fd_op_vtable.sendto = tls_sock_sendto_vmeth;
	tls_sock_fd_op_vtable.sendmsg = tls_sock_sendmsg_vmeth;
	tls_sock_fd_op_vtable.recvfrom = tls_sock_recvfrom_vmeth;
	tls_sock_fd_op_vtable.getsockopt = tls_sock_getsockopt_vmeth;
	tls_sock_fd_op_vtable.setsockopt = tls_sock_setsockopt_vmeth;
	tls_sock_fd_op_vtable.getpeername = tls_sock_getpeername_vmeth;
	tls_sock_fd_op_vtable.getsockname = tls_sock_getsockname_vmeth;

	return true;
}
SKADI_SUBSYSTEM_INIT_FUNCTIONS(skadi_init_fd_op_table);
