...
/* this is only used to call socket functions defined in the net subsystem */
...
struct socket_op_vtable sock_fd_op_vtable;
const struct socket_op_vtable *sock_fd_op_vtable_token;
				    ...
				    struct k_mutex **lock)
{
	...
	ctx = skadi_zvfs_get_fd_obj_and_vtable(sock,
...
static ssize_t zsock_handle_zerocopy_recv_packet(struct net_context *context, struct net_pkt *pkt, void *buf){
	const void **out_ptr = buf;
	void *derived;
	ssize_t pkt_size = pkt->buffer->len;

	pkt_size -= (size_t)(pkt->cursor.pos - pkt->cursor.buf->data);

	if(!pkt_size){
		/* 0 bytes in the packet... */
		net_pkt_unref(pkt);
		*out_ptr = 0;
		return 0;
	}

	__ASSERT_NO_MSG(pkt_size > 0);

	__ASSERT(!pkt->buffer->frags, "Pkt %p is not linear!", pkt);

	derived = (void*) skadi_cap_ops_derive_arg_ro(pkt->cursor.pos, pkt_size);

	__ASSERT_NO_MSG(derived);

	pkt->indirect_cap = (void*) derived;

	sys_slist_append(&context->zerocopy_recv_queue, &pkt->next);

	LOG_DBG("Derived buffer %p with len %zd\n", derived, pkt_size);
...
{
	// TODO this works as long as FIFO and queue have same layout...
	struct k_poll_event events[] = {
		K_POLL_EVENT_INITIALIZER(K_POLL_TYPE_FIFO_DATA_AVAILABLE,
					 K_POLL_MODE_NOTIFY_ONLY, (struct k_fifo *)&fifo->_queue),
	};

	return skadi_poll(events, ARRAY_SIZE(events), timeout);
}
...
static void zsock_flush_queue(struct net_context *ctx)
{
	...
	while ((p = skadi_fifo_get(ctx->recv_q, K_NO_WAIT)) != NULL) {
		if (is_listen) {
	...
	skadi_fifo_cancel_wait(ctx->recv_q);

	/* Wake reader if it was sleeping */
	(void)skadi_condvar_signal(ctx->cond.recv);
}
...
static int zsock_socket_internal(int family, int type, int proto)
{
	...
	ctx->recv_q = skadi_allocator_alloc_rw(sizeof(*ctx->recv_q));
	
	__ASSERT_NO_MSG(ctx->recv_q);
	if(!ctx->recv_q){
		skadi_zvfs_free_fd(fd);
		errno = -ENOMEM;
		return -1;
	...
	skadi_fifo_init(ctx->recv_q);

	ctx->cond.recv = skadi_allocator_alloc_rw(sizeof(*ctx->cond.recv));
	
	__ASSERT_NO_MSG(ctx->cond.recv);
	if(!ctx->cond.recv){
		skadi_zvfs_free_fd(fd);
		errno = -ENOMEM;
		return -1;
	 ...
	 */
	skadi_condvar_init(ctx->cond.recv);
	...
	__ASSERT_NO_MSG(sock_fd_op_vtable_token);

	skadi_zvfs_finalize_typed_fd(fd, ctx, (const struct fd_op_vtable *)sock_fd_op_vtable_token,
			    ZVFS_MODE_IFSOCK);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_socket, int family, int type, int proto)
	return z_impl_zsock_socket(family, type, proto);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_socket)
...
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_ARGS_ALLOW_SELF(int, skadi_close_trampoline, void *ctx);
...
int z_impl_zsock_close(int sock)
{
	...
	skadi_subsystem_check_function_pointer(vtable->fd_vtable.close, false, true);

	ret = skadi_close_trampoline(ctx, vtable->fd_vtable.close);

	skadi_mutex_unlock(lock);

	SYS_PORT_TRACING_OBJ_FUNC_EXIT(socket, close, sock, ret < 0 ? -errno : ret);

	skadi_zvfs_free_fd(sock);

	(void)sock_obj_core_dealloc(sock);

	return ret;
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_close, int fd)
	return zsock_close(fd);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_close)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_shutdown, int sock, int how)
	return z_impl_zsock_shutdown(sock, how);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_shutdown)
...
static void zsock_accepted_cb(struct net_context *new_ctx,
	...
	if (status == 0) {
		...
		__ASSERT_NO_MSG(new_ctx->recv_q);

		skadi_fifo_init(new_ctx->recv_q);
		skadi_condvar_init(new_ctx->cond.recv);

		__ASSERT_NO_MSG(parent->accept_q);

		skadi_fifo_put(parent->accept_q, new_ctx);
		...
		(void)skadi_condvar_signal(parent->cond.recv);
	}
...
{
	if (ctx->cond.lock) {
		(void)skadi_mutex_lock(ctx->cond.lock, K_FOREVER);
	}
	...
	/* if pkt is NULL, EOF */
	if (!pkt) {
		struct net_pkt *last_pkt = skadi_fifo_peek_tail(ctx->recv_q);
...
unlock:
	...
	if (ctx->cond.lock) {
		(void)skadi_mutex_unlock(ctx->cond.lock);
	}
}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_bind, int sock, const struct sockaddr *addr, socklen_t addrlen)
	return z_impl_zsock_bind(sock, addr, addrlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_bind)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_connect, int sock, const struct sockaddr *addr, socklen_t addrlen)
	return z_impl_zsock_connect(sock, addr, addrlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_connect)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_listen, int sock, int backlog)
	return z_impl_zsock_listen(sock, backlog);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_listen)
		     ...
		     socklen_t *addrlen)
{
	...
	ctx = skadi_fifo_get(parent->accept_q, K_NO_WAIT);
	if (ctx == NULL) {
		errno = EAGAIN;
		return -1;
	}
	...
	last_pkt = skadi_fifo_peek_tail(ctx->recv_q);
	if (last_pkt) {
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_accept, int sock, struct sockaddr *addr, socklen_t *addrlen)
	return z_impl_zsock_accept(sock, addr, addrlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_accept)
			       ...
			       uint32_t *retry_timeout)
{
	 ...
	 */
	if (skadi_sys_timepoint_expired(buf_timeout)) {
		if (status == -ENOBUFS) {
			status = -ENOMEM;
		} else {
			status = -ENOBUFS;
	...
	if (status == -ENOBUFS) {
		...
		skadi_sleep(K_MSEC(*retry_timeout));
	}
	...
	if (status == -EAGAIN) {
		if (IS_ENABLED(CONFIG_NET_NATIVE_TCP) &&
			...
			skadi_poll_event_init(&event,
					  K_POLL_TYPE_SEM_AVAILABLE,
					  K_POLL_MODE_NOTIFY_ONLY,
					  net_tcp_tx_sem_get(ctx));

			skadi_poll(&event, 1, K_MSEC(*retry_timeout));
		} else {
			skadi_sleep(K_MSEC(*retry_timeout));
		}
	}
			 ...
			 const struct sockaddr *dest_addr, socklen_t addrlen)
{
	...
	if ((flags & ZSOCK_MSG_DONTWAIT) || sock_is_nonblock(ctx)) {
		timeout = K_NO_WAIT;
		buf_timeout = skadi_sys_timepoint_calc(K_NO_WAIT);
	} else {
		net_context_get_option(ctx, NET_OPT_SNDTIMEO, &timeout, NULL);
		buf_timeout = skadi_sys_timepoint_calc(MAX_WAIT_BUFS);
	}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, __skadi_zsock_sendto, int sock, void *buf, size_t len, int flags, const struct sockaddr *dest_addr, socklen_t addrlen)
	return z_impl_zsock_sendto(sock, buf, len, flags, dest_addr, addrlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_sendto)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, __skadi_zsock_sendmsg, int sock, const struct msghdr *msg, int flags)
	return z_impl_zsock_sendmsg(sock, msg, flags);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_sendmsg)
...
int zsock_wait_data(struct net_context *ctx, k_timeout_t *timeout)
{
	...
	if (skadi_fifo_is_empty(ctx->recv_q)) {
		/* Wait for the data to arrive but without holding a lock */
		ret = skadi_condvar_wait(ctx->cond.recv, ctx->cond.lock,
				     *timeout);
		if (ret < 0) {
			return ret;
		}
				       ...
				       socklen_t *addrlen)
{
	...
	if (flags & ZSOCK_MSG_PEEK) {
		...
		pkt = skadi_fifo_peek_head(ctx->recv_q);
	} else {
		pkt = skadi_fifo_get(ctx->recv_q, timeout);
	}
	...
	if (msg != NULL) {
		...
		if(IS_ENABLED(CONFIG_SKADI_NET_ZEROCOPY) && (flags & ZSOCK_MSG_ZEROCOPY)){
			if(flags != ZSOCK_MSG_ZEROCOPY){
				LOG_ERR("Invalid flag combination: %d", flags);
				net_pkt_unref(pkt);
				return -EINVAL;
			}
			...
			if ((IS_ENABLED(CONFIG_NET_PKT_RXTIME_STATS) ||
				...
				net_socket_update_tc_rx_time(pkt, skadi_cycle_get_32());
			}

			return zsock_handle_zerocopy_recv_packet(ctx, pkt, buf);
		}
	...
	while (_max_len > 0) {
		...
		if (do_recv && len > 0) {
			...
			if(!IS_ENABLED(CONFIG_SKADI_NET_ZEROCOPY) || !(flags & ZSOCK_MSG_ZEROCOPY)){
				...
				*buf += len;
				...
				// will consume the entire packet below and return the buffer pointer
				len = pkt_len;
			}
		...
		if (do_recv && !peek) {
			if (len == pkt_len) {
				...
				pkt = skadi_fifo_get(ctx->recv_q, K_NO_WAIT);
				if (net_pkt_eof(pkt)) {
					sock_set_eof(ctx);
				}
			...
			skadi_fifo_put(ctx->recv_q, skadi_fifo_get(ctx->recv_q, K_NO_WAIT));
		}
	}
				       ...
				       int flags, k_timeout_t timeout)
{
	...
	for (end = skadi_sys_timepoint_calc(timeout); max_len > 0; timeout = skadi_sys_timepoint_timeout(end)) {
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, __skadi_zsock_recvfrom, int sock, void *buf, size_t max_len, int flags, struct sockaddr *src_addr, socklen_t *addrlen)
	__ASSERT((!src_addr && !addrlen) || (src_addr && addrlen), "srcaddr and addrlen inconsistent: src_addr %p addrlen %p", src_addr, addrlen);
	return z_impl_zsock_recvfrom(sock, buf, max_len, flags, src_addr, addrlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_recvfrom)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, __skadi_zsock_recvmsg, int sock, struct msghdr *msg, int flags)
	/* currently not supported */
	return -ENOSYS;
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_recvmsg)
...
int z_impl_zsock_fcntl_impl(int sock, int cmd, int flags)
{
	...
	ret = skadi_zvfs_fdtable_call_ioctl((const struct fd_op_vtable *)vtable,
				   obj, cmd, flags);
...
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_ARGS_VALIST(int, skadi_ioctl_trampoline, SKADI_SUBSYSTEM_REMOVE_PARENTHESIS(obj, request), void *obj, unsigned int request)
...
int z_impl_zsock_ioctl_impl(int sock, unsigned long request, va_list args)
{
	...
	ret = skadi_ioctl_trampoline(ctx, request, args, vtable->fd_vtable.ioctl);

	skadi_subsystem_check_function_pointer(vtable->fd_vtable.ioctl, false, true);
				  ...
				  struct k_poll_event *pev_end)
{
	if (pfd->events & ZSOCK_POLLIN) {
		...
		/* we need the tokens here for this to make sense to the scheduler */
		(*pev)->obj = &ctx->recv_q->_queue;
...
int zsock_poll_internal(struct zsock_pollfd *fds, int nfds, k_timeout_t timeout)
{
	...
	int ret = 0;
	int i;
	struct zsock_pollfd *pfd, *pfd_token;
	struct k_poll_event poll_events[CONFIG_NET_SOCKETS_POLL_MAX];
	struct k_poll_event *pev, *pev_token, **pev_token_ptr;
	struct k_poll_event *pev_end_token;
	const struct fd_op_vtable *vtable;
	struct k_mutex *lock;
	k_timepoint_t end;
	bool offload = false;
	...
	for (pfd = fds, i = nfds; i--; pfd++) {
		...
		(void)skadi_mutex_lock(lock, K_FOREVER);
		pfd_token = skadi_cap_ops_derive_arg(pfd, sizeof(*pfd));
		pev_token = skadi_cap_ops_derive_arg(poll_events, sizeof(poll_events));
		pev_end_token = pev_token + ARRAY_SIZE(poll_events);
		pev_token = (struct k_poll_event *)((uintptr_t)pev_token + (uintptr_t)pev - (uintptr_t)poll_events);
		pev_token_ptr = skadi_cap_ops_derive_arg(&pev_token, sizeof(&pev_token));

		__ASSERT_NO_MSG(pfd_token);
		__ASSERT_NO_MSG(pev_token);
		__ASSERT_NO_MSG(pev_token_ptr);

		result = skadi_zvfs_fdtable_call_ioctl(vtable, ctx,
					      ZFD_IOCTL_POLL_PREPARE,
					      pfd_token, pev_token_ptr, pev_end_token);

		pev = poll_events + (skadi_get_capability_offset(pev_token) / sizeof(*pev));
		
		skadi_cap_ops_drop(pfd_token);
		skadi_cap_ops_drop(pev_token);
		skadi_cap_ops_drop(pev_token_ptr);
	...
	if (offload) {
		...
		return skadi_zvfs_fdtable_call_ioctl(offl_vtable, offl_ctx,
	...
	do {
		ret = skadi_poll(poll_events, pev - poll_events, timeout);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_poll, struct zsock_pollfd *fds, int nfds, int poll_timeout)
	return z_impl_zsock_poll(fds, nfds, poll_timeout);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_poll)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_inet_pton, sa_family_t family, const char *src, void *dst)
	return z_impl_zsock_inet_pton(family, src, dst);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_inet_pton)
			 ...
			 void *optval, socklen_t *optlen)
{
	...
	case SOL_SOCKET:
		...
		case SO_TIMESTAMPING:
			...
			break;
		case SO_LAST_TXTIME:
			if (IS_ENABLED(CONFIG_NET_CONTEXT_TIMESTAMP_SOCKOPT)) {
				ret = net_context_get_option(ctx,
							     NET_OPT_LAST_TXTIME,
							     optval, optlen);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_getsockopt, int sock, int level, int optname, void *optval, socklen_t *optlen)
	return z_impl_zsock_getsockopt(sock, level, optname, optval, optlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_getsockopt)
			 ...
			 const void *optval, socklen_t optlen)
{
	...
	case SOL_SOCKET:
		...
		case SO_ZEROCOPY:
			if (IS_ENABLED(CONFIG_SKADI_NET_ZEROCOPY)) {
				ret = net_context_set_option(ctx,
								 NET_OPT_ZEROCOPY,
							     optval, optlen);
		...
		case SO_ZEROCOPY_RX_CALLBACK:
			if (IS_ENABLED(CONFIG_SKADI_NET_ZEROCOPY)) {
				ret = net_context_set_option(ctx,
								 NET_OPT_ZEROCOPY_RX_CB,
							     optval, optlen);
		...
		case SO_ZEROCOPY_TX_CALLBACK:
			if (IS_ENABLED(CONFIG_SKADI_NET_ZEROCOPY)) {
				ret = net_context_set_option(ctx,
								 NET_OPT_ZEROCOPY_TX_CB,
							     optval, optlen);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_setsockopt, int sock, int level, int optname, const void *optval, socklen_t optlen)
	return z_impl_zsock_setsockopt(sock, level, optname, optval, optlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_setsockopt)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_getpeername, int sock, struct sockaddr *addr, socklen_t *addrlen)
	return z_impl_zsock_getpeername(sock, addr, addrlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_getpeername)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zsock_getsockname, int sock, struct sockaddr *addr, socklen_t *addrlen)
	return z_impl_zsock_getsockname(sock, addr, addrlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zsock_getsockname)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, sock_read_vmeth, void *obj, void *buffer, size_t count)
{
	return zsock_recvfrom_ctx(obj, buffer, count, 0, NULL, 0);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(sock_read_vmeth)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, sock_write_vmeth, void *obj, const void *buffer, size_t count)
{
	return zsock_sendto_ctx(obj, buffer, count, 0, NULL, 0);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(sock_write_vmeth)
...
// we call this from this subsystem as well
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_ALLOW_SELF(int, sock_ioctl_vmeth, void *obj, unsigned int request, va_list args)
{
	switch (request) {
	...
	case ZFD_IOCTL_ZEROCOPY_GETBUF: {
		/* output pointer - capability for output buffer goes here */
		void **outptr = va_arg(args, void **);
		/* output size - size of the buffer*/
		ssize_t read_size, *out_size = va_arg(args, ssize_t *);
		struct sockaddr *sockaddr = va_arg(args, struct sockaddr *);
		socklen_t *socklen = va_arg(args, socklen_t *);
#if defined(CONFIG_NET_PKT_TIMESTAMP) || defined(CONFIG_NET_PKT_TXTIME)
		struct net_ptp_time *timestamp = va_arg(args, struct net_ptp_time*);
		struct net_context *ctx = (struct net_context *) obj;
		sys_slist_t *list_head = &ctx->zerocopy_recv_queue;
		struct net_pkt *pkt, *tmp;
#endif
		/* any size is OK */
		read_size = zsock_recvfrom_ctx(obj, outptr, -1, ZSOCK_MSG_ZEROCOPY,
			sockaddr, socklen);
		
		*out_size = read_size;
#if defined(CONFIG_NET_PKT_TIMESTAMP) || defined(CONFIG_NET_PKT_TXTIME)
		if(timestamp){
			SYS_SLIST_FOR_EACH_CONTAINER_SAFE(list_head, pkt, tmp, next){
				memcpy(timestamp, &pkt->timestamp, sizeof(*timestamp));
			}
		}
#endif
	...
	case ZFD_IOCTL_ZEROCOPY_FREEBUF: {
		/* input pointer - the capability we are supposed to free*/
		void *in_token = va_arg(args, void *);

		net_context_zerocopy_free_recv_packet((struct net_context *) obj, in_token);

		return IS_ENABLED(CONFIG_SKADI_NET_ZEROCOPY) ? 0 : -EINVAL;
	}
	default:
		errno = EOPNOTSUPP;
		return -1;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(sock_ioctl_vmeth)
...
// called from this subsystem as well
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_ALLOW_SELF(int, sock_close_vmeth, void *obj)
{
	return zsock_close_ctx(obj);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(sock_close_vmeth)
...
static bool skadi_init_vtable(void){
	/* TODO reveals some function pointers that the fd subsystem does not need ... */
	sock_fd_op_vtable_token = skadi_cap_ops_derive_arg_ro(&sock_fd_op_vtable, sizeof(sock_fd_op_vtable));
	
	__ASSERT_NO_MSG(sock_fd_op_vtable_token);

	sock_fd_op_vtable.fd_vtable.read = SKADI_SUBSYSTEM_FUNCTION_POINTER(sock_read_vmeth);
	sock_fd_op_vtable.fd_vtable.write = SKADI_SUBSYSTEM_FUNCTION_POINTER(sock_write_vmeth);
	sock_fd_op_vtable.fd_vtable.close = SKADI_SUBSYSTEM_FUNCTION_POINTER(sock_close_vmeth);
	sock_fd_op_vtable.fd_vtable.ioctl = SKADI_SUBSYSTEM_FUNCTION_POINTER(sock_ioctl_vmeth);

	return sock_fd_op_vtable_token != NULL;
}

SKADI_SUBSYSTEM_INIT_FUNCTIONS(skadi_init_vtable);
