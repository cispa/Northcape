...
void http_server_release_client(struct http_client_ctx *client)
{
	int i;
#ifdef CONFIG_SKADI_OS
	struct k_work_sync *sync = skadi_allocator_alloc_rw(sizeof(*sync));
...
#ifdef CONFIG_SKADI_OS
	skadi_work_cancel_delayable_sync(client->inactivity_timer, sync);

	skadi_allocator_free(sync);
#else
...
#ifdef CONFIG_SKADI_OS
	skadi_work_cleanup_delayable(&client->inactivity_timer);
#endif
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, client_timeout, struct k_work *work)
#else
...
#ifdef CONFIG_SKADI_OS
	struct http_client_ctx *client = work->user_data;
#else
	struct http_client_ctx *client =
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(client_timeout)
#endif
...
void http_client_timer_restart(struct http_client_ctx *client)
{
	__ASSERT_NO_MSG(IS_ARRAY_ELEMENT(server_ctx.clients, client));
#ifdef CONFIG_SKADI_OS
	skadi_work_reschedule(client->inactivity_timer, INACTIVITY_TIMEOUT);
#else
...
#ifdef CONFIG_SKADI_OS
	client->inactivity_timer = skadi_allocator_alloc_rw(sizeof(*client->inactivity_timer));
	if(!client->inactivity_timer){
		LOG_ERR("Could not allocate inactivity timer!");
		return;
	}
	skadi_work_init_delayable(client->inactivity_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(client_timeout));
#else
