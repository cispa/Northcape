...
struct ptp_msg *ptp_msg_from_pkt(struct net_pkt *pkt)
{
	...
	/* have to check like this, since ethernet header is forcibly removed after TX */
	if(net_pkt_ll_proto_type(pkt) != ETH_P_IP && net_pkt_ll_proto_type(pkt) != ETH_P_IPV6){
		/* not "our" PTP */
		net_pkt_cursor_restore(pkt, &backup);
		return NULL;
	}

	if(net_pkt_ll_proto_type(pkt) == ETH_P_IP){
	...
	if(net_pkt_ll_proto_type(pkt) == ETH_P_IPV6){

		ret = net_pkt_read(pkt, &ip6_hdr, sizeof(ip6_hdr));

		if(ret){
			LOG_ERR("Could not read IPv6 header!");
			net_pkt_cursor_restore(pkt, &backup);
			return NULL;
		}
