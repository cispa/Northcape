...
#ifdef SKADI_SUBSYSTEM
	skadi_mutex_init(&credential_lock);
#else
...
{
#ifdef SKADI_SUBSYSTEM
	skadi_mutex_lock(&credential_lock, K_FOREVER);
#else
...
{
#ifdef SKADI_SUBSYSTEM
	skadi_mutex_unlock(&credential_lock);
#else
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_tls_credential_add, sec_tag_t tag, enum tls_credential_type type, const void *cred, size_t credlen);
	return tls_credential_add(tag, type, cred, credlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_tls_credential_add)
#endif
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_tls_credential_get, sec_tag_t tag, enum tls_credential_type type, void *cred, size_t *credlen);
	return tls_credential_get(tag, type, cred, credlen);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_tls_credential_get)
#endif
...
int tls_credential_delete(sec_tag_t tag, enum tls_credential_type type)
{
	...
	(void)skadi_cap_ops_drop(credential->buf);
#endif
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_tls_credential_delete, sec_tag_t tag, enum tls_credential_type type);
	return tls_credential_delete(tag, type);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_tls_credential_delete)
#endif
