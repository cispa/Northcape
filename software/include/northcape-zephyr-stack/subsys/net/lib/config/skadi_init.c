...
#define LOOP_DIVIDER 10
	...
	if (check_interface(iface) == false) {
		skadi_sem_init(counter, 1, K_SEM_MAX_LIMIT);

		while (count-- > 0) {
			if (!skadi_sem_count_get(counter)) {
				break;
			}

			if (skadi_sem_take(waiter, K_MSEC(loop))) {
				if (!skadi_sem_count_get(counter)) {
					break;
				}
			}
	...
	while (!services_are_ready(flags) && count-- > 0) {
		skadi_sem_take(waiter, K_MSEC(loop));
	}
...
int net_config_init_app(const struct device *dev, const char *app_info)
{
	...
	waiter = skadi_allocator_alloc_rw(sizeof(*waiter));
	counter = skadi_allocator_alloc_rw(sizeof(*counter));

	if(!waiter || !counter){
		LOG_ERR("Could not allocate sems!");
		return -ENOMEM;
	}

	skadi_sem_init(waiter, 0, 1);
	skadi_sem_init(counter, 0, UINT_MAX);	
