...
NET_SOCKET_SERVICE_SYNC_DEFINE_STATIC(svc_udp, NULL, NULL,
				      SOCK_ID_MAX);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_ARGS(udp_callback_wrapper, enum zperf_status status, struct zperf_results *result, void *user_data);

static inline void call_udp_callback(enum zperf_status status, struct zperf_results *result, void *user_data, zperf_callback callback){
	...
	udp_callback_wrapper(status, result, user_data, callback);
...
#ifdef CONFIG_SKADI_OS
	ret = skadi_zsock_sendto(sock, reply, sizeof(reply), 0, addr,
			 ...
			 size_t datalen)
{
	...
	LOG_HEXDUMP_DBG(data, datalen, "Received UDP buffer!");
...
#ifdef CONFIG_SKADI_OS
				call_udp_callback(ZPERF_SESSION_STARTED, NULL, udp_user_data, udp_session_cb);
#else
	...
	case STATE_ONGOING:
		if (id < 0) { /* Negative id means session end. */
			...
			if (udp_session_cb != NULL) {
				...
				if(results_token){
					call_udp_callback(ZPERF_SESSION_FINISHED, results_token, udp_user_data, udp_session_cb);
...
static void zperf_udp_join_mcast_ipv4(char *if_name, struct in_addr *addr)
{
	...
	if (if_name[0]) {
		iface = skadi_net_if_get_by_index(skadi_net_if_get_by_name(if_name));
		if (iface == NULL) {
			iface = skadi_net_if_get_default();
		}
	} else {
		iface = skadi_net_if_get_default();
	}
	...
	if (iface != NULL) {
		skadi_net_ipv4_igmp_join(iface, addr, NULL);
	}
}
...
static void zperf_udp_join_mcast_ipv6(char *if_name, struct in6_addr *addr)
{
	...
	if (iface != NULL) {
		skadi_net_ipv6_mld_join(iface, addr);
	}
}
#else
static void zperf_udp_join_mcast_ipv6(char *if_name, struct in6_addr *addr)
...
#ifdef CONFIG_SKADI_OS
	skadi_zsock_getsockname(sock, &addr, &addr_len);
#else
...
#ifdef CONFIG_SKADI_OS
			skadi_net_ipv4_igmp_leave(iface, &addr4->sin_addr);
#else
...
#ifdef CONFIG_SKADI_OS
			skadi_net_ipv6_mld_leave(iface, &addr6->sin6_addr);
#else
...
static void udp_receiver_cleanup(void)
{
	int i;
#ifdef CONFIG_SKADI_OS
	(void)skadi_net_socket_service_unregister(&svc_udp);
#else
...
static int udp_recv_data(struct net_socket_service_event *pev)
{
	...
	static uint8_t *buf, **buf_ptr;
	ssize_t out_size, *out_size_ptr;
	struct sockaddr_t *addr_ptr;
	struct socklen_t *addrlen_ptr;
#else
	static uint8_t buf[UDP_RECEIVER_BUF_SIZE];
#endif
	int ret = 0;
	int family=0, sock_error;
	...
	buf_ptr = skadi_cap_ops_derive_arg_wo(&buf, sizeof(void*));
	out_size_ptr = skadi_cap_ops_derive_arg_wo(&out_size, sizeof(ssize_t *));

	/* also read by the implementation... */
	addr_ptr = skadi_cap_ops_derive_arg(&addr, sizeof(addr));
	addrlen_ptr = skadi_cap_ops_derive_arg(&addrlen, sizeof(addrlen));
	...
	ret = skadi_ioctl(pev->event.fd, ZFD_IOCTL_ZEROCOPY_GETBUF, (void**) buf_ptr, out_size_ptr, addr_ptr, addrlen_ptr, NULL);
	if(!ret){
		ret = out_size;
	}
#elif defined(CONFIG_SKADI_OS) && !defined(CONFIG_SKADI_NET_ZEROCOPY)
	ret = skadi_zsock_recvfrom(pev->event.fd, buf, sizeof(buf), 0,
			     &addr, &addrlen);
#else
...
#if defined(CONFIG_SKADI_OS) && defined(CONFIG_SKADI_NET_ZEROCOPY)
	ret = skadi_ioctl(pev->event.fd, ZFD_IOCTL_ZEROCOPY_FREEBUF, buf);

	if(ret){
		LOG_ERR("Could not freebuf via ioctl: %s (%d)", strerror(ret), -ret);
	}
	...
	if(addr_ptr){
		skadi_cap_ops_drop(addr_ptr);
	}if(addrlen_ptr){
		skadi_cap_ops_drop(addrlen_ptr);
	}
#endif
...
error:
	if (udp_session_cb != NULL) {
#ifdef CONFIG_SKADI_OS
		call_udp_callback(ZPERF_SESSION_ERROR, NULL, udp_user_data, udp_session_cb);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, udp_svc_handler, struct k_work *work)
#else
static void udp_svc_handler(struct k_work *work)
#endif
{
	int ret;
#ifdef CONFIG_SKADI_OS
	struct net_socket_service_event *pev = work->user_data;
	__ASSERT_NO_MSG(pev);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(udp_svc_handler);
#endif
...
#ifdef CONFIG_SKADI_OS
		fds[SOCK_ID_IPV4].fd = skadi_zsock_socket(AF_INET, SOCK_DGRAM,
						    IPPROTO_UDP);
#else
...
#ifdef CONFIG_SKADI_OS
		ret = skadi_zsock_bind(fds[SOCK_ID_IPV4].fd,
...
#ifdef CONFIG_SKADI_OS
		fds[SOCK_ID_IPV6].fd = skadi_zsock_socket(AF_INET6, SOCK_DGRAM,
						    IPPROTO_UDP);
#else
...
#ifdef CONFIG_SKADI_OS
		ret = skadi_zsock_bind(fds[SOCK_ID_IPV6].fd,
...
#ifdef CONFIG_SKADI_OS
	for(int i = 0; i < ARRAY_SIZE(fds); i++){
		svc_udp.pev[i].work.user_data = &svc_udp.pev[i];
		svc_udp.pev[i].callback = SKADI_SUBSYSTEM_FUNCTION_POINTER(udp_svc_handler);
	}
	ret = skadi_net_socket_service_register(&svc_udp, fds,
					  ARRAY_SIZE(fds), NULL);
#else
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zperf_udp_download, const struct zperf_download_params *param, zperf_callback callback, void *user_data)
		skadi_subsystem_check_function_pointer(callback, false, false);
		return zperf_udp_download(param, callback, user_data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zperf_udp_download)
#endif
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zperf_udp_download_stop)
		return zperf_udp_download_stop();
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zperf_udp_download_stop)
#endif
