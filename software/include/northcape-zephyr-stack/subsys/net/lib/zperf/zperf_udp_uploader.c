				   ...
				   bool is_mcast_pkt)
{
	...
	while (ret <= 0 && loop-- > 0) {
		...
		/* Send the packet */
#if defined(CONFIG_SKADI_OS) && defined(CONFIG_SKADI_NET_ZEROCOPY)
		ret = skadi_zsock_send_0copy(sock, sample_packet_token, packet_size, 0);
#elif defined(CONFIG_SKADI_OS)
		ret = skadi_zsock_send(sock, sample_packet, packet_size, 0);
#else
		...
		if (!is_mcast_pkt) {
			...
			ret = skadi_zsock_setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &rcvtimeo,
					       sizeof(rcvtimeo));
#else
...
#ifdef CONFIG_SKADI_OS
			ret = skadi_zsock_recv(sock, stats, sizeof(stats), 0);
#else
	...
	while (true) {
#ifdef CONFIG_SKADI_OS
		ret = skadi_zsock_recv(sock, stats, sizeof(stats), ZSOCK_MSG_DONTWAIT);
#else
...
#ifdef CONFIG_SKADI_OS
		if (skadi_zsock_setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE, &req,
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zperf_udp_upload, const struct zperf_upload_params *param, struct zperf_results *result)
		return zperf_udp_upload(param, result);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zperf_udp_upload)
#endif

#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_ARGS(udp_upload_cb, enum zperf_status status, struct zperf_results *result, void *user_data);
#endif

#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, udp_upload_async_work, struct k_work *work)
#else
...
#endif
{
...
#ifdef CONFIG_SKADI_OS
	udp_upload_cb(ZPERF_SESSION_STARTED, NULL, upload_ctx->user_data, upload_ctx->callback);
#else
	...
	if (ret < 0) {
#ifdef CONFIG_SKADI_OS
		udp_upload_cb(ZPERF_SESSION_ERROR, NULL, upload_ctx->user_data, upload_ctx->callback);
#else
...
#ifdef CONFIG_SKADI_OS
		{
			...
			udp_upload_cb(ZPERF_SESSION_FINISHED, result_token, upload_ctx->user_data, upload_ctx->callback);
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(udp_upload_async_work)
#endif
...
#ifdef CONFIG_SKADI_OS
	if (skadi_work_is_pending(&udp_async_upload_ctx.work)) {
		return -EBUSY;
	}
#else
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zperf_udp_upload_async, const struct zperf_upload_params *param, zperf_callback callback, void *user_data)
		skadi_subsystem_check_function_pointer(callback, false, false);
		return zperf_udp_upload_async(param, callback, user_data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zperf_udp_upload_async)
#endif

void zperf_udp_uploader_init(void)
{
#ifdef CONFIG_SKADI_OS
	skadi_work_init(&udp_async_upload_ctx.work, SKADI_SUBSYSTEM_FUNCTION_POINTER(udp_upload_async_work));
#ifdef CONFIG_SKADI_NET_ZEROCOPY
