...
static struct k_sem contexts_lock;
...
#if defined(CONFIG_SKADI_NET_ZEROCOPY)
	static uint8_t *skadi_net_0copy_alloc(struct net_buf *buf, size_t *size, k_timeout_t timeout){
		__ASSERT(NULL, "Alloc should NEVER be called for a zero-copy buf!");
		return NULL;
	}
	static uint8_t *skadi_net_0copy_ref(struct net_buf *buf, uint8_t *data){
		void *ret = NULL;
		bool derive_ok;
		skadi_restriction_t restriction = SKADI_NO_RESTRICTION;
		/* offset may be zero, but these should be set together... */
		__ASSERT_NO_MSG(buf->derive_len);
		/* derive from the original buffer such that we can use its refcount */
		derive_ok = skadi_cap_ops_derive(buf->heap_allocation, restriction, buf->derive_len, buf->derive_offset, SKADI_PERMISSION_READ | SKADI_PERMISSION_IRQ_ACCESSIBLE  | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS, (void**) &ret);
		__ASSERT_NO_MSG(ret && derive_ok);
		return ret;
	}

	SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_ARGS(skadi_net_0copy_destroy_callback, void *data);

	static void skadi_net_0copy_unref(struct net_buf *buf, uint8_t *data){
		bool dropped, inspect_ok;
		skadi_inspect_metadata_t metadata = {};
		
		dropped = skadi_cap_ops_drop(data);
		__ASSERT_NO_MSG(data);
		
		/* might be called in ISR */
		inspect_ok = skadi_cap_ops_inspect(buf->heap_allocation, &metadata);
		__ASSERT_NO_MSG(inspect_ok);

		__ASSERT_NO_MSG(buf->data_alloc);

		if(metadata.refcount == 0){
			struct net_context *context = buf->data_alloc->alloc_data;
			__ASSERT_NO_MSG(context);

			if(context->options.tx_buffer_completion_callback){
				/* last copy - can call callback */
				skadi_net_0copy_destroy_callback(data, context->options.tx_buffer_completion_callback);
			}
			/* restore to original */
			buf->derive_len = 0;
		}
	...
	static const struct net_buf_data_cb net_context_0copy_cb = {
		.alloc = skadi_net_0copy_alloc,
		.ref = skadi_net_0copy_ref,
		.unref = skadi_net_0copy_unref
	};
#endif
				    ...
				    const struct sockaddr *addr)
{
	...
	do {
#ifdef CONFIG_SKADI_OS
		local_port = skadi_sys_rand16_get() | 0x8000;
#else
...
void net_context_zerocopy_free_recv_packet(struct net_context *context, void *capability){
	sys_slist_t *list_head = &context->zerocopy_recv_queue;
	struct net_pkt *pkt, *tmp, *last = NULL;

	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(list_head, pkt, tmp, next){
		if(pkt->indirect_cap == capability || !capability){
			sys_slist_remove(list_head, last ? &last->next : NULL, &pkt->next);
			(void)skadi_cap_ops_drop(pkt->indirect_cap);
			net_pkt_unref(pkt);
			if(capability){
				return;
			}
	...
	if(capability){
		LOG_WRN("Could not free packet for 0-copy capability %p", capability);
	}
}
#else
		    ...
		    struct net_context **context)
{
...
#ifdef CONFIG_SKADI_OS
	skadi_sem_take(contexts_lock, K_FOREVER);
#else
	...
	for (i = 0; i < NET_MAX_CONTEXT; i++) {
		...
		if(contexts[i].recv_data_wait){
			skadi_sem_cleanup(contexts[i].recv_data_wait);
			skadi_allocator_free(contexts[i].recv_data_wait);
			contexts[i].recv_data_wait = NULL;
		}
		if(contexts[i].lock){
			skadi_mutex_cleanup(contexts[i].lock);
			skadi_allocator_free(contexts[i].lock);
			contexts[i].lock = NULL;
		}
#ifdef CONFIG_NET_SOCKETS
		if(contexts[i].recv_q){
			skadi_fifo_cleanup(contexts[i].recv_q);
			skadi_allocator_free(contexts[i].recv_q);
			contexts[i].recv_q = NULL;
		}

		if(contexts[i].cond.recv){
			skadi_condvar_cleanup(contexts[i].cond.recv);
			skadi_allocator_free(contexts[i].cond.recv);
			contexts[i].cond.recv = NULL;
		}
#endif /* CONFIG_NET_SOCKETS*/
#ifdef CONFIG_SKADI_NET_ZEROCOPY
	net_context_zerocopy_free_recv_packet(&contexts[i], NULL);
#endif /* CONFIG_SKADI_NET_ZEROCOPY*/
...
#ifdef CONFIG_SKADI_NET_ZEROCOPY
		sys_slist_init(&contexts[i].zerocopy_recv_queue);
#endif /* CONFIG_SKADI_NET_ZEROCOPY */
...
#ifdef CONFIG_SKADI_OS
			contexts[i].recv_data_wait = skadi_allocator_alloc_rw(sizeof(*contexts[i].recv_data_wait));

			__ASSERT_NO_MSG(contexts[i].recv_data_wait);
			
			if(!contexts[i].recv_data_wait){
				LOG_ERR("Could not allocate semaphore!");
				return -ENOMEM;
			}
			skadi_sem_init(contexts[i].recv_data_wait, 1, K_SEM_MAX_LIMIT);
#else
...
#ifdef CONFIG_SKADI_OS
		contexts[i].lock = skadi_allocator_alloc_rw(sizeof(*contexts[i].lock));
		__ASSERT_NO_MSG(contexts[i].lock);
		if(!contexts[i].lock){
			LOG_ERR("Could not allocate context!");
			return -ENOMEM;
		}
#ifdef CONFIG_NET_SOCKETS
		contexts[i].recv_q = skadi_allocator_alloc_rw(sizeof(*contexts[i].recv_q));
		contexts[i].cond.recv = skadi_allocator_alloc_rw(sizeof(*contexts[i].cond.recv));

		if(!contexts[i].recv_q || !contexts[i].cond.recv ){
			LOG_ERR("Could not allocate receive queue!");
			return -ENOMEM;
		}
#endif /* CONFIG_NET_SOCKETS*/

		skadi_mutex_init(contexts[i].lock);
#else
...
#ifdef CONFIG_SKADI_OS
    skadi_mutex_lock(context->lock,  K_FOREVER);
#else
...
#ifdef CONFIG_SKADI_OS
    skadi_mutex_unlock(context->lock);
#else
...
static int get_context_last_txtime(struct net_context *context,
	void *value, size_t *len)
{
#if defined(CONFIG_NET_CONTEXT_TIMESTAMP_SOCKOPT)
	struct net_ptp_time *ptp_time_out = value;
	if(!len || *len < sizeof(struct net_ptp_time)){
		return -EINVAL;
	}
	if(!context->options.last_txtimestamp_valid){
		LOG_ERR("No last txtimestamp!");
		memset(ptp_time_out, 0, sizeof(*ptp_time_out));
	}
	else{
		memcpy(ptp_time_out, &context->options.last_txtimestamp, sizeof(*ptp_time_out));
		// only returned once
		context->options.last_txtimestamp_valid = false;
	}

	*len = sizeof(*ptp_time_out);
	return 0;
#else
ARG_UNUSED(context);
ARG_UNUSED(value);
...
static int get_context_zerocopy(struct net_context *context,
	void *value, size_t *len)
{
#if defined(CONFIG_SKADI_NET_ZEROCOPY)
	*((bool *)value) = context->options.is_0copy_socket;
	if(len){
		*len = sizeof(bool);
	}
...
static int get_context_zerocopy_rx_cb(struct net_context *context,
	void *value, size_t *len)
{
#if defined(CONFIG_SKADI_NET_ZEROCOPY)
	*((void **)value) = context->options.rx_buffer_completion_callback;
	if(len){
		*len = sizeof(void *);
	}
	return 0;
...
static int get_context_zerocopy_tx_cb(struct net_context *context,
	void *value, size_t *len)
{
#if defined(CONFIG_SKADI_NET_ZEROCOPY)
	*((void **)value) = context->options.tx_buffer_completion_callback;
	if(len){
		*len = sizeof(void *);
	}
	return 0;
				    ...
				    socklen_t addrlen)
{
	...
	if(context->options.is_0copy_socket){
		...
		net_buf = net_buf_frag_last(pkt->buffer);
		__ASSERT_NO_MSG(net_buf);
		__ASSERT_NO_MSG(!net_buf->__buf);

		net_buf->derive_len = len;
		net_buf->derive_offset = skadi_get_capability_offset(buf);
		/* size is set such that the network stack does not try to add data here */
		net_buf->len = net_buf->size = len;

		__ASSERT_NO_MSG(buf);

		derive_ok = skadi_cap_ops_derive(buf, restriction, net_buf->derive_len, net_buf->derive_offset, SKADI_PERMISSION_READ | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS, (void**) &net_buf->__buf);
		__ASSERT_NO_MSG(derive_ok);
		__ASSERT_NO_MSG(net_buf->__buf);

		if(!derive_ok){
			/* just use the given data... */
			net_buf->__buf = (void *) buf;
		}
		/* the callbacks will take care of ref'ing / unref'ing the data accordingly */
		net_buf->data_alloc = &context->zerocopy_pool_alloc;

		net_buf->heap_allocation = (void *) buf;

		__ASSERT_NO_MSG(net_buf->__buf);
		/* important so network stack knows where data start */
		net_buf->data = net_buf->__buf;
			  ...
			  bool sendto)
{
	...
	/* for zerocopy, allocating with 0-length buffer */
	if (tmp_len < len && !IS_ENABLED(CONFIG_SKADI_NET_ZEROCOPY)) {
		if (net_context_get_type(context) == SOCK_DGRAM) {
...
#ifdef CONFIG_SKADI_OS
			skadi_sem_give(context->recv_data_wait);
#else
		     ...
		     void *user_data)
{
	...
	if (!K_TIMEOUT_EQ(timeout, K_NO_WAIT)) {
		 ...
		 */
#ifdef CONFIG_SKADI_OS
		skadi_sem_reset(context->recv_data_wait);
#else
...
#ifdef CONFIG_SKADI_OS
		if (skadi_sem_take(context->recv_data_wait, timeout) == -EAGAIN) {
#else
...
#ifdef CONFIG_NET_CONTEXT_TIMESTAMP_SOCKOPT
	static void net_context_timestamp_cb(struct net_pkt *pkt){
		if(!pkt->context){
			/* not sent from socket */
			return;
		}
		if((pkt->context->options.timestamping & SOF_TIMESTAMPING_TX_HARDWARE) == 0){
			/* timestamping not active */
			return;
		}
		if(!pkt->context->options.last_txtimestamp_valid){
			memcpy(&pkt->context->options.last_txtimestamp, &pkt->timestamp, sizeof(pkt->context->options.last_txtimestamp));
			pkt->context->options.last_txtimestamp_valid = true;
		}
	} 

	static bool net_context_timestamp_cb_registered;
...
#ifdef CONFIG_NET_CONTEXT_TIMESTAMP_SOCKOPT
	if((timestamping_flags & SOF_TIMESTAMPING_TX_HARDWARE) && !net_context_timestamp_cb_registered){
		net_if_register_timestamp_cb(&timestamp_cb, NULL, NULL, net_context_timestamp_cb);
	}
#endif
...
static int set_context_zerocopy(struct net_context *context,
	const void *value, size_t len)
{
#if defined(CONFIG_SKADI_NET_ZEROCOPY)
	int zero_copy_option = *((bool *)value);
	int ret;

	ret = set_bool_option(&context->options.is_0copy_socket,
				&zero_copy_option, sizeof(zero_copy_option));

	if(!ret && context->options.is_0copy_socket){
		/* need to setup the buffer pool */
		context->zerocopy_pool_alloc.cb = &net_context_0copy_cb;
		context->zerocopy_pool_alloc.alloc_data = context;;
		context->zerocopy_pool_alloc.max_alloc_size = 0;
	}
	return ret;
#else
	ARG_UNUSED(context);
...
static int set_context_zerocopy_rx_cb(struct net_context *context,
	const void *value, size_t len)
{
#if defined(CONFIG_SKADI_NET_ZEROCOPY)
	void **parsed_value = (void*)value;
	const void *fn_ptr = *parsed_value;
	if(len < sizeof(void*)){
		return -EINVAL;
	}
	skadi_subsystem_check_function_pointer(fn_ptr, false, false);
	context->options.rx_buffer_completion_callback = fn_ptr;
	return 0;
#else
	ARG_UNUSED(context);
	ARG_UNUSED(value);
...
static int set_context_zerocopy_tx_cb(struct net_context *context,
	const void *value, size_t len)
{
#if defined(CONFIG_SKADI_NET_ZEROCOPY)
	void **parsed_value = (void*) value;
	...
	skadi_subsystem_check_function_pointer(fn_ptr, false, false);
	context->options.tx_buffer_completion_callback = fn_ptr;
	return 0;
#else
	ARG_UNUSED(context);
	ARG_UNUSED(value);
			   ...
			   const void *value, size_t len)
{
	...
	case NET_OPT_TIMESTAMPING:
		...
		break;
	case NET_OPT_ZEROCOPY:
		ret = set_context_zerocopy(context, value, len);
		break;
	case NET_OPT_ZEROCOPY_RX_CB:
		ret = set_context_zerocopy_rx_cb(context, value, len);
		break;
	case NET_OPT_ZEROCOPY_TX_CB:
		ret = set_context_zerocopy_tx_cb(context, value, len);
		break;
	case NET_OPT_LAST_TXTIME:
		/* cannot bet set */
		ret = -ENOPROTOOPT;
		break;
	}
			    ...
			    void *value, size_t *len)
{
	...
	case NET_OPT_TIMESTAMPING:
		...
		break;
		case NET_OPT_ZEROCOPY:
		ret = get_context_zerocopy(context, value, len);
		break;
	case NET_OPT_ZEROCOPY_RX_CB:
		ret = get_context_zerocopy_rx_cb(context, value, len);
		break;
	case NET_OPT_ZEROCOPY_TX_CB:
		ret = get_context_zerocopy_tx_cb(context, value, len);
		break;
	case NET_OPT_LAST_TXTIME:
		ret = get_context_last_txtime(context, value, len);
		break;
	}
...
void net_context_foreach(net_context_cb_t cb, void *user_data)
{
	...
	for (i = 0; i < NET_MAX_CONTEXT; i++) {
		...
		#ifdef CONFIG_SKADI_OS
    skadi_mutex_lock(contexts[i].lock,  K_FOREVER);
#else
    k_mutex_lock(&contexts[i].lock,  K_FOREVER);
		...
		#ifdef CONFIG_SKADI_OS
    skadi_mutex_unlock(contexts[i].lock);
#else
...
{
#ifdef CONFIG_SKADI_OS
	contexts_lock = skadi_allocator_alloc_rw(sizeof(*contexts_lock));
	if(!contexts_lock){
		LOG_ERR("Could not initialize contexts lock!");
		return;
	}
	skadi_sem_init(contexts_lock, 1, K_SEM_MAX_LIMIT);
#else
