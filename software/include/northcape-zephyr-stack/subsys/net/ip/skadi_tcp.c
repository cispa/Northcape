...
struct k_mutex *tcp_lock;

K_MEM_SLAB_DEFINE_STATIC(tcp_conns_slab, sizeof(struct tcp),
				CONFIG_NET_MAX_CONTEXTS, 4);

struct k_mem_slab *tcp_conns_slab_ptr;

static struct k_work_q tcp_work_q, *tcp_work_q_ptr;
#ifdef CONFIG_SKADI_OS
static k_thread_stack_t *work_q_stack;
#else
...
static void tcp_derive_rto(struct tcp *conn)
{
	...
	skadi_sys_rand_get(&gain8, sizeof(uint8_t));
...
#ifdef CONFIG_SKADI_OS
	void (*SKADI_SUBSYSTEM_FUNCTION_POINTER(tcp_send_keepalive_probe))(struct k_work *work);
#else
...
static void keep_alive_timer_init(struct tcp *conn)
{
	...
	conn->keepalive_timer = skadi_allocator_alloc_rw(sizeof(*conn->keepalive_timer));

	if(!conn->keepalive_timer){
		NET_ERR("Could not allocate keepalive timer!");
		return;
	}
	skadi_work_init_delayable(conn->keepalive_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(tcp_send_keepalive_probe));
}
...
static void keep_alive_timer_restart(struct tcp *conn)
{
	...
	conn->keep_cur = 0;
	__ASSERT_NO_MSG(tcp_work_q_ptr);
	skadi_work_reschedule_for_queue(tcp_work_q_ptr, &conn->keepalive_timer,
				    K_SECONDS(conn->keep_idle));
}

static void keep_alive_timer_stop(struct tcp *conn)
{
	__ASSERT_NO_MSG(conn->keepalive_timer);
	skadi_work_cancel_delayable(conn->keepalive_timer);
}
...
static void tcp_send_queue_flush(struct tcp *conn)
{
	...
	__ASSERT_NO_MSG(conn->send_timer);
	skadi_work_cancel_delayable(conn->send_timer);
...
#define CLEAR_WORK_ITEM(ITEM)					\
	if(ITEM){									\
		skadi_work_cleanup_delayable(ITEM);		\
		skadi_allocator_free(ITEM);				\
	}

#define CLEAR_WORK_ITEM_NON_DELAYABLE(ITEM)		\
	if(ITEM){									\
		skadi_work_cleanup(ITEM);				\
		skadi_allocator_free(ITEM);				\
	}

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_conn_release, struct k_work *work)
{
	struct tcp *conn = work->user_data;
	struct net_pkt *pkt;
	 ...
	 */
	while ((pkt = skadi_fifo_get(conn->recv_data, K_NO_WAIT)) != NULL) {
		tcp_pkt_unref(pkt);
	}

	skadi_mutex_lock(conn->lock, K_FOREVER);
	...
	(void)skadi_work_cancel_delayable(conn->send_data_timer);
	...
	(void)skadi_work_cancel_delayable(conn->timewait_timer);
	(void)skadi_work_cancel_delayable(conn->fin_timer);
	(void)skadi_work_cancel_delayable(conn->persist_timer);
	(void)skadi_work_cancel_delayable(conn->ack_timer);
	(void)skadi_work_cancel_delayable(conn->send_timer);
	(void)skadi_work_cancel_delayable(conn->recv_queue_timer);
	keep_alive_timer_stop(conn);

	skadi_mutex_unlock(conn->lock);

	net_context_unref(conn->context);
	conn->context = NULL;

	skadi_mutex_lock(tcp_lock, K_FOREVER);
	sys_slist_find_and_remove(&tcp_conns, &conn->next);
	skadi_mutex_unlock(tcp_lock);

	if(conn->lock){
		skadi_mutex_cleanup(conn->lock);
		skadi_allocator_free(conn->lock);
	}

	if(conn->recv_data){
		skadi_fifo_cleanup(conn->recv_data);
		skadi_allocator_free(conn->recv_data);
	}

	if(conn->connect_sem){
		skadi_sem_cleanup(conn->connect_sem);
		skadi_allocator_free(conn->connect_sem);
	}

	if(conn->tx_sem){
		skadi_sem_cleanup(conn->tx_sem);
		skadi_allocator_free(conn->tx_sem);
	}

	CLEAR_WORK_ITEM(conn->send_timer);
	CLEAR_WORK_ITEM(conn->timewait_timer);
	CLEAR_WORK_ITEM(conn->fin_timer);
	CLEAR_WORK_ITEM(conn->send_data_timer);
	CLEAR_WORK_ITEM(conn->recv_queue_timer);
	CLEAR_WORK_ITEM(conn->persist_timer);
	CLEAR_WORK_ITEM(conn->ack_timer);

	skadi_mem_slab_free(tcp_conns_slab_ptr, (void *)conn);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_conn_release)
...
static int tcp_conn_unref(struct tcp *conn)
{
	...
	if (conn->in_connect) {
		...
		skadi_sem_reset(conn->connect_sem);
	}
	...
	__ASSERT_NO_MSG(tcp_work_q_ptr);
	__ASSERT_NO_MSG(conn->conn_release);
	 ...
	 * is released.
	 */
	skadi_work_submit_to_queue(tcp_work_q_ptr, conn->conn_release);
...
static bool tcp_send_process_no_lock(struct tcp *conn)
{
	...
	if (conn->in_retransmission) {
		...
		if (forget == false &&
		    !skadi_work_delayable_remaining_get(conn->send_timer)) {
	...
	if (conn->in_retransmission) {
		skadi_work_reschedule_for_queue(tcp_work_q_ptr, conn->send_timer,
					    K_MSEC(TCP_RTO_MS));
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_send_process, struct k_work *work)
{
	struct tcp *conn = work->user_data;
	bool unref;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_send_process)
...
static int tcp_update_recv_wnd(struct tcp *conn, int32_t delta)
{
	...
	if (((short_win_before && !short_win_after) ||
		...
		__ASSERT_NO_MSG(conn->ack_timer);
		skadi_work_cancel_delayable(conn->ack_timer);
		tcp_out(conn, ACK);
	}
				     ...
				     size_t len)
{
	...
	if (CONFIG_NET_TCP_RECV_QUEUE_TIMEOUT &&
		...
		__ASSERT_NO_MSG(conn->recv_queue_timer);
		...
		if (end_offset < pending_len) {
			...
			skadi_work_cancel_delayable(conn->recv_queue_timer);
		} else {
...
static enum net_verdict tcp_data_get(struct tcp *conn, struct net_pkt *pkt, size_t *len)
{
	...
	if (conn->context->recv_cb) {
		...
		*len += tcp_check_pending_data(conn, pkt, *len);
		 ...
		 */
		skadi_fifo_put(conn->recv_data, pkt);
		       ...
		       uint32_t seq)
{
	...
	if (is_destination_local(pkt)) {
		 ...
		 */
		__ASSERT_NO_MSG(conn->send_timer);
		skadi_work_schedule_for_queue(tcp_work_q_ptr,
...
static int tcp_pkt_append(struct net_context *context, struct net_pkt *pkt, const uint8_t *data, size_t len)
{
	...
	if(context->options.is_0copy_socket){
		ret = net_pkt_alloc_buffer_raw(pkt, 0,
			TCP_PKT_ALLOC_TIMEOUT);
		if (ret < 0) {
			return -ENOBUFS;
		}
		__ASSERT_NO_MSG(pkt->buffer);
		buf = net_buf_frag_last(pkt->buffer);
		__ASSERT_NO_MSG(buf);
		buf->len = buf->size = len;
		
		buf->derive_len = len;
		buf->derive_offset = skadi_get_capability_offset(data);

		derive_ok = skadi_cap_ops_derive(data, restriction, buf->derive_len, buf->derive_offset, SKADI_PERMISSION_READ | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS, (void**) &buf->__buf);
		...
		/* important so network stack knows where data start */
		buf->data = buf->__buf;

		/* the callbacks will take care of ref'ing / unref'ing the data accordingly */
		buf->data_alloc = &context->zerocopy_pool_alloc;

		buf->heap_allocation = (void *) data;

		return 0;
	}
	else {
		/* classical copying approach */
		if (pkt->buffer) {
			buf = net_buf_frag_last(pkt->buffer);
...
static int tcp_send_queued_data(struct tcp *conn)
{
	...
	__ASSERT_NO_MSG(conn->send_data_timer);

	if (skadi_work_delayable_remaining_get(conn->send_data_timer)) {
		subscribe = false;
	}
	...
	if (subscribe) {
		...
		skadi_work_reschedule_for_queue(tcp_work_q_ptr, conn->send_data_timer,
					    K_MSEC(TCP_RTO_MS));
	}
 out:
	return ret;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_cleanup_recv_queue, struct k_work *work)
{
	struct tcp *conn = work->user_data;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_cleanup_recv_queue)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_resend_data, struct k_work *work)
{
	struct tcp *conn = work->user_data; 
	...
	if (IS_ENABLED(CONFIG_NET_TCP_CONGESTION_AVOIDANCE) &&
		...
		if (tcp_window_full(conn)) {
			(void)skadi_sem_take(conn->tx_sem, K_NO_WAIT);
		}
	}
	...
	if (ret == 0) {
		if (conn->in_close && conn->send_data_total == 0) {
			...
			__ASSERT_NO_MSG(tcp_work_q_ptr);
			__ASSERT_NO_MSG(conn->fin_timer);
			skadi_work_reschedule_for_queue(tcp_work_q_ptr,
						    conn->fin_timer,
						    FIN_TIMEOUT);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_resend_data)


SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_timewait_timeout, struct k_work *work)
{
	struct tcp *conn = work->user_data;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_timewait_timeout)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_fin_timeout, struct k_work *work)
{
	struct tcp *conn = work->user_data;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_fin_timeout)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_last_ack_timeout, struct k_work *work)
{
	struct tcp *conn = work->user_data;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_last_ack_timeout)

static void tcp_setup_last_ack_timer(struct tcp *conn)
{
	conn->fin_timer = skadi_allocator_alloc_rw(sizeof(*conn->fin_timer));

	if(!conn->fin_timer){
		LOG_ERR("Could not alloc fin timer!");
		return;
	}
	 ...
	 */
	skadi_work_init_delayable(conn->fin_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(tcp_last_ack_timeout));

	conn->fin_timer->work.user_data = conn;
...
{
	__ASSERT_NO_MSG(conn->fin_timer);
	skadi_work_cancel_delayable(conn->fin_timer);
}

#if defined(CONFIG_NET_TCP_KEEPALIVE)
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_send_keepalive_probe, struct k_work *work)
{
	struct tcp *conn = work->user_data;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_send_zwp, struct k_work *work)
{
	struct tcp *conn = work->user_data;
	...
	if (conn->send_win == 0) {
		...
		__ASSERT_NO_MSG(tcp_work_q_ptr);
		__ASSERT_NO_MSG(conn->persist_timer);

		(void)skadi_work_reschedule_for_queue(
			tcp_work_q_ptr, conn->persist_timer, K_MSEC(timeout));
	}

	skadi_mutex_unlock(conn->lock);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_send_zwp)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, tcp_send_ack, struct k_work *work)
{
	struct tcp *conn = work->user_data;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(tcp_send_ack)
...
#define INIT_WORK_ITEM(ITEM, HANDLER) 																								\
	 ITEM = skadi_allocator_alloc_rw(sizeof(*ITEM));		\
	 if(!ITEM){																														\
		LOG_ERR("Could not allocate work item!");																					\
		return NULL;																												\
	 }																																\
	 skadi_work_init_delayable(ITEM, SKADI_SUBSYSTEM_FUNCTION_POINTER(HANDLER));											\
	 ITEM->work.user_data = conn;

#define INIT_WORK_ITEM_NON_DELAYABLE(ITEM, HANDLER) 																				\
	 ITEM = skadi_allocator_alloc_rw(sizeof(*ITEM));		\
	 if(!ITEM){																														\
		LOG_ERR("Could not allocate work item!");																					\
		return NULL;																												\
	 }																																\
	 skadi_work_init(ITEM, SKADI_SUBSYSTEM_FUNCTION_POINTER(HANDLER));														\
	 ITEM->user_data = conn;
...
static struct tcp *tcp_conn_alloc(void)
{
	...
	conn = skadi_mem_slab_alloc_alt(tcp_conns_slab_ptr, K_NO_WAIT);
	if (!conn) {
		NET_ERR("Cannot allocate slab");
		goto out;
	}

	/* cannot free the release work with the other work, as this is invoked while it is running */
	/* so we free it here */
	if(conn->conn_release){
		CLEAR_WORK_ITEM_NON_DELAYABLE(conn->conn_release);
	}
	...
	conn->lock = skadi_allocator_alloc_rw(sizeof(*conn->recv_data));
	
	if(!conn->lock){
		NET_ERR("Cannot allocate conn->lock!");
		goto fail;
	}

	skadi_mutex_init(conn->lock);

	conn->recv_data = skadi_allocator_alloc_rw(sizeof(*conn->recv_data));
	

	if(!conn->recv_data){
		NET_ERR("Cannot allocate conn->recv_data!");
		goto fail;
	}

	skadi_fifo_init(conn->recv_data);
	
	conn->connect_sem = skadi_allocator_alloc_rw(sizeof(*conn->connect_sem));
	
	conn->tx_sem = skadi_allocator_alloc_rw(sizeof(*conn->tx_sem));
	
	if(!conn->connect_sem || !conn->tx_sem){
		LOG_ERR("Cannot allocate conn semaphores!");
		goto fail;
	}

	skadi_sem_init(conn->connect_sem, 0, K_SEM_MAX_LIMIT);
	skadi_sem_init(conn->tx_sem, 1, 1);
	...
	INIT_WORK_ITEM(conn->send_timer, tcp_send_process);
	INIT_WORK_ITEM(conn->timewait_timer, tcp_timewait_timeout);
	INIT_WORK_ITEM(conn->fin_timer, tcp_fin_timeout);
	INIT_WORK_ITEM(conn->send_data_timer, tcp_resend_data);
	INIT_WORK_ITEM(conn->recv_queue_timer, tcp_cleanup_recv_queue);
	INIT_WORK_ITEM(conn->persist_timer, tcp_send_zwp);
	INIT_WORK_ITEM(conn->ack_timer, tcp_send_ack);
	INIT_WORK_ITEM_NON_DELAYABLE(conn->conn_release, tcp_conn_release);
...
{
	return seq + (k_ticks_to_ns_floor32(skadi_uptime_ticks()) >> 6);
}
...
static enum tcp_state tcp_enter_time_wait(struct tcp *conn)
{
	...
	__ASSERT_NO_MSG(tcp_work_q_ptr);
	__ASSERT_NO_MSG(conn->timewait_timer);

	skadi_work_reschedule_for_queue(
		tcp_work_q_ptr, conn->timewait_timer,
				...
				size_t len, uint32_t seq)
{
	...
	if (inserted) {
		...
		if (!skadi_work_delayable_is_pending(conn->recv_queue_timer)) {
			skadi_work_reschedule_for_queue(
				tcp_work_q_ptr, conn->recv_queue_timer,
					  ...
					  size_t *len, bool psh)
{
	...
	if (tcp_short_window(conn) || !psh) {
		__ASSERT_NO_MSG(tcp_work_q_ptr);
		skadi_work_schedule_for_queue(tcp_work_q_ptr, conn->ack_timer,
					  ACK_DELAY);
	} else {
		skadi_work_cancel_delayable(conn->ack_timer);
		tcp_out(conn, ACK);
...
static enum net_verdict tcp_in(struct tcp *conn, struct net_pkt *pkt)
{
	...
	if (th) {
		...
		if (conn->send_win == 0) {
			if (!skadi_work_delayable_is_pending(conn->persist_timer)) {
				conn->zwp_retries = 0;
				__ASSERT_NO_MSG(tcp_work_q_ptr);
				(void)skadi_work_reschedule_for_queue(
					tcp_work_q_ptr, conn->persist_timer,
					K_MSEC(TCP_RTO_MS));
			}
		} else {
			(void)skadi_work_cancel_delayable(conn->persist_timer);
...
next_state:
	...
	case TCP_LISTEN:
		if (FL(&fl, ==, SYN)) {
			...
			__ASSERT_NO_MSG(tcp_work_q_ptr);
			__ASSERT_NO_MSG(conn->establish_timer);
	...
	case TCP_SYN_RECEIVED:
		if (FL(&fl, &, ACK, th_ack(th) == conn->seq &&
			...
			skadi_work_cancel_delayable(conn->establish_timer);
			tcp_send_timer_cancel(conn);
			tcp_conn_ref(conn);
	...
	case TCP_ESTABLISHED:
		...
		NET_ASSERT((conn->send_data_total == 0) ||
			   skadi_work_delayable_is_pending(conn->send_data_timer),
		...
		if (conn->send_data_total == 0) {
			conn->send_data_retries = 0;
			__ASSERT_NO_MSG(conn->send_data_timer);
			skadi_work_cancel_delayable(conn->send_data_timer);
		}
...
out:
	...
	if (next) {
		...
		if (connection_ok) {
			...
			skadi_sem_give(conn->connect_sem);
		}
	...
	while (conn_handler && atomic_get(&conn->ref_count) > 0 &&
	       (recv_pkt = skadi_fifo_get(recv_data_fifo, K_NO_WAIT)) != NULL) {
		  ...
		  const struct msghdr *msg)
{
	...
	if (msg) {
		...
		ret = tcp_pkt_append(context, conn->send_data, data, len);
		if (ret < 0) {
			goto out;
		}
		    ...
		    void *user_data)
{
	...
	if (!IS_ENABLED(CONFIG_NET_TEST_PROTOCOL)) {
		if (conn->state == TCP_UNUSED || conn->state == TCP_CLOSED) {
			...
			ret = -EINPROGRESS;
			goto out;
		} else if (skadi_sem_take(conn->connect_sem, timeout) != 0 &&
			   conn->state != TCP_ESTABLISHED) {
			if (conn->in_connect) {
				conn->in_connect = false;
				tcp_conn_close(conn, -ETIMEDOUT);
...
static struct k_heap tcp_work_queue_heap;
#define WORKQUEUE_HEAP_SIZE_BYTES 128
static char heap_mem[WORKQUEUE_HEAP_SIZE_BYTES];

void net_tcp_init(void)
{
	int i;
	int rto;
	void* derived_work_queue;
	size_t stack_size;
	void* buffer_ptr;
    skadi_restriction_t restriction = SKADI_NO_RESTRICTION;
...
#define THREAD_PRIORITY K_PRIO_PREEMPT(CONFIG_NET_TCP_WORKER_PRIO)
...
#ifdef CONFIG_SKADI_OS
	work_q_stack = skadi_allocator_alloc_rw(CONFIG_NET_TCP_WORKQ_STACK_SIZE);
	
	if(!work_q_stack){
		LOG_ERR("Expected to be able to allocate work queue stack!");
		return;
	}

	if(skadi_cap_ops_derive(&tcp_work_q, restriction, sizeof(tcp_work_q), skadi_get_capability_offset(&tcp_work_q), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &derived_work_queue) == false || derived_work_queue == 0){
		LOG_ERR("Could not derive work queue capability!");
		return;
	}

	stack_size = CONFIG_NET_TCP_WORKQ_STACK_SIZE;
	tcp_work_q_ptr = (struct k_work_q *) derived_work_queue;

	skadi_work_queue_init(tcp_work_q_ptr);

	if(skadi_cap_ops_derive(&tcp_conns_slab, restriction, sizeof(tcp_conns_slab), skadi_get_capability_offset(&tcp_conns_slab), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &buffer_ptr) == false || buffer_ptr == 0){
		LOG_ERR("Could not derive from TCP conns slab!");
		return;
	}

	tcp_conns_slab_ptr = (void *) buffer_ptr;

	if(skadi_cap_ops_derive(tcp_conns_slab.buffer, restriction, tcp_conns_slab.info.num_blocks * WB_UP(tcp_conns_slab.info.block_size), skadi_get_capability_offset(tcp_conns_slab.buffer), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &buffer_ptr) == false  || !buffer_ptr){
		LOG_ERR("Could not derive for k_mem_slab->buffer");
		return;
	}

	/* some internal pointers, such as free list, buffer, wait q are not set (correctly) yet */
	if(skadi_mem_slab_init(tcp_conns_slab_ptr, (void*)buffer_ptr, tcp_conns_slab_ptr->info.block_size, tcp_conns_slab_ptr->info.num_blocks) != 0){
		LOG_ERR("Could not init slab!");
		return;
	}

	tcp_lock = skadi_allocator_alloc_rw(sizeof(*tcp_lock));

	if(!tcp_lock){
		LOG_ERR("Could not alloc mutex!");
		return;
	}

	skadi_mutex_init(tcp_lock);
#else
	tcp_work_q_ptr = &tcp_work_q;
	stack_size = K_KERNEL_STACK_SIZEOF(work_q_stack);
	(void)derived_work_queue;
	(void) buffer_ptr;
#endif
	 ...
	 */
	skadi_work_queue_start(tcp_work_q_ptr, work_q_stack,
	...
	skadi_thread_name_set(&tcp_work_q_ptr->thread, "tcp_work");

	skadi_heap_init(&tcp_work_queue_heap, heap_mem, WORKQUEUE_HEAP_SIZE_BYTES);

	skadi_thread_heap_assign(&tcp_work_q_ptr->thread, &tcp_work_queue_heap);

	NET_DBG("Workq started. Thread ID: %p", &tcp_work_q_ptr->thread);
}
