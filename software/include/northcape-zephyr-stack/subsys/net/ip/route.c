...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, route_lifetime_timeout, struct k_work *work)
#else
...
#ifdef CONFIG_SKADI_OS
		__ASSERT_NO_MSG(route_lifetime_timer);
		skadi_work_reschedule(route_lifetime_timer, K_MSEC(next_update));
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(route_lifetime_timeout)
#endif
...
#ifdef CONFIG_SKADI_OS
		net_timeout_set(&route->lifetime, lifetime, skadi_uptime_get_32());
#else
...
#ifdef CONFIG_SKADI_OS
		__ASSERT_NO_MSG(route_lifetime_timer);
		skadi_work_reschedule(route_lifetime_timer, K_NO_WAIT);
#else
...
#ifdef CONFIG_SKADI_OS
		__ASSERT_NO_MSG(route_lifetime_timer);
		skadi_work_cancel_delayable(route_lifetime_timer);
#else
...
#ifdef CONFIG_SKADI_OS
		route_lifetime_timer = skadi_allocator_alloc_rw(sizeof(*route_lifetime_timer));
		skadi_work_init_delayable(route_lifetime_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(route_lifetime_timeout));
#else
