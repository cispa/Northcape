...
static struct net_conn *conn_get_unused(void)
{
	...
	#ifdef CONFIG_SKADI_OS
    skadi_mutex_lock(conn_lock, K_FOREVER);
#else
...
#ifdef CONFIG_SKADI_OS
	conn_lock = skadi_allocator_alloc_rw(sizeof(*conn_lock));

	if(!conn_lock){
		LOG_ERR("Could not allocate conn_lock");
		return;
	}

	skadi_mutex_init(conn_lock);
#endif
