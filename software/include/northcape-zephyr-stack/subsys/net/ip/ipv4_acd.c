...
#ifdef CONFIG_SKADI_OS
		__ASSERT_NO_MSG(ipv4_acd_timer);
		skadi_work_cancel_delayable(ipv4_acd_timer);
#else
...
#ifdef CONFIG_SKADI_OS
		__ASSERT_NO_MSG(ipv4_acd_timer);
		skadi_work_reschedule(ipv4_acd_timer, timeout);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, ipv4_acd_timeout, struct k_work *work)
#else
...
{
#ifdef CONFIG_SKADI_OS
	ipv4_acd_timer = skadi_allocator_alloc_rw(sizeof(*ipv4_acd_timer));

	if(!ipv4_acd_timer){
		LOG_ERR("Could not allocate ACD timer!");
		return;
	}

	skadi_work_init_delayable(ipv4_acd_timer, ipv4_acd_timeout);
