		      ...
		      net_icmp_handler_t handler)
{
	...
	if(lock == NULL){
		...
		if(!lock){
			LOG_ERR("Could not allocate lock!");
			return -ENOMEM;
		}
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_net_icmp_init_ctx, struct net_icmp_ctx *ctx, uint8_t type, uint8_t code, net_icmp_handler_t handler)
	return net_icmp_init_ctx(ctx, type, code, handler);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_icmp_init_ctx)
#endif
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_net_icmp_cleanup_ctx, struct net_icmp_ctx *ctx)
	return net_icmp_cleanup_ctx(ctx);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_icmp_cleanup_ctx)
#endif
...
#ifdef CONFIG_SKADI_OS
			uint32_t time_stamp = htonl(skadi_cycle_get_32());
#else
...
#ifdef CONFIG_SKADI_OS
	params.identifier = skadi_sys_rand16_get();
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_net_icmp_send_echo_request, struct net_icmp_ctx *ctx, struct net_if *iface, struct sockaddr *dst, struct net_icmp_ping_params *params, void *user_data);
	return net_icmp_send_echo_request(ctx, iface, dst, params, user_data);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_icmp_send_echo_request)
