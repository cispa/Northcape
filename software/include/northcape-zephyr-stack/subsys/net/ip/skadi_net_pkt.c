...
#endif /* CONFIG_NET_BUF_VARIABLE_DATA_SIZE */
...
struct k_mem_slab *rx_pkts_ptr, *tx_pkts_ptr;
...
void net_pkt_unref(struct net_pkt *pkt)
{
	...
	NET_DBG("%s [%d] pkt %p ref %ld frags %p (%s():%d)",
		slab2str(pkt->slab), skadi_mem_slab_num_free_get(pkt->slab),
	...
	skadi_mem_slab_free(pkt->slab, (void *)pkt);
}


#ifdef CONFIG_SKADI_OS
	#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, skadi_net_pkt_unref_debug, struct net_pkt *pkt, const char *caller, int line)
		net_pkt_unref_debug(pkt, caller, line);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_unref_debug);
	#else
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, skadi_net_pkt_unref, struct net_pkt *pkt)
		net_pkt_unref(pkt);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_unref);
	#endif	
#endif
...
#ifdef CONFIG_SKADI_OS
	#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(struct net_pkt *, skadi_net_pkt_ref_debug, struct net_pkt *pkt, const char *caller, int line)
		return net_pkt_ref_debug(pkt, caller, line);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_ref_debug);
	#else
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(struct net_pkt *, skadi_net_pkt_ref, struct net_pkt *pkt)
		return net_pkt_ref(pkt);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_ref);
	#endif	
#endif
...
void net_pkt_print(void)
{
	NET_DBG("TX %u RX %u RDATA %d TDATA %d",
		skadi_mem_slab_num_free_get(tx_pkts_ptr),
		skadi_mem_slab_num_free_get(rx_pkts_ptr),
...
#endif
{
	k_timepoint_t end = skadi_sys_timepoint_calc(timeout);
...
#endif
{
   ...
   if (size == 0) {
	...
	if (!pool) {
		pool = pkt->slab == tx_pkts_ptr ? &tx_bufs : &rx_bufs;
	}
...
#endif
{
	...
	if (IS_ENABLED(CONFIG_NET_PKT_RXTIME_STATS) ||
		...
		create_time = skadi_cycle_get_32();
	} else {
		ARG_UNUSED(create_time);
	}

	pkt = skadi_mem_slab_alloc_alt(slab, timeout);
	if (!pkt) {
		return NULL;
	}
...
{
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	return pkt_alloc(tx_pkts_ptr, timeout, caller, line);
#else
	return pkt_alloc(tx_pkts_ptr, timeout);
#endif
}
...
{
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	return pkt_alloc(rx_pkts_ptr, timeout, caller, line);
#else
	return pkt_alloc(rx_pkts_ptr, timeout);
#endif
}
...
#endif
{
	...
	if (pkt) {
		// iface was provided by the driver, which receives a token
		// this breaks logic that compares the pointer
		net_pkt_set_iface(pkt, iface ? iface->net_if_or : iface);
	}
...
{
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	return pkt_alloc_on_iface(tx_pkts_ptr, iface, timeout, caller, line);
#else
	return pkt_alloc_on_iface(tx_pkts_ptr, iface, timeout);
#endif
}
...
{
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	return pkt_alloc_on_iface(rx_pkts_ptr, iface, timeout, caller, line);
#else
	return pkt_alloc_on_iface(rx_pkts_ptr, iface, timeout);
#endif
}
...
{
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	return pkt_alloc_with_buffer(tx_pkts_ptr, iface, size, family,
...
{
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	return pkt_alloc_with_buffer(rx_pkts_ptr, iface, size, family,
					proto, timeout, caller, line);
#else
	return pkt_alloc_with_buffer(rx_pkts_ptr, iface, size, family,
					proto, timeout);
...
#ifdef CONFIG_SKADI_OS
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(struct net_pkt *, skadi_net_pkt_rx_alloc_with_buffer_debug, struct net_if *iface, size_t size, sa_family_t family, enum net_ip_protocol proto, k_timeout_t timeout, const char *caller, int line)
	return net_pkt_rx_alloc_with_buffer_debug(iface, size, family, proto, timeout, caller, line);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_rx_alloc_with_buffer_debug);
#else
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(struct net_pkt *, skadi_net_pkt_rx_alloc_with_buffer, struct net_if *iface, size_t size, sa_family_t family, enum net_ip_protocol proto, k_timeout_t timeout)
	return net_pkt_rx_alloc_with_buffer(iface, size, family, proto, timeout);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_rx_alloc_with_buffer);
#endif
#endif
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_net_pkt_read, struct net_pkt *pkt, void *data, size_t length)
		return net_pkt_read(pkt, data, length);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_pkt_read);
#endif
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_net_pkt_write, struct net_pkt *pkt, const void *data, size_t length)
		return net_pkt_write(pkt, data, length);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_pkt_write);
#endif
...
{
	return net_pkt_clone_internal(pkt, rx_pkts_ptr, timeout);
}
...
int net_pkt_pull(struct net_pkt *pkt, size_t length)
{
	...
	while (length) {
		...
		if (left) {
			...
			/* memmove might break - buffer not writable */
			c_op->buf->data += rem;
#else
...
void net_pkt_init(void)
{
	...
	NET_DBG("Allocating %u RX (%zu bytes), %u TX (%zu bytes), "
		...
		skadi_mem_slab_num_free_get(rx_pkts_ptr),
		(size_t)(skadi_mem_slab_num_free_get(rx_pkts_ptr) *
			 sizeof(struct net_pkt)),
		skadi_mem_slab_num_free_get(tx_pkts_ptr),
		(size_t)(skadi_mem_slab_num_free_get(tx_pkts_ptr) *
			 sizeof(struct net_pkt)),
		get_frees(&rx_bufs), get_size(&rx_bufs),
		get_frees(&tx_bufs), get_size(&tx_bufs));
#endif

	if(skadi_cap_ops_derive(&rx_pkts, restriction, sizeof(rx_pkts), skadi_get_capability_offset(&rx_pkts), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &slab_ptr) == false  || !slab_ptr){
		LOG_ERR("Could not derive for k_mem_slab for RX");
		return;
	}

	rx_pkts_ptr = (void *) slab_ptr;

	if(skadi_cap_ops_derive(&tx_pkts, restriction, sizeof(tx_pkts), skadi_get_capability_offset(&tx_pkts), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &slab_ptr) == false  || !slab_ptr){
		LOG_ERR("Could not derive for k_mem_slab for TX");
		return;
	}

	tx_pkts_ptr = (void *) slab_ptr;

	if(skadi_cap_ops_derive(tx_pkts_ptr->buffer, restriction, tx_pkts_ptr->info.num_blocks * WB_UP(tx_pkts_ptr->info.block_size), skadi_get_capability_offset(tx_pkts_ptr->buffer), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &buffer_ptr) == false  || !buffer_ptr){
		LOG_ERR("Could not derive for k_mem_slab->buffer for TX");
		return;
	}

	/* some internal pointers, such as free list, buffer, wait q are not set (correctly) yet */
	if(skadi_mem_slab_init(tx_pkts_ptr, (void*)buffer_ptr, tx_pkts_ptr->info.block_size, tx_pkts_ptr->info.num_blocks) != 0){
		LOG_ERR("Could not init slab!");
		return;
	}

	if(skadi_cap_ops_derive(rx_pkts_ptr->buffer, restriction, rx_pkts_ptr->info.num_blocks * WB_UP(rx_pkts_ptr->info.block_size), skadi_get_capability_offset(rx_pkts_ptr->buffer), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &buffer_ptr) == false  || !buffer_ptr){
		LOG_ERR("Could not derive for k_mem_slab->buffer for TX");
		return;
	}

	/* some internal pointers, such as free list, buffer, wait q are not set (correctly) yet */
	if(skadi_mem_slab_init(rx_pkts_ptr, (void*)buffer_ptr, rx_pkts_ptr->info.block_size, rx_pkts_ptr->info.num_blocks) != 0){
		LOG_ERR("Could not init slab!");
		return;
	}

	rx_bufs.free = skadi_allocator_alloc_rw(sizeof(*rx_bufs.free));

	if(!rx_bufs.free){
		LOG_ERR("Could not alloc LIFO!");
		return;
	}

	tx_bufs.free = skadi_allocator_alloc_rw(sizeof(*tx_bufs.free));

	if(!tx_bufs.free){
		LOG_ERR("Could not alloc LIFO!");
		return;
	}
#ifdef CONFIG_NET_BUF_FIXED_DATA_SIZE
	/* net_bufs are used for fragments etc. */
	if(skadi_cap_ops_derive(net_buf_data_rx_bufs, restriction, sizeof(net_buf_data_rx_bufs), skadi_get_capability_offset(net_buf_data_rx_bufs), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &buffer_ptr) == false || !buffer_ptr){
		LOG_ERR("Could not derive from buf struct!");
		return;
	}

	net_buf_fixed_rx_bufs.data_pool = (uint8_t*) buffer_ptr;

	if(skadi_cap_ops_derive(net_buf_data_tx_bufs, restriction, sizeof(net_buf_data_tx_bufs), skadi_get_capability_offset(net_buf_data_tx_bufs), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &buffer_ptr) == false || !buffer_ptr){
		LOG_ERR("Could not derive from buf struct!");
		return;
	}

	net_buf_fixed_tx_bufs.data_pool = (uint8_t*) buffer_ptr;

#endif

	if(skadi_cap_ops_derive(_net_buf_rx_bufs, restriction, sizeof(_net_buf_rx_bufs), skadi_get_capability_offset(_net_buf_rx_bufs), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &buffer_ptr) == false || !buffer_ptr){
		LOG_ERR("Could not derive from buf struct!");
		return;
	}

	rx_bufs.__bufs = (struct net_buf*) buffer_ptr;

	

	if(skadi_cap_ops_derive(_net_buf_tx_bufs, restriction, sizeof(_net_buf_tx_bufs), skadi_get_capability_offset(_net_buf_tx_bufs), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS,  &buffer_ptr) == false || !buffer_ptr){
		LOG_ERR("Could not derive from buf struct!");
		return;
	}

	tx_bufs.__bufs = (struct net_buf*) buffer_ptr;
#ifdef CONFIG_NET_BUF_VARIABLE_DATA_SIZE
	skadi_heap_init(tx_bufs.alloc->alloc_data, kheap_net_buf_mem_pool_tx_bufs, CONFIG_NET_PKT_BUF_TX_DATA_POOL_SIZE);
	skadi_heap_init(rx_bufs.alloc->alloc_data, kheap_net_buf_mem_pool_rx_bufs, CONFIG_NET_PKT_BUF_RX_DATA_POOL_SIZE);
#endif

	skadi_lifo_init(rx_bufs.free);
	skadi_lifo_init(tx_bufs.free);
}
