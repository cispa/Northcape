...
#ifdef CONFIG_SKADI_OS
		struct k_work_delayable *dwork = reassembly[i].timer;
#else
		struct k_work_delayable *dwork = &reassembly[i].timer;
#endif

	for (i = 0; i < CONFIG_NET_IPV4_FRAGMENT_MAX_COUNT; i++) {
		if (k_work_delayable_remaining_get(dwork) &&
		    reassembly[i].id == id &&
...
#ifdef CONFIG_SKADI_OS
	skadi_work_reschedule(dwork, K_SECONDS(CONFIG_NET_IPV4_FRAGMENT_TIMEOUT));
#else
	k_work_reschedule(dwork, K_SECONDS(CONFIG_NET_IPV4_FRAGMENT_TIMEOUT));
#endif
...
static bool reassembly_cancel(uint32_t id, struct in_addr *src, struct in_addr *dst)
{
	...
	for (i = 0; i < CONFIG_NET_IPV4_FRAGMENT_MAX_COUNT; i++) {
		...
		remaining = k_ticks_to_ms_ceil32(
			k_work_delayable_remaining_get(dwork));
#ifdef CONFIG_SKADI_OS
		skadi_work_cancel_delayable(dwork);
#else
...
{
#ifdef CONFIG_SKADI_OS
		struct k_work_delayable *dwork = reass->timer;
#else
		struct k_work_delayable *dwork = &reass->timer;
#endif
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, reassembly_timeout, struct k_work *work)
#else
static void reassembly_timeout(struct k_work *work)
#endif
{
...
#ifdef CONFIG_SKADI_OS
	struct net_ipv4_reassembly *reass = work->user_data;
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(reassembly_timeout)
#endif
...
#ifdef CONFIG_SKADI_OS
		reassembly[i].timer = skadi_allocator_alloc_rw(sizeof(*reassembly[i].timer));
		if(!reassembly[i].timer){
			LOG_ERR("Could not allocate reassembly timer %d!",i);
			return;
		}
		skadi_work_init_delayable(reassembly[i].timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(reassembly_timeout));
		reassembly[i].timer->user_data = reassembly[i];
#else
