...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(struct net_pkt *, skadi_net_pkt_rx_alloc_contiguous, size_t size, k_timeout_t timeout)
	struct net_pkt *pkt = net_pkt_rx_alloc_contiguous(size, timeout);
	struct net_pkt *buffer;

	if(!pkt){
		return pkt;
	}

	buf = skadi_net_buf_alloc_rx_with_data(buf_data, XILINX_AXIENET_ETH_BUFFER_SIZE, K_NO_WAIT);

	__ASSERT_NO_MSG(buf);

	if(!buf){
		LOG_ERR("Could not allocate buffer metadata!");
		return -ENOMEM;
	}

	skadi_net_pkt_append_buffer(pkt, buf);

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_rx_alloc_debug);
#endif
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, skadi_net_pkt_read, struct net_pkt *pkt, void *data, size_t length)
		return net_pkt_read(pkt, data, length);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_read);
#endif
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, skadi_net_pkt_write, struct net_pkt *pkt, const void *data, size_t length)
		return net_pkt_write(pkt, data, length);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_pkt_write);
#endif
