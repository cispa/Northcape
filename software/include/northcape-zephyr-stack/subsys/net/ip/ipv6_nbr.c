...
#ifdef CONFIG_SKADI_OS
static struct k_mutex *nbr_lock;

void net_ipv6_nbr_lock(void)
{
	__ASSERT_NO_MSG(nbr_lock);
	(void)skadi_mutex_lock(nbr_lock, K_FOREVER);
}

void net_ipv6_nbr_unlock(void)
{
	skadi_mutex_unlock(nbr_lock);
}
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, ipv6_ns_reply_timeout, struct k_work *work)
#else
static void ipv6_ns_reply_timeout(struct k_work *work)
#endif
{
#ifdef CONFIG_SKADI_OS
	int64_t current = skadi_uptime_get();
#else
	...
	for (i = 0; i < CONFIG_NET_IPV6_MAX_NEIGHBORS; i++) {
		...
		if (remaining > 0) {
#ifdef CONFIG_SKADI_OS
			__ASSERT_NO_MSG(ipv6_ns_reply_timer);
			if (!skadi_work_delayable_remaining_get(
				    ipv6_ns_reply_timer)) {
				skadi_work_reschedule(ipv6_ns_reply_timer,
						  K_MSEC(remaining));
			}
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(ipv6_ns_reply_timeout)
#endif
...
static void ipv6_nd_restart_reachable_timer(struct net_nbr *nbr, int64_t time)
{
	...
	if (nbr) {
#ifdef CONFIG_SKADI_OS
		net_ipv6_nbr_data(nbr)->reachable = skadi_uptime_get();
#else
	...
	remaining = k_ticks_to_ms_ceil32(
		skadi_work_delayable_remaining_get(ipv6_nd_reachable_timer));
	if (!remaining || remaining > time) {
		skadi_work_reschedule(ipv6_nd_reachable_timer, K_MSEC(time));
	}
#else
	remaining = k_ticks_to_ms_ceil32(
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, ipv6_nd_reachable_timeout, struct k_work *work)
#else
static void ipv6_nd_reachable_timeout(struct k_work *work)
#endif
{
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(ipv6_nd_reachable_timeout)
#endif
		     ...
		     bool is_my_address)
{
	...
	if (pending) {
...
#ifdef CONFIG_SKADI_OS
		net_ipv6_nbr_data(nbr)->send_ns = skadi_uptime_get();
#else
		...
		if (!skadi_work_delayable_remaining_get(ipv6_ns_reply_timer)) {
			skadi_work_reschedule(ipv6_ns_reply_timer,
...
{
#ifdef CONFIG_SKADI_OS
	return net_timeout_remaining(&ifaddr->lifetime, skadi_uptime_get_32());
#else
...
#ifdef CONFIG_SKADI_OS
	ipv6_ns_reply_timer = skadi_allocator_alloc_rw(sizeof(*ipv6_ns_reply_timer));

	if(!ipv6_ns_reply_timer){
		NET_ERR("Could not allocate reply timer!");
		return;
	}

	skadi_work_init_delayable(ipv6_ns_reply_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(ipv6_ns_reply_timeout));
#else
...
#ifdef CONFIG_SKADI_OS
	ipv6_nd_reachable_timer = skadi_allocator_alloc_rw(sizeof(*ipv6_nd_reachable_timer));

	if(!ipv6_nd_reachable_timer){
		NET_ERR("Could not allocate reply timer!");
		return;
	}

	skadi_work_init_delayable(ipv6_nd_reachable_timer,
			      SKADI_SUBSYSTEM_FUNCTION_POINTER(ipv6_nd_reachable_timeout));
#else
...
#ifdef CONFIG_SKADI_OS
	nbr_lock = skadi_allocator_alloc_rw(sizeof(*nbr_lock));

	if(!nbr_lock){
		LOG_ERR("Could not alloc neighbor lock!");
		return;
	}

	skadi_mutex_init(nbr_lock);
#endif
