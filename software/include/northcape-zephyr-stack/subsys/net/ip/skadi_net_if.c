...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(struct net_if *, __skadi_net_if_get_by_index, int index)
	return z_impl_net_if_get_by_index(index);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_if_get_by_index)
...
static bool net_if_tx(struct net_if *iface, struct net_pkt *pkt)
{
	...
	if (net_if_flag_is_set(iface, NET_IF_LOWER_UP)) {
		...
		if (IS_ENABLED(CONFIG_NET_PKT_TXTIME_STATS) ||
		    IS_ENABLED(CONFIG_TRACING_NET_CORE)) {
			uint32_t end_tick = skadi_cycle_get_32();
...
void net_if_stats_reset(struct net_if *iface)
{
	...
	STRUCT_SECTION_FOREACH(net_if, tmp) {
		if (iface == tmp) {
			net_if_lock_write(iface);
			memset(&iface->stats, 0, sizeof(iface->stats));
			net_if_unlock_write(iface);
			return;
		}
	}
#else
...
static inline void init_iface(struct net_if *iface)
{
	...
	iface->lock = skadi_allocator_alloc_rw(sizeof(*iface->lock));
	iface->tx_lock = skadi_allocator_alloc_rw(sizeof(*iface->tx_lock));
	iface->writer_unlock_event = skadi_allocator_alloc_rw(sizeof(*iface->writer_unlock_event));

	if(!iface->lock || !iface->tx_lock || !iface->writer_unlock_event){
		LOG_ERR("Could not allocate mutexes - not initializing interface!");
		return;
	}

	skadi_mutex_init(iface->lock);
	skadi_mutex_init(iface->tx_lock);
	skadi_event_init(iface->writer_unlock_event);
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, skadi_net_if_set_link_addr_locked, struct net_if *iface, uint8_t *addr, uint8_t len, enum net_link_type type)
	return net_if_set_link_addr_locked(iface, addr, len, type);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_if_set_link_addr_locked);
#endif
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(struct net_if *, __skadi_net_if_get_default)
	return net_if_get_default();
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_if_get_default)
...
static void iface_router_update_timer(uint32_t now)
{
	...
	__ASSERT_NO_MSG(router_timer);
	if (new_delay == UINT32_MAX) {
		skadi_work_cancel_delayable(router_timer);
	} else {
		skadi_work_reschedule(router_timer, K_MSEC(new_delay));
	}

	skadi_mutex_unlock(lock);
}

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, iface_router_expired, struct k_work *work)
{
#ifdef CONFIG_SKADI_OS
	uint32_t current_time = skadi_uptime_get_32();
#else
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(iface_router_expired)
...
#ifdef CONFIG_SKADI_OS
			routers[i].life_start = skadi_uptime_get_32();
#else
...
#ifdef CONFIG_SKADI_OS
		iface_router_update_timer(skadi_uptime_get_32());
#else
...
{
	router_timer = skadi_allocator_alloc_rw(sizeof(*router_timer));
	if(router_timer == NULL){
		LOG_ERR("Could not allocate router timer!");
		return;
	}
	skadi_work_init_delayable(router_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(iface_router_expired));
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, dad_timeout, struct k_work *work)
{
#ifdef CONFIG_SKADI_OS
	uint32_t current_time = skadi_uptime_get_32();
#else
	...
	if ((ifaddr != NULL) && (delay > 0)) {
		__ASSERT_NO_MSG(dad_timer);
		skadi_work_reschedule(dad_timer, K_MSEC((uint32_t)delay));
	}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(dad_timeout);
			   ...
			   struct net_if_addr *ifaddr)
{
	...
	if (net_if_is_up(iface)) {
		...
		if (!net_ipv6_start_dad(iface, ifaddr)) {
#ifdef CONFIG_SKADI_OS
			ifaddr->dad_start = skadi_uptime_get_32();
#else
			...
			if (!skadi_work_delayable_remaining_get(dad_timer)) {
				skadi_work_reschedule(dad_timer,
						  K_MSEC(DAD_TIMEOUT));
			}
		}
	} else {
...
{
	dad_timer = skadi_allocator_alloc_rw(sizeof(*dad_timer));
	if(dad_timer == NULL){
		LOG_ERR("Could not allocate DAD timer!");
		return;
	}
	skadi_work_init_delayable(dad_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(dad_timeout));
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, rs_timeout, struct k_work *work)
{
#ifdef CONFIG_SKADI_OS
	uint32_t current_time = skadi_uptime_get_32();
#else
	...
	if ((ipv6 != NULL) && (delay > 0)) {
		__ASSERT_NO_MSG(rs_timer);
		skadi_work_reschedule(rs_timer, K_MSEC(ipv6->rs_start +
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(rs_timeout)
...
void net_if_start_rs(struct net_if *iface)
{
	...
	if (!net_ipv6_start_rs(iface)) {
#ifdef CONFIG_SKADI_OS
		ipv6->rs_start = skadi_uptime_get_32();
#else
		...
		/* FUTURE: use schedule, not reschedule. */
		__ASSERT_NO_MSG(rs_timer);
		if (!skadi_work_delayable_remaining_get(rs_timer)) {
			skadi_work_reschedule(rs_timer, K_MSEC(RS_TIMEOUT));
		}
	}
...
{
	rs_timer = skadi_allocator_alloc_rw(sizeof(*rs_timer));
	if(rs_timer == NULL){
		LOG_ERR("Could not allocate DAD timer!");
		return;
	}
	skadi_work_init_delayable(rs_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(rs_timeout));
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, address_lifetime_timeout, struct k_work *work)
{
	...
	if (next_update != UINT32_MAX) {
		...
		__ASSERT_NO_MSG(address_lifetime_timer);
		skadi_work_reschedule(address_lifetime_timer, K_MSEC(next_update));
	}

	skadi_mutex_unlock(lock);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(address_lifetime_timeout)
...
static void address_start_timer(struct net_if_addr *ifaddr, uint32_t vlifetime)
{
	...
	sys_slist_append(&active_address_lifetime_timers,
			 &ifaddr->lifetime.node);
#ifdef CONFIG_SKADI_OS
	net_timeout_set(&ifaddr->lifetime, vlifetime, skadi_uptime_get_32());
#else
	net_timeout_set(&ifaddr->lifetime, vlifetime, k_uptime_get_32());
#endif
	__ASSERT_NO_MSG(address_lifetime_timer);
	skadi_work_reschedule(address_lifetime_timer, K_NO_WAIT);
}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(struct net_if_addr *, __skadi_net_if_ipv6_addr_add, struct net_if *iface, struct in6_addr *addr, enum net_addr_type addr_type, uint32_t vlifetime)
	struct net_if_addr *ret = net_if_ipv6_addr_add(iface, addr, addr_type, vlifetime);
	// dropped by caller
	ret = skadi_cap_ops_derive_arg(ret, sizeof(*ret));
	__ASSERT_NO_MSG(ret);

	return ret;
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_if_ipv6_addr_add)
...
static void prefix_lifetime_expired(struct net_if_ipv6_prefix *ifprefix)
{
	...
	net_if_lock_write(ifprefix->iface);
	...
	net_if_unlock_write(ifprefix->iface);
}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, prefix_lifetime_timeout, struct k_work *work)
{
	uint32_t next_update = UINT32_MAX;
#ifdef CONFIG_SKADI_OS
	uint32_t current_time = skadi_uptime_get_32();
	...
	if (next_update != UINT32_MAX) {
		__ASSERT_NO_MSG(prefix_lifetime_timer);
		skadi_work_reschedule(prefix_lifetime_timer, K_MSEC(next_update));
	}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(prefix_lifetime_timeout)
...
#ifdef CONFIG_SKADI_OS
	net_timeout_set(&ifprefix->lifetime, lifetime, skadi_uptime_get_32());
#else
	net_timeout_set(&ifprefix->lifetime, lifetime, k_uptime_get_32());
#endif
	__ASSERT_NO_MSG(prefix_lifetime_timer);
	skadi_work_reschedule(prefix_lifetime_timer, K_NO_WAIT);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(struct net_if_ipv6_prefix *, __skadi_net_if_ipv6_prefix_add, struct net_if *iface, struct in6_addr *prefix, uint8_t len, uint32_t lifetime)
	struct net_if_ipv6_prefix *ret = net_if_ipv6_prefix_add(iface, prefix, len, lifetime);
	// dropped by caller
	ret = skadi_cap_ops_derive_arg(ret, sizeof(*ret));
	__ASSERT_NO_MSG(ret);

	return ret;
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_if_ipv6_prefix_add)
...
bool net_if_ipv6_addr_onlink(struct net_if **iface, struct in6_addr *addr)
{
	...
	STRUCT_SECTION_FOREACH(net_if, tmp) {
		...
		net_if_lock_read(tmp);

		ipv6 = tmp->config.ip.ipv6;
		if (!ipv6) {
			net_if_unlock_read(tmp);
			continue;
		}
...
#ifdef CONFIG_SKADI_OS
	router->life_start = skadi_uptime_get_32();
#else
...
uint32_t net_if_ipv6_calc_reachable_time(struct net_if_ipv6 *ipv6)
{
	...
	return min_reachable +
	       skadi_sys_rand32_get() % (max_reachable - min_reachable);
#else
	return min_reachable +
...
static void iface_ipv6_init(int if_count)
{
	...
	address_lifetime_timer = skadi_allocator_alloc_rw(sizeof(*address_lifetime_timer));
	if(address_lifetime_timer == NULL){
		LOG_ERR("Could not allocate address lifetime timer!");
		return;
	}
	skadi_work_init_delayable(address_lifetime_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(address_lifetime_timeout));

	prefix_lifetime_timer = skadi_allocator_alloc_rw(sizeof(*prefix_lifetime_timer));
	if(prefix_lifetime_timer == NULL){
		LOG_ERR("Could not allocate prefix lifetime timer!");
		return;
	}
	skadi_work_init_delayable(prefix_lifetime_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(prefix_lifetime_timeout));
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(struct net_if_addr *, __skadi_net_if_ipv4_addr_add, struct net_if *iface, struct in_addr *addr, enum net_addr_type addr_type, uint32_t vlifetime)
	struct net_if_addr *ret = net_if_ipv4_addr_add(iface, addr, addr_type, vlifetime);
	// dropped by caller
	ret = skadi_cap_ops_derive_arg(ret, sizeof(*ret));
	__ASSERT_NO_MSG(ret);

	return ret;
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_if_ipv4_addr_add)
			       ...
			       unsigned int maddr_count)
{
	...
	if (!ifaddr->is_infinite) {
		...
		if (sys_slist_is_empty(&active_address_lifetime_timers)) {
			__ASSERT_NO_MSG(address_lifetime_timer);
			skadi_work_cancel_delayable(address_lifetime_timer);
		}
#endif
		skadi_mutex_unlock(lock);
	}
			      ...
			      enum net_if_checksum_type chksum_type)
{
	...
	switch(caps){
		case ETHERNET_HW_RX_CHKSUM_OFFLOAD:
			if(iface->rx_checksum_cache_set){
				return !((iface->rx_checksum_support_cache & chksum_type) == chksum_type);
			}
			break;
		case ETHERNET_HW_TX_CHKSUM_OFFLOAD:
			if(iface->tx_checksum_cache_set){
				return !((iface->tx_checksum_support_cache & chksum_type) == chksum_type);
			}
			break;
		default:
			LOG_WRN("Uncacheable caps %x!", caps);
			break;
	}

	if (!(net_eth_get_hw_capabilities(iface) & caps)) {
		/* we can also eliminate this call in the future */
		if (caps == ETHERNET_HW_RX_CHKSUM_OFFLOAD) {
			iface->rx_checksum_cache_set = 1;
			iface->rx_checksum_support_cache = 0;
		} else {
			iface->tx_checksum_cache_set = 1;
			iface->tx_checksum_support_cache = 0;
		}
	...
	ret = !((config.chksum_support & chksum_type) == chksum_type);

	if (caps == ETHERNET_HW_RX_CHKSUM_OFFLOAD) {
		iface->rx_checksum_cache_set = 1;
		iface->rx_checksum_support_cache = config.chksum_support;
	} else {
		iface->tx_checksum_cache_set = 1;
		iface->tx_checksum_support_cache = config.chksum_support;
	}
...
int net_if_suspend(struct net_if *iface)
{
	...
	net_stats_add_suspend_start_time(iface, skadi_cycle_get_32());
...
int net_if_resume(struct net_if *iface)
{
	...
	net_stats_add_suspend_end_time(iface, skadi_cycle_get_32());
...
#if defined(CONFIG_NET_PKT_TIMESTAMP_THREAD)
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, net_tx_ts_thread, void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p1);
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);
	...
	while (1) {
		pkt = skadi_fifo_get(tx_ts_queue, K_FOREVER);
		if (pkt) {
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(net_tx_ts_thread)
...
{
	skadi_fifo_put(tx_ts_queue, pkt);
	net_pkt_ref(pkt);
}

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_net_if_add_tx_timestamp, struct net_pkt *pkt);
	net_if_add_tx_timestamp(pkt);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_if_add_tx_timestamp)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_net_if_get_name, struct net_if *iface, char *buf, int len)
	return net_if_get_name(iface, buf, len);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_if_get_name)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_net_if_get_by_name, const char *name)
	return net_if_get_by_name(name);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_net_if_get_by_name)
...
void net_if_init(void)
{
...
#ifdef CONFIG_NET_PKT_TIMESTAMP_THREAD
	tx_ts_queue = skadi_allocator_alloc_rw(sizeof(*tx_ts_queue));

	if(!tx_ts_queue){
		LOG_ERR("Could not alloc tx_ts_queue!");
		return;
	}

	skadi_fifo_init(tx_ts_queue);

#endif /* CONFIG_NET_PKT_TIMESTAMP_THREAD */

	if(!lock){
		lock = skadi_allocator_alloc_rw(sizeof(*lock));
		if(!lock){
			LOG_ERR("Could not alloc lock!");
			return;
		}
		skadi_mutex_init(lock);
	}
...
#if defined(CONFIG_NET_PKT_TIMESTAMP_THREAD)
	{
		...
		params.new_thread = &tx_thread_ts;
		params.stack = tx_ts_stack;
		params.stack_size = K_THREAD_STACK_SIZEOF(tx_ts_stack);
		params.entry = SKADI_SUBSYSTEM_FUNCTION_POINTER(net_tx_ts_thread);

		params.p1 = NULL;
		params.p2 = NULL;
		params.p3 = NULL;
		params.prio = K_PRIO_COOP(1);
		params.options = 0;
		params.delay = K_NO_WAIT;

		skadi_thread_create(&params);
		skadi_thread_name_set(&tx_thread_ts, "tx_tstamp");
	}
#endif /* CONFIG_NET_PKT_TIMESTAMP_THREAD */
