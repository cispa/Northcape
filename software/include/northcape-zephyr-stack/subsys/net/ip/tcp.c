...
static void keep_alive_timer_restart(struct tcp *conn)
{
	...
	k_work_reschedule_for_queue(tcp_work_q_ptr, &conn->keepalive_timer,
...
static int tcp_conn_unref(struct tcp *conn)
{
	 ...
	 * is released.
	 */
	k_work_submit_to_queue(tcp_work_q_ptr, &conn->conn_release);
...
static bool tcp_send_process_no_lock(struct tcp *conn)
{
	...
	if (conn->in_retransmission) {
		k_work_reschedule_for_queue(tcp_work_q_ptr, &conn->send_timer,
					    K_MSEC(TCP_RTO_MS));
		       ...
		       uint32_t seq)
{
	...
	if (is_destination_local(pkt)) {
		 ...
		 */
		k_work_schedule_for_queue(tcp_work_q_ptr,
...
static int tcp_send_queued_data(struct tcp *conn)
{
	...
	if (subscribe) {
		...
		k_work_reschedule_for_queue(tcp_work_q_ptr, &conn->send_data_timer,
					    K_MSEC(TCP_RTO_MS));
	}
 out:
	return ret;
...
static void tcp_resend_data(struct k_work *work)
{
	...
	if (ret == 0) {
		if (conn->in_close && conn->send_data_total == 0) {
			...
			k_work_reschedule_for_queue(tcp_work_q_ptr,
						    &conn->fin_timer,
						    FIN_TIMEOUT);
					  ...
					  size_t *len, bool psh)
{
	...
	if (tcp_short_window(conn) || !psh) {
		__ASSERT_NO_MSG(tcp_work_q_ptr);
		k_work_schedule_for_queue(tcp_work_q_ptr, &conn->ack_timer,
					  ACK_DELAY);
	} else {
		k_work_cancel_delayable(&conn->ack_timer);
		tcp_out(conn, ACK);
...
#define THREAD_PRIORITY K_PRIO_PREEMPT(CONFIG_NET_TCP_WORKER_PRIO)
	...
	if(skadi_cap_ops_derive(&tcp_work_q, 0, 0, false, sizeof(tcp_work_q), skadi_get_capability_offset(&tcp_work_q), true, true, false,  SKADI_RESTRICTIONS_NONE, &derived_work_queue) == false || derived_work_queue == 0){
	 ...
	 */
	k_work_queue_start(tcp_work_q_ptr, work_q_stack,
	...
	k_thread_name_set(&tcp_work_q_ptr->thread, "tcp_work");
