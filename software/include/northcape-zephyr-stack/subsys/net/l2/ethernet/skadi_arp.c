...
static void arp_entry_cleanup(struct arp_entry *entry, bool pending)
{
	...
	if (pending) {
		...
		while (!skadi_fifo_is_empty(entry->pending_queue)) {
			pkt = skadi_fifo_get(entry->pending_queue, K_FOREVER);
					       ...
					       struct in_addr *dst)
{
	...
	if (sys_slist_is_empty(&arp_pending_entries)) {
		__ASSERT_NO_MSG(arp_request_timer);
		skadi_work_cancel_delayable(arp_request_timer);
	}
...
static void arp_entry_register_pending(struct arp_entry *entry)
{
	...
	entry->req_start = skadi_uptime_get_32();

	__ASSERT_NO_MSG(arp_request_timer);

	/* Let's start the timer if necessary */
	if (!skadi_work_delayable_remaining_get(arp_request_timer)) {
		skadi_work_reschedule(arp_request_timer,
				  K_MSEC(ARP_REQUEST_TIMEOUT));
	}
}

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, arp_request_timeout, struct k_work *work)
{
	uint32_t current = skadi_uptime_get_32();
	struct arp_entry *entry, *next;

	ARG_UNUSED(work);

	skadi_mutex_lock(arp_mutex, K_FOREVER);
	...
	skadi_mutex_unlock(arp_mutex);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(arp_request_timeout)
					  ...
					  struct in_addr *current_ip)
{
	...
	if (entry) {
		if (!net_pkt_ipv4_acd(pkt)) {
			/* packet should be readable to everyone now */
			skadi_fifo_put(entry->pending_queue, net_pkt_ref(pending));
		}
				...
				struct in_addr *current_ip)
{
	...
	if (!entry) {
		...
		if (!entry) {
			 ...
			 */
			if (skadi_queue_unique_append(&entry->pending_queue->_queue,
						  net_pkt_ref(pkt))) {
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, arp_gratuitous_work_handler, struct k_work *work)
{
	ARG_UNUSED(work);

	net_if_foreach(iface_cb, NULL);

	__ASSERT_NO_MSG(arp_gratuitous_work);
	skadi_work_reschedule(arp_gratuitous_work,
			  K_SECONDS(CONFIG_NET_ARP_GRATUITOUS_INTERVAL));
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(arp_gratuitous_work_handler)
#endif /* defined(CONFIG_NET_ARP_GRATUITOUS_TRANSMISSION) */
	...
	if (!entry) {
		...
		if (force) {
			...
			if (arp_ent) {
				...
				if (arp_ent) {
					arp_ent->req_start = skadi_uptime_get_32();
					arp_ent->iface = iface;
...
void net_arp_init(void)
{
	...
	arp_request_timer = skadi_allocator_alloc_rw(sizeof(*arp_request_timer));

	if(!arp_request_timer){
		LOG_ERR("Failed to allocate request timer!");
		return;
	}
	...
	for (i = 0; i < CONFIG_NET_ARP_TABLE_SIZE; i++) {
		arp_entries[i].pending_queue = skadi_allocator_alloc_rw(sizeof(*arp_entries[i].pending_queue));
		
		if(!arp_entries[i].pending_queue){
			LOG_ERR("Failed to allocate pending queue!");
			return;
		}

		/* Inserting entry as free with initialised packet queue */
		skadi_fifo_init(arp_entries[i].pending_queue);
		sys_slist_prepend(&arp_free_entries, &arp_entries[i].node);
	}
	

	skadi_work_init_delayable(arp_request_timer, SKADI_SUBSYSTEM_FUNCTION_POINTER(arp_request_timeout));

	arp_mutex = skadi_allocator_alloc_rw(sizeof(*arp_mutex));

	if(!arp_mutex){
		LOG_ERR("Failed to alloc mutex!");
		return;
	}
	
	skadi_mutex_init(arp_mutex);
	...
	skadi_work_init_delayable(arp_gratuitous_work,
			      SKADI_FUNCTION_POINTER(arp_gratuitous_work_handler));
	skadi_work_reschedule(arp_gratuitous_work,
			  K_SECONDS(CONFIG_NET_ARP_GRATUITOUS_INTERVAL));
#endif /* defined(CONFIG_NET_ARP_GRATUITOUS_TRANSMISSION) */
}
