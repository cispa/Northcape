...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, carrier_on_off, struct k_work *work)
#else
static void carrier_on_off(struct k_work *work)
#endif
{
	#ifdef CONFIG_SKADI_OS
	struct ethernet_context *ctx = work->user_data;
	#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(carrier_on_off)
#endif
...
#ifdef CONFIG_SKADI_OS
		skadi_work_submit(ctx->carrier_work);
#else
...
void ethernet_init(struct net_if *iface)
{
	struct ethernet_context *ctx = net_if_l2_data(iface);
#ifdef CONFIG_SKADI_OS
	struct k_work *carrier_work = skadi_allocator_alloc_rw(sizeof(*carrier_work));

	/* unrestricted capability token for scheduler subsystem */
	if(!carrier_work){
		LOG_ERR("Could not initialize ethernet context for work queue!");
		k_panic();
	}

	ctx->carrier_work = carrier_work;
#endif
	...
	ctx->iface = iface;
#ifdef CONFIG_SKADI_OS
	skadi_work_init(ctx->carrier_work, SKADI_SUBSYSTEM_FUNCTION_POINTER(carrier_on_off));
	carrier_work->user_data = ctx;
#else
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, skadi_ethernet_init, struct net_if *iface)
		ethernet_init(iface);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_ethernet_init)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, skadi_net_eth_carrier_on, struct net_if *iface)
		net_eth_carrier_on(iface);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_eth_carrier_on)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, skadi_net_eth_carrier_off, struct net_if *iface)
		net_eth_carrier_off(iface);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_net_eth_carrier_off)
#endif
