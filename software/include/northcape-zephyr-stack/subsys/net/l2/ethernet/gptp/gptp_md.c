...
#ifdef CONFIG_NET_GPTP_EXTRA_PROP_CALLBACK
	typedef void (*gptp_prop_callback_t)(int64_t );
	static gptp_prop_callback_t gptp_prop_callback = NULL;

	void gptp_set_prop_callback(gptp_prop_callback_t new_prop_callback){
		gptp_prop_callback = new_prop_callback;
	}
...
#ifdef CONFIG_NET_GPTP_EXTRA_PROP_CALLBACK
	if(gptp_prop_callback){
		gptp_prop_callback(prop_time);
	}
#endif
}
