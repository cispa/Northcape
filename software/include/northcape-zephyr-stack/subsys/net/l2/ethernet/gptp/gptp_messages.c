...
#ifdef CONFIG_NET_GPTP_EXTRA_SYNC_TX_CALLBACK

typedef void (*gptp_sync_tx_callback_t)(const struct net_ptp_time *tx_time);
static gptp_sync_tx_callback_t gptp_sync_tx_callback = NULL;

void gptp_register_sync_tx_callback(gptp_sync_tx_callback_t new_gptp_sync_tx_callback){
	gptp_sync_tx_callback = new_gptp_sync_tx_callback;
}
...
#ifdef CONFIG_NET_GPTP_EXTRA_SYNC_TX_CALLBACK
		if(gptp_sync_tx_callback){
			gptp_sync_tx_callback(&pkt->timestamp);
		}
#endif
