...
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_ARGS(int, __stdout_hook_trampoline, int arg);

int z_impl_zephyr_fputc(int a, FILE *out)
{
	__ASSERT(_stdout_hook, "stdout_hook has not been set yet - cannot print!");
	__stdout_hook_trampoline(a, _stdout_hook);
	return 0;
}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_stdout_hook_install, int(*hook_token)(int))
{
	_stdout_hook = hook_token;
	__stdout.flags |= _FDEV_SETUP_WRITE;
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_stdout_hook_install)

static LIBC_DATA int(*_stdin_hook)(int);

SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_ARGS(int, stdin_hook_trampoline, int arg);

int skadi_libc_stdin_get_wrapper(FILE *stream){
	__ASSERT_NO_MSG(_stdin_hook);
	return stdin_hook_trampoline((int)(uintptr_t) stream, _stdin_hook);
}

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_stdin_hook_install, int(*hook_token)(int))
{
	_stdin_hook = hook_token;
	__stdin.get = skadi_libc_stdin_get_wrapper;
	__stdin.flags |= _FDEV_SETUP_READ;
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_stdin_hook_install)
...
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_ARGS_ALLOW_SELF(int, __cbputc_wrapper, int c, void *ctx);

static int cbputc(char c, FILE *_s)
{
	struct cb_bits *s = (struct cb_bits *) _s;
	__cbputc_wrapper(c, s->ctx, s->out);
	return 0;
}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_cbvprintf, cbprintf_cb out, void *ctx, const char *fp, va_list ap);
	return cbvprintf(out, ctx, fp, ap);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_cbvprintf)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_exit, int status)
    _exit(status);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_exit)

#if defined(CONFIG_MULTITHREADING) && !defined(SKADI_SUBSYSTEM)
#define _LOCK_T void *
