...
/* for skadi, we can only configure this after ISR subsystem was loaded */
					  ...
					  void *cb_data)
{
	...
	SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_ARGS(skadi_cb_wrapper, const struct device *dev, void *user_data);
#endif
...
static void uart_ns16550_isr(const struct device *dev)
{
	...
	if (dev_data->cb) {
#ifdef CONFIG_SKADI_LOADER
		if(!skadi_token_is_in_our_text(dev_data->cb)){
			/* subsystem call needed, we probably jump into other subsystem */
			skadi_subsystem_check_function_pointer(dev_data->cb, false, true);
			skadi_cb_wrapper(dev, dev_data->cb_data, dev_data->cb);
		}
		else{
...
#ifdef CONFIG_SKADI_OS
	SKADI_GENERATE_IRQ_HANDLER_WRAPPER(uart_ns16550_isr)
...
#define UART_NS16550_IRQ_CONFIG(n)                                            \
	...
	{                                                                     \
		ARG_UNUSED(dev);                                              \
		LOG_INF("Registering UART interrupt handler %p!", SKADI_IRQ_HANDLER_FUNCTION_POINTER(n, uart_ns16550_isr));	\
		if(skadi_register_interrupt_handler(DT_INST_IRQN(n), NULL, SKADI_IRQ_HANDLER_FUNCTION_POINTER(n, uart_ns16550_isr)) == false){	\
			LOG_ERR("Could not register UART ISR handler!");																							\
		}																																		\
		LOG_INF("Registered UART interrupt handler!");	\
		skadi_irq_enable(DT_INST_IRQN(n), DT_INST_IRQ(n, priority));	\
	}
#else
#define UART_NS16550_IRQ_CONFIG(n)                                            \
	static void uart_ns16550_irq_config_func##n(const struct device *dev) \
...
#define UART_NS16550_DEVICE_IO_MMIO_INIT(n)                                          \
	...
	DEVICE_DT_INST_DEFINE(n, uart_ns16550_init, NULL,                            \
			      ...
			      PRE_KERNEL_1, CONFIG_SKADI_SERIAL_INIT_PRIORITY,             \
