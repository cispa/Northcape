...
#ifdef CONFIG_SKADI_LOADER
	skadi_sys_clock_announce(dticks);
#else
...
#ifdef CONFIG_SKADI_LOADER
	void skadi_clic_timer_isr(void){
		k_spinlock_key_t key = k_spin_lock(&lock);
...
#endif
	void skadi_trigger_immediate_timer_interrupt(void){
		set_mtimecmp(0);
	}
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, skadi_trigger_immediate_timer_interrupt, void)
		skadi_trigger_immediate_timer_interrupt();
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_trigger_immediate_timer_interrupt)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_WITHOUT_IRQ("j skadi_sys_clock_timer_isr_irq_restored\n\t", false, false, void, skadi_sys_clock_timer_isr)
		skadi_clic_timer_isr();
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_sys_clock_timer_isr)
#endif
...
#ifdef CONFIG_SKADI_LOADER
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_sys_clock_set_timeout, int32_t ticks, bool idle)
	sys_clock_set_timeout(ticks, idle);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sys_clock_set_timeout)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, __skadi_sys_clock_elapsed, void)
		return sys_clock_elapsed();
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sys_clock_elapsed)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, skadi_sys_clock_cycle_get_32, void)
		return sys_clock_cycle_get_32();
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_sys_clock_cycle_get_32)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint64_t, skadi_sys_clock_cycle_get_64, void)
		return sys_clock_cycle_get_64();
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_sys_clock_cycle_get_64)
#endif
...
__boot_func static int thread_init_trampolines(void){
	...
	init_ok &= skadi_sys_clock_cycle_get_32_register_init_function();
	init_ok &= skadi_sys_clock_cycle_get_64_register_init_function();
	init_ok &= skadi_trigger_immediate_timer_interrupt_register_init_function();
...
#ifndef CONFIG_SKADI_LOADER
	/* in skadi, we will enable the IRQ in due time using the trampolines */
