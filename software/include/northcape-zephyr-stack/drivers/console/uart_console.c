...
static int console_out(int c)
{
	...
	if ('\n' == c) {
		...
		skadi_uart_poll_out(uart_console_dev, '\r');
#else
		uart_poll_out(uart_console_dev, '\r');
#endif
	}
#ifdef SKADI_SUBSYSTEM
	skadi_uart_poll_out(uart_console_dev, c);
#else
...
#ifdef SKADI_SUBSYSTEM
	rx = skadi_uart_fifo_read(uart, buf, size);
#else
...
#ifdef SKADI_SUBSYSTEM
		skadi_uart_irq_rx_disable(uart);
#else
...
static void insert_char(char *pos, char c, uint8_t end)
{
	...
	*(pos++) = c;
	...
	while (end-- > 0) {
#ifdef SKADI_SUBSYSTEM
		skadi_uart_poll_out(uart_console_dev, tmp);
#else
...
{
#ifdef SKADI_SUBSYSTEM
	skadi_uart_poll_out(uart_console_dev, '\b');
#else
	uart_poll_out(uart_console_dev, '\b');
#endif
	if (end == 0U) {
#ifdef SKADI_SUBSYSTEM
		skadi_uart_poll_out(uart_console_dev, ' ');
		skadi_uart_poll_out(uart_console_dev, '\b');
#else
	...
	while (end-- > 0) {
		*pos = *(pos + 1);
#ifdef SKADI_SUBSYSTEM
		skadi_uart_poll_out(uart_console_dev, *(pos++));
#else
...
static bool handle_mcumgr(struct console_input *cmd, uint8_t byte)
{
	...
	if (mcumgr_state == CONSOLE_MCUMGR_STATE_PAYLOAD && byte == '\n') {
		...
		cmd->is_mcumgr = 1;
#ifdef SKADI_SUBSYSTEM
		skadi_fifo_put(lines_queue, cmd);
#else
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, uart_console_isr, const struct device *unused, void *user_data)
#else
...
#endif
{
...
#ifdef SKADI_SUBSYSTEM
	while (skadi_uart_irq_update(uart_console_dev) > 0 &&
		   skadi_uart_irq_is_pending(uart_console_dev) > 0) {
#else
	while (uart_irq_update(uart_console_dev) > 0 &&
...
#ifdef SKADI_SUBSYSTEM
		rx = skadi_uart_irq_rx_ready(uart_console_dev);
#else
		...
		if (!cmd) {
#ifdef SKADI_SUBSYSTEM
			cmd = skadi_fifo_get(avail_queue, K_NO_WAIT);
#else
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(uart_console_isr)
#endif
...
#ifdef SKADI_SUBSYSTEM
	skadi_uart_irq_rx_disable(uart_console_dev);
	skadi_uart_irq_tx_disable(uart_console_dev);

	skadi_uart_irq_callback_set(uart_console_dev, SKADI_SUBSYSTEM_FUNCTION_POINTER(uart_console_isr));
	
	/* Drain the fifo */
	while (skadi_uart_irq_rx_ready(uart_console_dev) > 0) {
		skadi_uart_fifo_read(uart_console_dev, &c, 1);
	}

	skadi_uart_irq_rx_enable(uart_console_dev);
#else
...
#if defined(CONFIG_SKADI_OS) && defined(SKADI_SUBSYSTEM)
/* this is the setup code for the UART subsystem */

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_console_out, int c)
	return console_out(c);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_console_out)

SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_VOID(__skadi_stdout_hook_install, int(*hook_token)(int));

static int uart_console_init(void){
	int (*hook_token)(int) = SKADI_SUBSYSTEM_FUNCTION_POINTER(__skadi_console_out);

	__ASSERT_NO_MSG(hook_token);

	__skadi_stdout_hook_install(hook_token);
...
#ifndef SKADI_SUBSYSTEM
/* this is the setup code for the loader's initial UART */
/**
