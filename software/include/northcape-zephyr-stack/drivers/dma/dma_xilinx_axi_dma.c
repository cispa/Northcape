...
static int dma_xilinx_axi_dma_clean_up_sg_descriptors(
	const struct device *dev, struct dma_xilinx_axi_dma_channel *channel_data, const char *chan_name)
{
	...
	__ASSERT_NO_MSG(current_descriptor);
	...
	while (current_descriptor->status & XILINX_AXI_DMA_SG_DESCRIPTOR_STATUS_COMPLETE_MASK ||
		...
		if (dma_xilinx_axi_dma_channel_has_error(channel_data, current_descriptor)) {
			...
			__ASSERT_NO_MSG(current_descriptor->status & XILINX_AXI_DMA_SG_DESCRIPTOR_STATUS_COMPLETE_MASK);
		}
						    ...
						    bool is_first, bool is_last)
{
	...
	__ASSERT_NO_MSG(!current_descriptor->control);
	...
	new_control = (uint32_t)block_size;

	if (is_first) {
		new_control = new_control | XILINX_AXI_DMA_SG_DESCRIPTOR_CTRL_SOF_MASK;
	}
	if (is_last) {
		new_control = new_control | XILINX_AXI_DMA_SG_DESCRIPTOR_CTRL_EOF_MASK;
	}

	current_descriptor->control = new_control;
...
#pragma GCC diagnostic pop
	...
	if(CONFIG_DMA_XILINX_AXI_DMA_POLL_INTERVAL){
