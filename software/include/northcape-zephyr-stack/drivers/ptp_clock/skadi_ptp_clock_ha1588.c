 ...
 *  gGmbH
 *
 * Based on ha1588 Linux driver Copyright (C) 2018 Geon Technologies, LLC.
 *
 ...
 */
 ...
 static inline int __ptp_clock_ha1588_set_period(const struct device *dev, uint64_t period){
	...
	LOG_DBG("Setting ha1588 period to %"PRIu32" ns %"PRIu32" fractional ns!", period_ns, period_fractional_ns);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_ptp_clock_ha1588_set_period, const struct device *dev, uint64_t period)
{
	return __ptp_clock_ha1588_set_period(skadi_get_own_device_representation(dev), period);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_clock_ha1588_set_period)

static int ptp_clock_ha1588_set_unlocked(const struct device *dev,
...
static int __ptp_clock_ha1588_set(const struct device *dev, struct net_ptp_time *tm){
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_ptp_clock_ha1588_set, const struct device *dev, struct net_ptp_time *tm)
{
	return __ptp_clock_ha1588_set(skadi_get_own_device_representation(dev), tm);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_clock_ha1588_set)

static int ptp_clock_ha1588_get_unlocked(const struct device *dev,
	struct net_ptp_time *tm)
{
	const struct ptp_clock_ha1588_config *config = dev->config;
...
static int __ptp_clock_ha1588_get(const struct device *dev, struct net_ptp_time *tm){
	struct ptp_clock_ha1588_data *data = dev->data;
	int ret;
	k_spinlock_key_t key = k_spin_lock(&data->lock);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_ptp_clock_ha1588_get, const struct device *dev, struct net_ptp_time *tm)
{
	return __ptp_clock_ha1588_get(skadi_get_own_device_representation(dev), tm);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_clock_ha1588_get)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_ptp_clock_ha1588_adjust, const struct device *or_dev, int increment)
{
	const struct device *dev = skadi_get_own_device_representation(or_dev);
	struct ptp_clock_ha1588_data *data = dev->data;
	struct net_ptp_time tm;
	...
	ret = __ptp_clock_ha1588_get(skadi_get_own_device_representation(dev), &tm);
	...
	ret = __ptp_clock_ha1588_set(skadi_get_own_device_representation(dev), &tm);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_clock_ha1588_adjust)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_ptp_clock_ha1588_rate_adjust, const struct device *or_dev, double ratio)
{
	const struct device *dev = skadi_get_own_device_representation(or_dev);
	const struct ptp_clock_ha1588_config *config = dev->config;
	struct ptp_clock_ha1588_data *data = dev->data;
	...
	ret = __ptp_clock_ha1588_set_period(dev, (uint64_t) new_period);

	k_spin_unlock(&data->lock, key);

	return ret;
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_clock_ha1588_rate_adjust)
 
 #ifdef CONFIG_PTP_CLOCK_HA1588_TSU
 
	 static int ptp_tsu_ha1588_get_tstamp_unlocked(const struct device *dev, struct ha1588_tsu_timestamp *tstamp, bool is_rx){
		 const struct ptp_clock_ha1588_config *config = dev->config;
		 ...
		 uint64_t secval;
		 bool timestamp_everything_enabled = is_rx ? data->timestamp_everything_enabled_rx : data->timestamp_everything_enabled_tx;
		  ...
		  */
		 readval = sys_read32(config->reg_addr + (is_rx ? HA1588_TSU_RXQ_FILTER : HA1588_TSU_TXQ_FILTER));
		 ...
		 if(!num_avail){
			 LOG_DBG("Could not get %s timestamp from ha1588!", is_rx ? "rx" : "tx");
			 return -EIO;
		 }
 
		 sys_write32(timestamp_everything_enabled ? HA1588_TSU_TIMESTAMP_ALL : HA1588_TSU_RESET_CTRL, config->reg_addr + (is_rx ? HA1588_TSU_RXQ_CTRL : HA1588_TSU_TXQ_CTRL));
		 sys_write32(timestamp_everything_enabled ? HA1588_TSU_TIMESTAMP_ALL | HA1588_TSU_READ : HA1588_TSU_READ, config->reg_addr + (is_rx ? HA1588_TSU_RXQ_CTRL : HA1588_TSU_TXQ_CTRL));
 
		 do {
			 readval = sys_read32(config->reg_addr + (is_rx ? HA1588_TSU_RXQ_CTRL : HA1588_TSU_TXQ_CTRL));
		 ...
		 LOG_DBG("Got %s timestamp %"PRIu64".%"PRIu32" with sequence ID %"PRIu16" checksum %"
			 PRIu16 " message id %"PRIu16, is_rx ? "rx" : "tx", secval, tsu_out_2,
			 tstamp->ptp_seqid, tstamp->ptp_checksum, tstamp->ptp_msgid);
		 
		 LOG_DBG("Got TSU raw values %"PRIx32" %"PRIx32" %"PRIx32" %"PRIx32, 
			 tsu_out_3, tsu_out_2, tsu_out_1, tsu_out_0);
 
 
		 return ret;
	 }
 
	 int ptp_tsu_ha1588_get_rx_tstamp(const struct device *dev, 
		 ...
		 ret = ptp_tsu_ha1588_get_tstamp_unlocked(skadi_get_own_device_representation(dev), tstamp, true);
	 ...
	 int ptp_tsu_ha1588_get_tx_tstamp(const struct device *dev, 
		 struct ha1588_tsu_timestamp *tstamp){
		 struct ptp_clock_ha1588_data *data = dev->data;
		 int ret;
	 ...
	 int ptp_tsu_ha1588_set_timestamp_all_rx(const struct device *dev,
		 bool enable){
		 const struct ptp_clock_ha1588_config *config = dev->config;
		 struct ptp_clock_ha1588_data *data = dev->data;
 
		 if(config->has_timestamp_everything_option){
			 sys_write32(enable ? HA1588_TSU_TIMESTAMP_ALL : HA1588_TSU_RESET_CTRL, config->reg_addr + HA1588_TSU_RXQ_CTRL);
	 ...
	 int ptp_tsu_ha1588_set_timestamp_all_tx(const struct device *dev,
		 bool enable){
		 const struct ptp_clock_ha1588_config *config = dev->config;
		 struct ptp_clock_ha1588_data *data = dev->data;
 
		 if(config->has_timestamp_everything_option){
			 sys_write32(enable ? HA1588_TSU_TIMESTAMP_ALL : HA1588_TSU_RESET_CTRL, config->reg_addr + HA1588_TSU_TXQ_CTRL);
	 ...
	 bool ptp_tsu_ha1588_packet_matches_rx_filter(struct net_pkt *pkt){
		 ...
		 ret = skadi_net_pkt_read(pkt, &eth_hdr, sizeof(eth_hdr));
		 ...
		 if(ntohs(eth_hdr.type) == NET_ETH_PTYPE_IP || ntohs(eth_hdr.type) == NET_ETH_PTYPE_IPV6){
			 ...
			 if(is_ipv6){
				 /* IPV6 header */
				 ret = skadi_net_pkt_read(pkt, &ip6_hdr, sizeof(ip6_hdr));
				 ...
				 /* IPv4 header */
				 ret = skadi_net_pkt_read(pkt, &ip_hdr, sizeof(ip_hdr));
			 ...
			 ret = skadi_net_pkt_read(pkt, &udp_header, sizeof(udp_header));
			 ...
			 if(!(ntohs(udp_header.dst_port) == PTP_SOCKET_PORT_EVENT || ntohs(udp_header.dst_port) == PTP_SOCKET_PORT_GENERAL)){
				 goto out_false;
			 }
 
			 /* one final check: message ID in PTP header must match */
			 ret = skadi_net_pkt_read(pkt, &first_word, sizeof(first_word));
 ...
 SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_ptp_tsu_ha1588_get_rx_tstamp, const struct device *dev, struct ha1588_tsu_timestamp *tstamp)
 	 return ptp_tsu_ha1588_get_rx_tstamp(skadi_get_own_device_representation(dev), tstamp);
 SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_tsu_ha1588_get_rx_tstamp)

 SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_ptp_tsu_ha1588_get_tx_tstamp, const struct device *dev, struct ha1588_tsu_timestamp *tstamp)
	 return ptp_tsu_ha1588_get_tx_tstamp(skadi_get_own_device_representation(dev), tstamp);
 SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_tsu_ha1588_get_tx_tstamp)

 SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_ptp_tsu_ha1588_set_timestamp_all_rx, const struct device *dev, bool enable)
	 return ptp_tsu_ha1588_set_timestamp_all_rx(skadi_get_own_device_representation(dev), enable);
 SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_tsu_ha1588_set_timestamp_all_rx)

 SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_ptp_tsu_ha1588_set_timestamp_all_tx, const struct device *dev, bool enable)
	 return ptp_tsu_ha1588_set_timestamp_all_tx(skadi_get_own_device_representation(dev), enable);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_tsu_ha1588_set_timestamp_all_tx)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(bool, __skadi_ptp_tsu_ha1588_packet_matches_rx_filter, struct net_pkt *pkt)
	 return ptp_tsu_ha1588_packet_matches_rx_filter(pkt);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_tsu_ha1588_packet_matches_rx_filter)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_ptp_tsu_ha1588_reset, const struct device *dev)
	 ptp_tsu_ha1588_reset(skadi_get_own_device_representation(dev));
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_ptp_tsu_ha1588_reset)
 ...
 static int ptp_clock_ha1588_init(const struct device *dev){
	 ...
	 ret = __ptp_clock_ha1588_set_period(dev, config->rtc_period);
 
	 return ret ? ret : ptp_tsu_ha1588_init(dev);
 }
 
 static struct ptp_clock_driver_api ptp_clock_ha1588_api = {
	 0
 };

 static int ptp_clock_ha1588_init_api(const struct device *dev)
{
	/* this is called at initialization time - the device is still mutable */
	struct device *dev_mut = (struct device *) dev;
	void* ptp_clock_driver_api= NULL;
	skadi_restriction_t restriction = SKADI_NO_RESTRICTION;

	ptp_clock_ha1588_api.set = SKADI_SUBSYSTEM_FUNCTION_POINTER(__skadi_ptp_clock_ha1588_set);
	ptp_clock_ha1588_api.get = SKADI_SUBSYSTEM_FUNCTION_POINTER(__skadi_ptp_clock_ha1588_get);
	ptp_clock_ha1588_api.adjust = SKADI_SUBSYSTEM_FUNCTION_POINTER(__skadi_ptp_clock_ha1588_adjust);
	ptp_clock_ha1588_api.rate_adjust = SKADI_SUBSYSTEM_FUNCTION_POINTER(__skadi_ptp_clock_ha1588_rate_adjust);
	

	if(skadi_cap_ops_derive(&ptp_clock_ha1588_api, restriction, sizeof(ptp_clock_ha1588_api), skadi_get_capability_offset(&ptp_clock_ha1588_api), SKADI_PERMISSION_READ | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS, &ptp_clock_driver_api) == false || ptp_clock_driver_api == 0){
	...
	dev_mut->api = (void *) ptp_clock_driver_api;

	return 0;
}

static bool skadi_ha1588_init(void){
	LOG_INF("Initializing ha1588 API!");

	SKADI_DEVICES_INIT(ptp_clock_ha1588_init_api);

	LOG_INF("Ha1588 API complete!");

	return true;
}
/* we need to substitute the device API in before write permissions are dropped, so we need to do this during init */
SKADI_SUBSYSTEM_INIT_FUNCTIONS(skadi_ha1588_init)


 
 #define PTP_CLOCK_HA1588_INIT(n)						\
										 \
	 static const struct ptp_clock_ha1588_config				\
		 ptp_clock_ha1588_##n##_config = {				\
			 .reg_addr = DT_INST_REG_ADDR(n),			\
			 .rtc_period = ((uint64_t) DT_INST_PROP_BY_IDX(n, ha1588_period, 0)) << 32 |		\
				 ((uint64_t) DT_INST_PROP_BY_IDX(n, ha1588_period, 1)),\
			 .has_timestamp_everything_option = DT_INST_PROP_OR(n, ha1588_ts_all_supported, false) \
		 };								\
										 \
	 static struct ptp_clock_ha1588_data ptp_clock_ha1588_##n##_data;	\
										 \
	 DEVICE_DT_INST_DEFINE(n, &ptp_clock_ha1588_init, NULL,			\
				 &ptp_clock_ha1588_##n##_data,			\
				 &ptp_clock_ha1588_##n##_config,			\
				 POST_KERNEL, CONFIG_PTP_CLOCK_INIT_PRIORITY,	\
				 &ptp_clock_ha1588_api);
