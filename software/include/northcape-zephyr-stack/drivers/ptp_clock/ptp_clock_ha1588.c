...
static int ptp_clock_ha1588_set(const struct device *dev,
	struct net_ptp_time *tm){
...
static int ptp_clock_ha1588_get(const struct device *dev,
	struct net_ptp_time *tm){
	struct ptp_clock_ha1588_data *data = dev->data;
	int ret;
	k_spinlock_key_t key = k_spin_lock(&data->lock);
...
static int ptp_clock_ha1588_adjust(const struct device *dev,
	int increment){
	struct ptp_clock_ha1588_data *data = dev->data;
	struct net_ptp_time tm;
...
static int ptp_clock_ha1588_rate_adjust(const struct device *dev,
	double ratio){
	const struct ptp_clock_ha1588_config *config = dev->config;
	struct ptp_clock_ha1588_data *data = dev->data;
	...
	static int ptp_tsu_ha1588_get_tstamp_unlocked(const struct device *dev, struct ha1588_tsu_timestamp *tstamp, bool is_rx){
		...
		LOG_DBG("Num avail: %"PRIu8, num_avail);
