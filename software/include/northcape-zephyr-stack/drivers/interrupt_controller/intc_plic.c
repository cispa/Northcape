...
LOG_MODULE_REGISTER(skadi_plic_subsystem, CONFIG_SKADI_SUBSYSTEM_LOG_LEVEL);
...
static const struct device *save_dev;
	...
	void z_irq_spurious(const void *unused){
		ARG_UNUSED(unused);
		LOG_ERR("Spurious IRQ: %d", riscv_plic_get_irq());
	}
#endif
 ...
 */
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, riscv_plic_irq_enable, uint32_t irq)
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(riscv_plic_irq_enable)
#endif
 ...
 */
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, riscv_plic_irq_disable, uint32_t irq)
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(riscv_plic_irq_disable)
#endif
 ...
 */
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, riscv_plic_irq_is_enabled, uint32_t irq)
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(riscv_plic_irq_is_enabled)
#endif
 ...
 */
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, riscv_plic_set_priority, uint32_t irq, uint32_t priority)
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(riscv_plic_set_priority)
#endif
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_ARGS(isr_stub, const void *arg);
#endif
...
static void plic_irq_handler(const struct device *dev)
{
	...
	if (local_irq == 0U || local_irq >= config->num_irqs) {
		z_irq_spurious(NULL);
		/* TODO something is wrong with the handling of edge-triggered interrupts... */
		return;
	}
...
#ifdef CONFIG_SKADI_OS
	LOG_DBG("Jumping into ISR handler from PLIC!");
	/* need to do a subsystem call, as this is a different task */
	skadi_subsystem_check_function_pointer(ite->isr, true, false);
	isr_stub(ite->arg, ite->isr);
	LOG_DBG("Returned from ISR handler to PLIC!");
#else
	ite->isr(ite->arg);
#endif
...
#ifdef CONFIG_SKADI_OS
	SKADI_GENERATE_IRQ_HANDLER_WRAPPER(plic_irq_handler)	
#endif
...
#define PLIC_INTC_IRQ_FUNC_DEFINE(n)                                                               \
	...
	{                                                                                          \
		...
		if(skadi_register_interrupt_handler(DT_INST_IRQN(n), NULL, SKADI_IRQ_HANDLER_FUNCTION_POINTER(n,plic_irq_handler)) == false){	\
			LOG_ERR("Could not register ISR handler!");																							\
		}																																		\
		LOG_INF("Registered interrupt handler - enabling interrupt!");	\
		skadi_irq_enable(DT_INST_IRQN(n), SKADI_IRQ_PRIORITY_DEFAULT); \
	}
#endif
...
#ifdef CONFIG_SKADI_OS
/* in Skadi, we need to maintain the interrupts locally */
#define PLIC_INTC_CONFIG_IRQ_TABLE(n) \
	struct _isr_table_entry __sw_isr_table plic_irq_table##n[DT_INST_PROP(n, riscv_ndev)] = {0}

#define PLIC_INTC_CONFIG_INIT(n)                                                                   \
	PLIC_INTC_IRQ_FUNC_DECLARE(n);                                                             \
	PLIC_HART_CONTEXT_DECLARE(n);                                                              \
	PLIC_INTC_CONFIG_IRQ_TABLE(n);																\
	static struct _isr_table_entry *last_plic_irq_table = plic_irq_table##n; /* we can assume there is only up to one PLIC in the system */ \
	static uint32_t max_irq = DT_INST_PROP(n,riscv_ndev) - 1;	\
	static const struct plic_config plic_config_##n = {                                        \
		...
		.isr_table = plic_irq_table##n, /* locally managed table */                          \
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(bool, skadi_plic_register_handler, const int irq_number, const void *arg, void (*isr)(const void *arg))
{
	const uint32_t local_irq = irq_from_level_2(irq_number);

	if(local_irq > max_irq){
		LOG_ERR("Local IRQ out of range!");
		return false;
	}
	last_plic_irq_table[local_irq].arg = arg;
	last_plic_irq_table[local_irq].isr = isr;

	return true;
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_plic_register_handler)
