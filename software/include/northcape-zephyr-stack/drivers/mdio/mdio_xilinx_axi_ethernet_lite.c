...
static inline int mdio_axi_eth_lite_complete_transaction(const struct 
							 mdio_axi_eth_lite_config *config)
{
	int waited_cycles = 0;
	...
	while (mdio_axi_eth_lite_check_busy(config) && waited_cycles < MDIO_MAX_WAIT_US) {
			     ...
			     uint16_t *value)
{
	...
	if (mdio_axi_eth_lite_check_busy(config)) {
	...
	if (mdio_axi_eth_lite_complete_transaction(config)) {
		(void)k_mutex_unlock(&data->mutex);
		return -EIO;
	}
...
static DEVICE_API(mdio, mdio_axi_eth_lite_api) = {.read = mdio_axi_eth_lite_read,
