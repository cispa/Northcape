...
{
	volatile mem_addr_t reg_addr = (mem_addr_t) (((uint8_t *)__skadi_mmio_0_1086324736_262144) + reg_offset); /*(uint32_t *)((uint8_t *)(config->reg) + reg_offset);*/
...
{
	const volatile mem_addr_t reg_addr = (mem_addr_t) (((uint8_t *)__skadi_mmio_0_1086324736_262144) + reg_offset); /*(uint32_t *)((uint8_t *)(config->reg) + reg_offset);*/
	const uint32_t ret = sys_read32(reg_addr);

	barrier_dmem_fence_full(); /* make sure that read commits */
	return ret;
}

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, mdio_xilinx_axienet_bus_disable, const struct device *orig_dev)
{
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(mdio_xilinx_axienet_bus_disable)

#define MAX_MDIO_WAIT 20
		...
		struct mdio_xilinx_axienet_data *data)
{
	...
	& XILINX_AXIENET_MDIO_SETUP_REG_MDIO_ENABLE_MASK) == 0
		...
		if (config->have_irq) {
			...
			err = skadi_sem_take(data->irq_sema,
		...
		while ((xilinx_axienet_read_mdio_register(config,
				XILINX_AXIENET_MDIO_SETUP_REG_OFFSET)
				& XILINX_AXIENET_MDIO_SETUP_REG_MDIO_ENABLE_MASK) == 0 && waited < MAX_MDIO_WAIT) {
			LOG_DBG("Waiting for bus enable!");
			waited ++;
			/* TODO there is something wrong with skadi_usleep */
			k_busy_wait(1);
		}

		if(waited == MAX_MDIO_WAIT){
			LOG_ERR("Timed out waiting for bus enable!");
		}
		else{
...
static int wait_bus_ready(const struct mdio_xilinx_axienet_config *config, const struct mdio_xilinx_axienet_data *data){
	int waited = 0;
	...
	while ((xilinx_axienet_read_mdio_register(config,
		...
		& XILINX_AXIENET_MDIO_CONTROL_REG_MASK_READY) == 0x0 && waited < MAX_MDIO_WAIT) {
	...
	if(waited == MAX_MDIO_WAIT){
		LOG_ERR("Timed out waiting for MDIO read");
		return -EIO;
	}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, mdio_xilinx_axienet_bus_enable, const struct device *orig_dev)
{
	const struct device *dev = skadi_get_own_device_representation(orig_dev);
	const struct mdio_xilinx_axienet_config *config = dev->config;
	struct mdio_xilinx_axienet_data *data = dev->data;
	...
	clock_divider_full = DIV_ROUND_UP(config->clock_frequency_hz, XILINX_AXIENET_MDIO_MDIO_TARGET_FREQUENCY_HZ * 2);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(mdio_xilinx_axienet_bus_enable)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, mdio_xilinx_axienet_read, const struct device *orig_dev, uint8_t prtad, uint8_t devad,
			uint16_t *data)
{
	const struct device *dev = skadi_get_own_device_representation(orig_dev);
	const struct mdio_xilinx_axienet_config *config = dev->config;
	...
	(void)skadi_mutex_lock(&dev_data->mutex, K_FOREVER);

	if(wait_bus_ready(config, dev_data) < 0){
		(void)skadi_mutex_unlock(&dev_data->mutex);
		return -EIO;
	}
	...
	if (config->have_irq) {
		(void)skadi_mutex_unlock(&dev_data->mutex);
		err = skadi_sem_take(dev_data->irq_sema,
			K_MSEC(XILINX_AXIENET_MDIO_INTERRUPT_TIMEOUT_MS));
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(mdio_xilinx_axienet_read)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, mdio_xilinx_axienet_write, const struct device *orig_dev, uint8_t prtad, uint8_t devad,
			 uint16_t data)
{
	const struct device *dev = skadi_get_own_device_representation(orig_dev);
	const struct mdio_xilinx_axienet_config *config = dev->config;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(mdio_xilinx_axienet_write)

#ifdef CONFIG_SKADI_OS
static struct mdio_driver_api mdio_xilinx_axienet_api = {
	0
};
#endif
...
static bool mdio_xilinx_axienet_skadi_init(void){
	...
	mdio_xilinx_axienet_api.bus_disable = SKADI_SUBSYSTEM_FUNCTION_POINTER(mdio_xilinx_axienet_bus_disable);
	mdio_xilinx_axienet_api.bus_enable = SKADI_SUBSYSTEM_FUNCTION_POINTER(mdio_xilinx_axienet_bus_enable);
	mdio_xilinx_axienet_api.read = SKADI_SUBSYSTEM_FUNCTION_POINTER(mdio_xilinx_axienet_read);
	mdio_xilinx_axienet_api.write = SKADI_SUBSYSTEM_FUNCTION_POINTER(mdio_xilinx_axienet_write);

	SKADI_DEVICES_API_INIT(mdio_xilinx_axienet_api)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, mdio_xilinx_axienet_probe, const struct device *orig_dev)
{
	const struct device *dev = skadi_get_own_device_representation(orig_dev);
	const struct mdio_xilinx_axienet_config *config = dev->config;
	struct mdio_xilinx_axienet_data *data = dev->data;
	...
	if (config->have_irq) {
		data->irq_sema = skadi_allocator_alloc_rw(sizeof(*data->irq_sema));

		if(!data->irq_sema){
			return -ENOMEM;
		}

		err = skadi_sem_init(data->irq_sema, 0, K_SEM_MAX_LIMIT);
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(mdio_xilinx_axienet_probe)
...
{								\
	...
	DEVICE_DT_INST_DEFINE(inst, mdio_xilinx_axienet_probe,  \
		  NULL,					\
		  &mdio_xilinx_axienet_data##inst,		\
		  &mdio_xilinx_axienet_config##inst,		\
		  POST_KERNEL,				\
