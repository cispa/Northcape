...
struct net_buf {
	...
	const struct net_buf_data_alloc *data_alloc;
	uint32_t derive_len;
	uint32_t derive_offset;
...
#ifdef CONFIG_SKADI_OS
	/* we need a writable reference here */
...
#ifdef CONFIG_SKADI_NET_ZEROCOPY
			(void)pool;
			buf->data_alloc->cb->unref(buf, buf->__buf);
#else
