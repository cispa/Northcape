...
BUILD_ASSERT(sizeof(struct llext_const_symbol) == IS_ENABLED(CONFIG_SKADI_OS) ? 3 * sizeof(uintptr_t) : 2 * sizeof(uintptr_t));
...
#define EXPORT_SYMBOL(x)																\
	...
	static const STRUCT_SECTION_ITERABLE(llext_const_symbol, CONCAT(x, _sym)) = {		\
		.name = x ## _sym_name, .addr = (const void *)&x, .size = sizeof(x)				\
	};																					\
	_Pragma("GCC diagnostic pop")
#else
#define EXPORT_SYMBOL(x)																\
...
#define EXPORT_SYMBOL(x)																\
	...
	_Pragma("GCC diagnostic ignored \"-Wpointer-arith\"") 								\
	static const STRUCT_SECTION_ITERABLE(llext_const_symbol, CONCAT(x, _sym)) = {		\
		.name = STRINGIFY(x), .addr = (const void *)&x,	.size = sizeof(x)				\
	};																					\
	_Pragma("GCC diagnostic pop")
#else
#define EXPORT_SYMBOL(x)																\
