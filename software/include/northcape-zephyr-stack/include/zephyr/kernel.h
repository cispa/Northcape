 ...
 */
/**
 * Part of the mutex group readable by non-scheduler subsystems.
 * Optimization for is_locked, is_last_unlocked APIs.
 * @ingroup mutex_apis
 */
struct mutex_owner_lock_count{
	k_tid_t owner_id;
	const struct mutex_owner_lock_count  *sched_mutex;
	uint32_t lock_count;
};
...
#ifdef CONFIG_SKADI_OS
	k_tid_t owner_id;
	/** read-only capability for owner and lock count - used to check lock status */
	const struct mutex_owner_lock_count  *sched_mutex;
#endif
	/** Current lock count */
	uint32_t lock_count;
...
#ifdef CONFIG_SKADI_OS
	/* used to forward some parameter to the handler function */
	void *user_data;
#endif
...
#define Z_HEAP_DEFINE_IN_SECT(name, bytes, in_section)		\
	char in_section						\
	     __aligned(CONFIG_DCACHE_LINE_SIZE) /* CHUNK_UNIT */			\
