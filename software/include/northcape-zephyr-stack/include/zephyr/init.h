...
struct init_entry {
	...
	/**
	 * @brief Initialization priority for run-time merging of subsystem / loader init functions
	 */
	int priority;
#endif
};
...
#define SYS_INIT_NAMED(name, init_fn_, level, prio)                                       \
	static const Z_DECL_ALIGN(struct init_entry)                                      \
		...
		Z_INIT_ENTRY_NAME(name) = {.init_fn = {.sys = (init_fn_)},                \
			Z_INIT_SYS_INIT_DEV_NULL, COND_CODE_1(CONFIG_SKADI_OS, (.priority=prio), ())}
