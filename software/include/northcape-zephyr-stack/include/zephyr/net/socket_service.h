...
struct net_socket_service_desc {
	...
	/* 
	 * we are collecting services from different subsystems, 
	 * i.e., STRUCT_SECTION_FOREACH does not cut it
	 */
	sys_dnode_t list_node;
	...
	int *idx;
#ifdef CONFIG_SKADI_OS
	/** Have the fields been replaced with skadi tokens? */
	bool is_initialized;
#endif
};
...
#define __z_net_socket_service_define(_name, _work_q, _cb, _count, ...) \
	...
	STRUCT_SECTION_ITERABLE(net_socket_service_desc, _name) = { \
 ...
 */
__syscall int net_socket_service_register(struct net_socket_service_desc *service,
 ...
 * @retval -ENINVAL Invalid parameter.
 */
static inline int net_socket_service_unregister(struct net_socket_service_desc *service)
{
