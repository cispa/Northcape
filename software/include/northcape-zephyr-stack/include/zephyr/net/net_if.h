...
struct net_if {
	...
	/** Non-task-id-restricted version of the token */
	struct net_if *net_if_token;
	/** Version of the interface used by the networking subsystem */
	struct net_if *net_if_or;
#endif
	...
	/** Event that is signalled when a net_if_writer unlocks */
	struct k_event *writer_unlock_event;


	/** cache for TX checksum support */
	long rx_checksum_support_cache;
	long tx_checksum_support_cache;

	/** reader-writer lock for net_if */
	atomic_t net_if_readers;

	uint8_t rx_checksum_cache_set: 1;
	uint8_t tx_checksum_cache_set: 1;
#else
	/** Mutex protecting this network interface instance */
	struct k_mutex lock;
...
static inline void _net_if_lock_read(struct net_if *iface, const char *file, const int line){
	/* debug */
	ARG_UNUSED(file);
	ARG_UNUSED(line);
	...
	while(true){
		atomic_inc(&iface->net_if_readers);

		if(!iface->lock || !skadi_mutex_is_locked(iface->lock)){
			/* success - was not locked! */
			break;
		}

		atomic_dec(&iface->net_if_readers);

		skadi_subsystem_yield();

		(void)skadi_event_wait(iface->writer_unlock_event, 0x1, false, K_FOREVER);
	}
}
#define net_if_lock_read(IFACE) _net_if_lock_read(IFACE, __FILE__, __LINE__)

static inline void _net_if_unlock_read(struct net_if *iface, const char *file, const int line){
	/* debug */
	ARG_UNUSED(file);
	ARG_UNUSED(line);
...
#define net_if_unlock_read(IFACE) _net_if_unlock_read(IFACE, __FILE__, __LINE__)


static inline void _net_if_lock_write(struct net_if *iface, const char *file, const int line){
	/* debug */
	ARG_UNUSED(file);
	ARG_UNUSED(line);
	...
	(void)skadi_event_clear(iface->writer_unlock_event, 0x1);

	(void)skadi_mutex_lock(iface->lock, K_FOREVER);

	while(atomic_get(&iface->net_if_readers)){
		/* wait for readers to finish, if any - readers see that the mutex is locked and do not attempt to lock it*/
		skadi_subsystem_yield();
	}
}
#define net_if_lock_write(IFACE) _net_if_lock_write(IFACE, __FILE__, __LINE__)


#define net_if_lock net_if_lock_write

static inline void _net_if_unlock_write(struct net_if *iface, const char *file, const int line){
	bool last_unlock;
	...
	last_unlock = skadi_mutex_is_last_lock(iface->lock);

	skadi_mutex_unlock(iface->lock);

	if(last_unlock){
		(void)skadi_event_post(iface->writer_unlock_event, 0x1);
	}
}

#define net_if_unlock_write(IFACE) _net_if_unlock_write(IFACE, __FILE__, __LINE__)

#define net_if_unlock net_if_unlock_write
...
static inline void net_if_tx_lock(struct net_if *iface)
{
	...
	(void)skadi_mutex_lock(iface->tx_lock, K_FOREVER);
}
...
static inline void net_if_tx_unlock(struct net_if *iface)
{
	...
	skadi_mutex_unlock(iface->tx_lock);
}
#else
...
#define net_if_lock_write net_if_lock
#define net_if_lock_read  net_if_lock

#define net_if_unlock_write net_if_unlock
#define net_if_unlock_read  net_if_unlock
#endif
/** @endcond */
