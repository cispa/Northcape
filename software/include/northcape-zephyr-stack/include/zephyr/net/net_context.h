...
__net_socket struct net_context {
	...
	struct {
		...
		bool is_0copy_socket;
		
		void (*tx_buffer_completion_callback)(void *buffer);
		void (*rx_buffer_completion_callback)(void *buffer);
#endif
#if defined CONFIG_NET_CONTEXT_TIMESTAMP_SOCKOPT
		struct net_ptp_time last_txtimestamp;
		bool last_txtimestamp_valid;
#endif
	} options;
...
#if defined(CONFIG_SKADI_NET_ZEROCOPY)
	struct net_buf_data_alloc zerocopy_pool_alloc;
	sys_slist_t zerocopy_recv_queue;
#endif
...
enum net_context_option {
	...
	NET_OPT_ZEROCOPY		  = 19, /**< Zero-copy socket */
	NET_OPT_ZEROCOPY_RX_CB	  = 20, /**< Zero-copy socket RX callback */
	NET_OPT_ZEROCOPY_TX_CB	  = 21, /**< Zero-copy socket TX callback */
	NET_OPT_LAST_TXTIME		  = 22 /** < Last TX time on the socket */
};
...
/**
 * @brief Go over context and free any packets in 0-copy buffer.
 */
