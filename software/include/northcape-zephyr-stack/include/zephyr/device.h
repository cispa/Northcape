...
/* in case of Skadi loader, we do not want to export devices, as the loader will go away... */
#if defined(CONFIG_LLEXT_EXPORT_DEVICES) && (!defined(CONFIG_SKADI_LOADER) || defined(SKADI_SUBSYSTEM))
...
/**
 * @brief Variant of @ref DEVICE_DT_DEFINE that never exports the device to extensions.
 */
#define DEVICE_DT_DEFINE_NOEXPORT(node_id, init_fn, pm, data, config, level, prio, api, \
			 ...)                                                  \
...
#define DEVICE_DT_DEFINE(node_id, init_fn, pm, data, config, level, prio, api, \
			 ...)                                                  \
	DEVICE_DT_DEFINE_NOEXPORT(node_id, init_fn, pm, data, config, level, prio, api, __VA_ARGS__) \
	IF_ENABLED(CONFIG_LLEXT_EXPORT_DEVICES, (Z_DEVICE_EXPORT(node_id);))

/**
 * @brief Variant of @ref DEVICE_DT_INST_DEFINE that never exports the device to extensions.
 */
#define DEVICE_DT_INST_DEFINE_NOEXPORT(inst, ...)                                       \
	DEVICE_DT_DEFINE_NOEXPORT(DT_DRV_INST(inst), __VA_ARGS__)
...
#if defined(CONFIG_DEVICE_STORE_DT_ID) || defined(__DOXYGEN__)
	const int dt_id;
#endif /* CONFIG_DEVICE_STORE_DT_ID */
};
...
#if defined(CONFIG_DEVICE_STORE_DT_ID) || defined(__DOXYGEN__)
/**
 * @brief Get the device tree node ID from a device.
 * @param dev device whose node ID to look up
 * @return node ID
 */
static inline int device_get_dt_id(const struct device *dev)
{
	
	return dev->dt_id;
}

static inline const struct device *device_get_by_dt_id(int dt_id){
	STRUCT_SECTION_FOREACH(device, dev) {
		const int dt_id_found = device_get_dt_id(dev);

		if (!device_is_ready(dev)) {
			continue;
		}

		if(dt_id_found == dt_id){
			return dev;
		}
	}
		...
		IF_ENABLED(CONFIG_DEVICE_DT_METADATA,					\
			   (IF_ENABLED(DT_NODE_EXISTS(node_id_),			\
				       (.dt_meta = &Z_DEVICE_DT_METADATA_NAME_GET(	\
						dev_id_),))))								\
		IF_ENABLED(CONFIG_DEVICE_STORE_DT_ID, (.dt_id = DT_DEP_ORD(node_id_),))				\
	}
...
#define Z_DEVICE_INIT_ENTRY_DEFINE(node_id, dev_id, init_fn_, level, prio)                         \
	...
	static const Z_DECL_ALIGN(struct init_entry) __used __noasan Z_INIT_ENTRY_SECTION(         \
		...
		Z_INIT_ENTRY_NAME(DEVICE_NAME_GET(dev_id)) = {                                     \
			...
			COND_CODE_1(CONFIG_SKADI_OS, (.priority=prio), ())\
	}
