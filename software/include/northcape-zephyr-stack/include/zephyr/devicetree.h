...
#define DT_REG_ADDR_BY_IDX(node_id, idx) 			\
	DT_CAT4(__SKADI_,node_id,_REG_IDX_,idx)
 ...
 */
#define DT_REG_ADDR_BY_IDX(node_id, idx) \
	DT_REG_ADDR_BY_IDX_RAW(node_id, idx)
#endif
...
/**
 * @brief Get a node's (only) register block address, always provides physical address
 * @param node_id node identifier
 * @return node's register block address (physical address, not usable by Skadi subsystems!)
 */
...
#else
#define DT_REG_ADDR_U64(node_id) ((uint64_t) DT_REG_ADDR(node_id))
#endif

/**
 * @brief 64-bit version of DT_REG_ADDR_RAW()
 *
 * This macro version adds the appropriate suffix for 64-bit unsigned
 * integer literals.
 * Note that this macro is equivalent to DT_REG_ADDR_RAW() in linker/ASM context.
 *
 * @param node_id node identifier
 * @return node's register block address, always raw physical address - not usable in Skadi subsystems!
 */
#define DT_REG_ADDR_U64_RAW(node_id) DT_U64_C(DT_REG_ADDR_RAW(node_id))
...
/**
 * @brief Get a `DT_DRV_COMPAT` instance's idx-th register block's address, as raw physical address
 * @param inst instance number
 * @param idx index of the register whose address to return
 * @return address of the instance's idx-th register block; for Northacpe, might not be accessible!
 */
