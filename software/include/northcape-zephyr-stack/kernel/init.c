...
/* Skadi: one "kernel" instance for the loader, one actual instance for the subsystem */
...
/* Skadi: this is imported by threading-aware subsystems to speed up getting current thread */
uintptr_t *const skadi_sched_current_reloc;
EXPORT_SYMBOL(skadi_sched_current_reloc);

/* needs to be readable... */
static bool skadi_setup_current_reloc(void){
	uintptr_t tmp = (uintptr_t)&skadi_sched_current_reloc;
	/* write-protection not yet enabled at init time */
	uintptr_t **writable_pointer = (uintptr_t **) tmp;
	uintptr_t* derived_token = (void*)skadi_cap_ops_derive_arg_ro(&_current_cpu->current_id, sizeof(uintptr_t));

	/* start with default value */
	_current_cpu->current_id = (uintptr_t)&_thread_dummy;

	z_main_thread.thread_id = (uintptr_t) &z_main_thread;

	*writable_pointer = derived_token;

	__ASSERT_NO_MSG(derived_token);

	return derived_token != NULL;
}
...
/* we "miss" this during early initialization, so do this first thing after being loaded */
static bool setup_dummy_thread(void){
	z_dummy_thread_init(&_thread_dummy);

	return true;
}
SKADI_SUBSYSTEM_INIT_FUNCTIONS(setup_dummy_thread, skadi_setup_current_reloc);
...
#ifdef SKADI_SUBSYSTEM
/* the loader collects this from itself and the other subsystems */
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_VOID(z_sys_init_run_level, enum init_level level);
__pinned_bss
...
static void z_sys_init_run_level(enum init_level level)
{
	...
	/* there is no point checking loader until submodules have been loaded */
	if(level >= INIT_LEVEL_PRE_KERNEL_1 && entry->priority > CONFIG_SKADI_MODULES_INIT_PRIO){
		do{
			result = skadi_loader_call_next_init_function(level, entry->priority);
		}
		while(result > 0);
	}

	/* either no remaining subsystem init functions with the same / lower prio or error */
#endif
...
#ifdef CONFIG_SKADI_LOADER
	/* we have to check if any subsystems have not been called yet */
	if(level >= INIT_LEVEL_PRE_KERNEL_1){
		do{
			result = skadi_loader_call_next_init_function(level, INT_MAX);
		}
		while(result > 0);
	}
	...
	LOG_INF("Runlevel %u complete!", level);
}

SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_ARGS_ALLOW_SELF(__skadi_nuuk_wrapper, uint64_t return_address);
/* nuuk HAS to run with task ID 0 */
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_IMPL_ALLOW_SELF(__skadi_nuuk_wrapper, 1, uint64_t return_address)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_loader_proxy_z_sys_init_run_level, enum init_level level)
	z_sys_init_run_level(level);

	if(level == INIT_LEVEL_APPLICATION && IS_ENABLED(CONFIG_SKADI_NUUK)){
		void *nuuk = (void*) skadi_loader_get_symbol("__skadi_nuuk_callee_trampoline");

		__ASSERT(nuuk, "Could not find nuuk!");

		if(!nuuk){
			LOG_ERR("Nuuk callee trampoline missing!");
			k_panic();
		}
		LOG_INF("Releasing remaining initial allocator heap!");

		skadi_init_alloc_relese_unused_heap();
		
		LOG_INF("Final runlevel complete - launching Nuuk!");
		/* we forward our return address, which the callee trampoline harness conveniently provides */
		__skadi_nuuk_wrapper(return_capability, nuuk);
	}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_loader_proxy_z_sys_init_run_level)
...
/* use function pointers to resolve dependency conflict with main subsystem */
#ifdef CONFIG_BOOTARGS

SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_ARGS(int, __skadi_main, int argc, char **argv);
static int main(int argc, char **argv){
	__ASSERT_NO_MSG(main_ptr);

#ifdef CONFIG_SKADI_ALLOCATOR_PRINT_FREE_MEM
	__skadi_allocator_print_free_mem();
#endif

	LOG_INF("Calling main %p", main_ptr);
	return __skadi_main(argc, argv, main_ptr);
}
#else
/* TODO problem with trampoline generation... */
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_ARGS(int, __skadi_main, uintptr_t dummy_param);
static int main(void){
	...
	LOG_INF("Calling main %p", main_ptr);
	return __skadi_main(0, main_ptr);
}
...
#ifdef SKADI_SUBSYSTEM
	/* we have to do this BEFORE the loader gets nuuked */
	main_ptr = (void*) skadi_loader_get_symbol("main_callee_trampoline");
	if(!main_ptr){
		LOG_WRN("Could not resolve main()!");
	}
#endif
...
/* Skadi uses a different mechanism */
	...
	LOG_INF("Runlevels complete!");

	z_init_static_threads();

	LOG_INF("Static threads init complete!");
...
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_ARGS(__skadi_switch_to_main_thread, char *stack_ptr);
SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_IMPL(__skadi_switch_to_main_thread, 1, char *stack_ptr);

__boot_func
static FUNC_NORETURN void switch_to_main_thread(char *stack_ptr)
{
	void *switch_to_main = (void*) skadi_loader_get_symbol("__skadi_switch_to_main_thread_callee_trampoline");

	__ASSERT_NO_MSG(switch_to_main);
    
	SKADI_SUBSYSTEM_INITIALIZE_CALLER_TRAMPOLINE(__skadi_switch_to_main_thread);

	__skadi_loader_proxy_z_sys_init_run_level_register_init_function();

	LOG_INF("Calling __skadi_switch_to_main_thread at %p!", switch_to_main);
	__skadi_switch_to_main_thread(stack_ptr, switch_to_main);
	CODE_UNREACHABLE;
}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_switch_to_main_thread, char *stack_ptr)
	ARG_UNUSED(stack_ptr);
	switch_to_main_thread(prepare_multithreading());
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_switch_to_main_thread)
#endif /* SKADI_SUBSYSTEM */
...
FUNC_NORETURN void z_cstart(uint64_t start_time)
{
...
#if defined(CONFIG_MULTITHREADING) && !defined(CONFIG_SKADI_OS)
	/* TODO setup in subsystem */
	z_dummy_thread_init(&_thread_dummy);
#endif /* CONFIG_MULTITHREADING */
