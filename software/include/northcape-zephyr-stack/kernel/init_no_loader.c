...
// used by the ISR subsystem
// TODO this should be properly isolated in a subsystem (probably)
#include <zephyr/llext/llext.h>
EXPORT_SYMBOL(_kernel);
...
static void bg_thread_main(void *unused1, void *unused2, void *unused3)
{
	...
	/* need to do a subsystem call into main function provided by subsystem */
	(void)skadi_loader_call_main(argc, argv);
#else /* CONFIG_SKADI_SUBSYSTEM_PROVIDES_MAIN */
	(void)main(argc, argv);
#endif 
#else
#ifdef CONFIG_SKADI_SUBSYSTEM_PROVIDES_MAIN
	/* need to do a subsystem call into main function provided by subsystem */
	(void)skadi_loader_call_main(0, NULL);
#else
	extern int main(void);

	(void)main();
#endif /* CONFIG_SKADI_SUBSYSTEM_PROVIDES_MAIN */
...
#ifdef CONFIG_SKADI_OS
	extern void skadi_init_clear_irq_stack(void *irq_stack);
	if(_kernel.cpus[id].irq_stack){
		skadi_init_clear_irq_stack(_kernel.cpus[id].irq_stack);
	}
#endif
