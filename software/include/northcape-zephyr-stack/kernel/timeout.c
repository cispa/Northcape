...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(k_ticks_t, __skadi_timeout_remaining, const struct _timeout *timeout);
	return z_timeout_remaining(timeout);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timeout_remaining);
#endif
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_sys_clock_announce, int32_t ticks)
	sys_clock_announce(ticks);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sys_clock_announce)
#endif
...
#ifdef CONFIG_SKADI_LOADER
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int64_t, skadi_sys_clock_tick_get)
	return sys_clock_tick_get();
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_sys_clock_tick_get)
#endif
...
#ifdef CONFIG_SKADI_LOADER
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int64_t, skadi_uptime_ticks, void)
	return k_uptime_ticks();
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_uptime_ticks)
#endif
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(k_timepoint_t, __skadi_sys_timepoint_calc, k_timeout_t timeout);
	return sys_timepoint_calc(timeout);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sys_timepoint_calc)
#endif
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(k_timeout_t, __skadi_sys_timepoint_timeout, k_timepoint_t timeout);
	return sys_timepoint_timeout(timeout);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sys_timepoint_timeout)
#endif
...
__boot_func static int timeout_init_trampolines(void){
    bool init_ok = true;

	init_ok &= skadi_sys_clock_tick_get_register_init_function();
	init_ok &= __skadi_timeout_remaining_register_init_function();
	init_ok &= skadi_uptime_ticks_register_init_function();
	
    return init_ok == true ? 0 : -ENOMEM;
}

SYS_INIT(timeout_init_trampolines, PRE_KERNEL_1, CONFIG_LOADER_SKADI_TRAMPOLINE_INIT_PRIO);
