...
SKADI_INTERFACE_WRAPPER_INIT(SKADI_POLL_SIGNAL);
/* TODO needed as paddding? */
SKADI_INTERFACE_WRAPPER_INIT_GLOBAL(SKADI_POLL_EVENT);
SKADI_INTERFACE_WRAPPER_DECLARE(SKADI_MSGQ);
SKADI_INTERFACE_WRAPPER_DECLARE(SKADI_PIPE);
SKADI_INTERFACE_WRAPPER_DECLARE(SKADI_QUEUE);
SKADI_INTERFACE_WRAPPER_DECLARE(SKADI_SEM);
...
#ifdef CONFIG_SKADI_LOADER
		if (k_sem_count_get(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_SEM, struct k_sem, event->sem)) > 0U) {
#else
...
#ifdef CONFIG_SKADI_LOADER
		if (!k_queue_is_empty(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, event->queue))) {
#else
...
#ifdef CONFIG_SKADI_LOADER
		if(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_POLL_SIGNAL, struct k_poll_signal, event->signal)->signaled != 0U) {
#else
...
#ifdef CONFIG_SKADI_LOADER
		if(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_MSGQ, struct k_msgq, event->msgq)->used_msgs > 0) {
#else
...
#ifdef CONFIG_SKADI_LOADER
		if (k_pipe_read_avail(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_PIPE, struct k_pipe, event->pipe))) {
#else
...
#ifdef CONFIG_SKADI_LOADER
		add_event(&SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_SEM, struct k_sem, event->sem)->poll_events, event, poller);
#else
...
#ifdef CONFIG_SKADI_LOADER
		add_event(&SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, event->queue)->poll_events, event, poller);
#else
...
#ifdef CONFIG_SKADI_LOADER
		add_event(&SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_POLL_SIGNAL, struct k_poll_signal, event->signal)->poll_events, event, poller);
#else
...
#ifdef CONFIG_SKADI_LOADER
		add_event(&SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_MSGQ, struct k_msgq, event->msgq)->poll_events, event, poller);
#else
...
#ifdef CONFIG_SKADI_LOADER
		add_event(&SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_PIPE, struct k_pipe, event->pipe)->poll_events, event, poller);
#else
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_poll, struct k_poll_event *events, int num_events, k_timeout_t timeout)
		__ASSERT_NO_MSG(events);
		return z_impl_k_poll(events, num_events, timeout);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_poll)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	#define INIT_FN(POLL_SIGNAL) z_impl_k_poll_signal_init(POLL_SIGNAL)
	#define FREE_FN(POLL_SIGNAL) (void)(POLL_SIGNAL);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_poll_signal_init, struct k_poll_signal *sig)
		__ASSERT_NO_MSG(sig);
		SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_POLL_SIGNAL, struct k_poll_signal, INIT_FN, FREE_FN, sig);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_poll_signal_init)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_poll_signal_reset, struct k_poll_signal *sig)
		__ASSERT_NO_MSG(sig);
		z_impl_k_poll_signal_reset(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_POLL_SIGNAL, struct k_poll_signal, sig));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_poll_signal_reset)
#endif
...
#ifdef CONFIG_SKADI_LOADER
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_poll_signal_check, struct k_poll_signal *sig, unsigned int *signaled, int *result)
	z_impl_k_poll_signal_check(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_POLL_SIGNAL, struct k_poll_signal, sig), signaled, result);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_poll_signal_check)
#endif
...
int z_impl_k_poll_signal_raise(struct k_poll_signal *sig, int result)
{
	...
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_poll_signal_raise, struct k_poll_signal *sig, int result)
		__ASSERT_NO_MSG(sig);
		return z_impl_k_poll_signal_raise(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_POLL_SIGNAL, struct k_poll_signal, sig), result);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_poll_signal_raise)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_poll_signal_cleanup, struct k_poll_signal *sig)
		SKADI_INTERFACE_WRAPPER_REMOVE(SKADI_POLL_SIGNAL, sig);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_poll_signal_cleanup)
#endif
...
__boot_func static int signal_init_trampolines(void){
    bool init_ok = true;

	init_ok &= __skadi_poll_register_init_function();
	init_ok &= __skadi_poll_signal_init_register_init_function();
	init_ok &= __skadi_poll_signal_check_register_init_function();
	init_ok &= __skadi_poll_signal_raise_register_init_function();
	init_ok &= __skadi_poll_signal_reset_register_init_function();
	
    return init_ok == true ? 0 : -ENOMEM;
}


SYS_INIT(signal_init_trampolines, PRE_KERNEL_1, CONFIG_LOADER_SKADI_TRAMPOLINE_INIT_PRIO);
