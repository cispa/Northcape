...
#ifdef CONFIG_SKADI_LOADER
SKADI_INTERFACE_WRAPPER_INIT(SKADI_EVENT);
#define INIT_FN(EVENT) k_event_init(EVENT)
	#define FREE_FN(EVENT) (void)(EVENT);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_event_init, struct k_event *event);
	SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_EVENT, struct k_event, INIT_FN, FREE_FN, event);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_event_init)
#endif
...
#ifdef SKADI_SUBSYSTEM
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, __skadi_event_post, struct k_event *event, uint32_t events);
		return z_impl_k_event_post(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_EVENT, struct k_event, event), events);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_event_post);
#endif
...
#ifdef SKADI_SUBSYSTEM
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, __skadi_event_set, struct k_event *event, uint32_t events);
		return z_impl_k_event_set(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_EVENT, struct k_event, event), events);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_event_set);
#endif
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, __skadi_event_set_masked, struct k_event *event, uint32_t events, uint32_t events_masked);
	return z_impl_k_event_set_masked(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_EVENT, struct k_event, event), events, events_masked);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_event_set_masked);
#endif
...
#ifdef SKADI_SUBSYSTEM
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, __skadi_event_clear, struct k_event *event, uint32_t events);
		return z_impl_k_event_clear(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_EVENT, struct k_event, event), events);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_event_clear);
#endif
...
#ifdef SKADI_SUBSYSTEM
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, __skadi_event_wait, struct k_event *event, uint32_t events, bool reset, k_timeout_t timeout);
		return z_impl_k_event_wait(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_EVENT, struct k_event, event), events, reset, timeout);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_event_wait);
#endif
...
#ifdef SKADI_SUBSYSTEM
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, __skadi_event_wait_all, struct k_event *event, uint32_t events, bool reset, k_timeout_t timeout);
		return z_impl_k_event_wait_all(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_EVENT, struct k_event, event), events, reset, timeout);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_event_wait_all);
#endif


#ifdef SKADI_SUBSYSTEM
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_event_cleanup, struct k_event *event){
		SKADI_INTERFACE_WRAPPER_REMOVE(SKADI_EVENT, event);
	}
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_event_cleanup)
#endif
