...
#ifdef CONFIG_SKADI_LOADER
SKADI_INTERFACE_WRAPPER_INIT(SKADI_STACK);
#define INIT_FN(STACK) k_stack_init(STACK, buffer, num_entries);
#define FREE_FN(STACK) (void)(STACK);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_stack_init, struct k_stack *s, stack_data_t *buffer, uint32_t num_entries);
	SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_STACK, struct k_stack, INIT_FN, FREE_FN, s);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_stack_init);
#endif
...
#ifdef CONFIG_SKADI_LOADER
#define INIT_FN_ALLOC(STACK) ret = k_stack_alloc_init(STACK, num_entries);
#define FREE_FN_ALLOC(STACK) (void)(STACK);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int32_t, __skadi_stack_alloc_init, struct k_stack *s, uint32_t num_entries);
	int ret;
	SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_STACK, struct k_stack, INIT_FN_ALLOC, FREE_FN_ALLOC, s);
	return ret;
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_stack_alloc_init);
#endif
...
#ifdef CONFIG_SKADI_LOADER
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_stack_cleanup, struct k_stack *s);
	int ret = k_stack_cleanup(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_STACK, struct k_stack, s));
	SKADI_INTERFACE_WRAPPER_REMOVE(SKADI_STACK, s);
	return ret;
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_stack_cleanup);
#endif
...
#ifdef CONFIG_SKADI_LOADER
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_stack_push, struct k_stack *s, stack_data_t data);
	return k_stack_push(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_STACK, struct k_stack, s), data);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_stack_push);
#endif
...
#ifdef CONFIG_SKADI_LOADER
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_stack_pop, struct k_stack *s, stack_data_t *data, k_timeout_t timeout);
	return k_stack_pop(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_STACK, struct k_stack, s), data, timeout);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_stack_pop);
#endif
