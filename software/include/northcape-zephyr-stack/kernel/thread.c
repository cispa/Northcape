...
SKADI_INTERFACE_WRAPPER_INIT_GLOBAL(SKADI_THREAD);
...
int z_impl_k_thread_name_set(k_tid_t thread, const char *str)
{
	...
	int __skadi_k_thread_name_set(k_tid_t thread, const char *str){
		return z_impl_k_thread_name_set(thread, str);
	}
	EXPORT_SYMBOL(__skadi_k_thread_name_set);
...
#ifdef CONFIG_SKADI_OS
	 int __skadi_k_thread_name_copy(k_tid_t thread, char *buf, size_t size){
		return z_impl_k_thread_name_copy(thread, buf, size);
	 }
	 EXPORT_SYMBOL(__skadi_k_thread_name_copy);
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_thread_start, k_tid_t thread)
		z_impl_k_thread_start(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_THREAD, struct k_thread, thread));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_thread_start)
#endif
			 ...
			 int prio, uint32_t options, const char *name)
{
	...
	__ASSERT((uintptr_t)stack_ptr <= ((uintptr_t)stack) + stack_size, "Stack ptr %p is outside stack %p with length %zu", stack_ptr, stack, stack_size);
...
#ifdef CONFIG_SKADI_LOADER
	/* need to setup thread ID if not done so already */
	if(!new_thread->thread_id){
		new_thread->thread_id = (uintptr_t) new_thread;
	}
#endif
...
#ifdef CONFIG_SKADI_LOADER
	#define INIT_FN(THREAD) z_impl_k_thread_create(THREAD, params->stack, params->stack_size, params->entry, params->p1, params->p2, params->p3, params->prio, params->options, params->delay); THREAD->thread_id = (uintptr_t)params->new_thread
	#define FREE_FN(THREAD) (void)(THREAD);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(k_tid_t, __skadi_thread_create, const struct skadi_thread_create_params *params)
		__ASSERT_NO_MSG(params);
		
		SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_THREAD, struct k_thread, INIT_FN, FREE_FN, params->new_thread);

		return params->new_thread;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_thread_create)
#endif

#ifdef CONFIG_SKADI_LOADER
	SKADI_INTERFACE_WRAPPER_DECLARE(SKADI_HEAP);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_thread_heap_assign, struct k_thread *thread, struct k_heap *heap)
		if(thread == _current){
			_current->resource_pool = SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_HEAP, struct k_heap, heap);
		}
		else{
			SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_THREAD, struct k_thread, thread)->resource_pool = SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_HEAP, struct k_heap, heap);
		}
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_thread_heap_assign)
#endif
...
__boot_func static int thread_init_trampolines(void){
    bool init_ok = true;

	init_ok &= __skadi_thread_start_register_init_function();
	init_ok &= __skadi_thread_create_register_init_function();
	
    return init_ok == true ? 0 : -ENOMEM;
}

SYS_INIT(thread_init_trampolines, PRE_KERNEL_1, CONFIG_LOADER_SKADI_TRAMPOLINE_INIT_PRIO);
