...
#ifdef CONFIG_SKADI_LOADER
	SKADI_INTERFACE_WRAPPER_INIT_GLOBAL(SKADI_QUEUE);
	#define INIT_FN(QUEUE) z_impl_k_queue_init(QUEUE)
	#define FREE_FN(QUEUE) (void)(QUEUE);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_queue_init, struct k_queue *queue)
		SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_QUEUE, struct k_queue, INIT_FN, FREE_FN, queue);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_init)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_queue_cancel_wait, struct k_queue *queue)
		return z_impl_k_queue_cancel_wait(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_cancel_wait)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_queue_insert, struct k_queue *queue, void *prev, void *data)
		return k_queue_insert(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), prev, data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_insert)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_queue_append, struct k_queue *queue, void *data)
		return k_queue_append(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_append)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_queue_prepend, struct k_queue *queue, void *data)
		return k_queue_prepend(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_prepend)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int32_t, __skadi_queue_alloc_append, struct k_queue *queue, void *data)
		return z_impl_k_queue_alloc_append(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_alloc_append)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int32_t, __skadi_queue_alloc_prepend, struct k_queue *queue, void *data)
		return z_impl_k_queue_alloc_prepend(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_alloc_prepend)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_queue_append_list, struct k_queue *queue, void *head, void *tail)
		return k_queue_append_list(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), head, tail);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_append_list)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_queue_merge_slist, struct k_queue *queue, sys_slist_t *list)
		return k_queue_merge_slist(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), list);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_merge_slist)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void *, __skadi_queue_get, struct k_queue *queue, k_timeout_t timeout)
		__ASSERT_NO_MSG(queue);
		return z_impl_k_queue_get(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), timeout);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_get)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(bool, __skadi_queue_remove, struct k_queue *queue, void *data)
		return k_queue_remove(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_remove)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(bool, __skadi_queue_unique_append, struct k_queue *queue, void *data)
		return k_queue_unique_append(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_unique_append)
#endif

#ifdef CONFIG_SKADI_LOADER
bool k_queue_alloc_unique_append(struct k_queue *queue, void *data)
{
	SYS_PORT_TRACING_OBJ_FUNC_ENTER(k_queue, unique_alloc_append, queue);

	sys_sfnode_t *test;

	SYS_SFLIST_FOR_EACH_NODE(&queue->data_q, test) {
		if (test == (sys_sfnode_t *) data || ((struct alloc_node *) test) -> data == data) {
			SYS_PORT_TRACING_OBJ_FUNC_EXIT(k_queue, unique_alloc_append, queue, false);

			return false;
		}
	}

	k_queue_alloc_append(queue, data);

	SYS_PORT_TRACING_OBJ_FUNC_EXIT(k_queue, unique_alloc_append, queue, true);

	return true;
}
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(bool, __skadi_queue_alloc_unique_append, struct k_queue *queue, void *data)
		return k_queue_alloc_unique_append(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue), data);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_alloc_unique_append)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void *, __skadi_queue_peek_head, struct k_queue *queue)
		return k_queue_peek_head(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_peek_head)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void *, __skadi_queue_peek_tail, struct k_queue *queue)
		return k_queue_peek_tail(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_peek_tail)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(bool, __skadi_queue_is_empty, struct k_queue *queue)
		return k_queue_is_empty(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_QUEUE, struct k_queue, queue));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_is_empty)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_queue_cleanup, struct k_queue *queue)
		SKADI_INTERFACE_WRAPPER_REMOVE(SKADI_QUEUE, queue);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_queue_cleanup)
#endif
...
__boot_func static int msgq_init_trampolines(void){
	...
	init_ok &= __skadi_queue_init_register_init_function();

	init_ok &= __skadi_queue_cancel_wait_register_init_function();

	init_ok &= __skadi_queue_append_register_init_function();

	init_ok &= __skadi_queue_alloc_append_register_init_function();

	init_ok &= __skadi_queue_prepend_register_init_function();

	init_ok &= __skadi_queue_alloc_prepend_register_init_function();

	init_ok &= __skadi_queue_insert_register_init_function();

	init_ok &= __skadi_queue_append_list_register_init_function();

	init_ok &= __skadi_queue_merge_slist_register_init_function();

	init_ok &=  __skadi_queue_get_register_init_function();

	init_ok &=  __skadi_queue_remove_register_init_function();

	init_ok &=  __skadi_queue_unique_append_register_init_function();

	init_ok &= __skadi_queue_alloc_unique_append_register_init_function();

	init_ok &=  __skadi_queue_peek_head_register_init_function();

	init_ok &=  __skadi_queue_peek_tail_register_init_function();
