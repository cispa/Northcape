...
SKADI_INTERFACE_WRAPPER_DECLARE(SKADI_MUTEX);
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_INTERFACE_WRAPPER_INIT(SKADI_CONDVAR);
	#define INIT_FN(CONDVAR) ret = z_impl_k_condvar_init(CONDVAR)
	#define FREE_FN(CONDVAR) (void)(CONDVAR);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_condvar_init, struct k_condvar *condvar){
		int ret;
		__ASSERT_NO_MSG(condvar);

		SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_CONDVAR, struct k_condvar, INIT_FN, FREE_FN, condvar);

		return ret;
	}
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_condvar_init)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_condvar_signal, struct k_condvar *condvar)
		__ASSERT_NO_MSG(condvar);
		return z_impl_k_condvar_signal(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_CONDVAR, struct k_condvar, condvar));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_condvar_signal)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_condvar_broadcast, struct k_condvar *condvar)
		__ASSERT_NO_MSG(condvar);
		return z_impl_k_condvar_broadcast(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_CONDVAR, struct k_condvar, condvar));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_condvar_broadcast)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_condvar_wait, struct k_condvar *condvar, struct k_mutex *mutex, k_timeout_t timeout)
		__ASSERT_NO_MSG(condvar);
		__ASSERT_NO_MSG(mutex);
		return z_impl_k_condvar_wait(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_CONDVAR, struct k_condvar, condvar), SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_MUTEX, struct k_mutex, mutex), timeout);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_condvar_wait)
#endif

#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_condvar_cleanup, struct k_condvar *condvar){
		SKADI_INTERFACE_WRAPPER_REMOVE(SKADI_CONDVAR, condvar);
	}
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_condvar_cleanup)
#endif
...
__boot_func static int condvar_init_trampolines(void){
    bool init_ok = true;

	init_ok &= __skadi_condvar_init_register_init_function();
	init_ok &= __skadi_condvar_signal_register_init_function();
	init_ok &= __skadi_condvar_broadcast_register_init_function();
	init_ok &= __skadi_condvar_wait_register_init_function();
	
    return init_ok == true ? 0 : -ENOMEM;
}

SYS_INIT(condvar_init_trampolines,PRE_KERNEL_1, CONFIG_LOADER_SKADI_TRAMPOLINE_INIT_PRIO);
