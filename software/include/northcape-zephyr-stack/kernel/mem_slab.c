...
#ifdef CONFIG_SKADI_LOADER
	SKADI_INTERFACE_WRAPPER_INIT(SKADI_MEM_SLAB);
	#define INIT_FN(MEM_SLAB) ret = k_mem_slab_init(MEM_SLAB, buffer, block_size, num_blocks)
	#define FREE_FN(MEM_SLAB) (void)(MEM_SLAB);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_mem_slab_init, struct k_mem_slab *slab, void *buffer, size_t block_size, uint32_t num_blocks)
		int ret;
		SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_MEM_SLAB, struct k_mem_slab, INIT_FN, FREE_FN, slab);
		return ret;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_mem_slab_init)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_mem_slab_alloc, struct k_mem_slab *slab, void **mem, k_timeout_t timeout)
		int ret;
		ret = k_mem_slab_alloc(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_MEM_SLAB, struct k_mem_slab, slab), mem, timeout);
		return ret;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_mem_slab_alloc)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void *, __skadi_mem_slab_alloc_alt, struct k_mem_slab *slab, k_timeout_t timeout)
		void *ret;
		int retval = k_mem_slab_alloc(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_MEM_SLAB, struct k_mem_slab, slab), &ret, timeout);
		return retval ? NULL : ret;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_mem_slab_alloc_alt)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_mem_slab_free, struct k_mem_slab *slab, void *mem)
		k_mem_slab_free(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_MEM_SLAB, struct k_mem_slab, slab), mem);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_mem_slab_free)
#endif

#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(uint32_t, __skadi_mem_slab_num_free_get, struct k_mem_slab *slab){
		uint32_t ret;
		ret = k_mem_slab_num_free_get(slab);

		return ret;
	}
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_mem_slab_num_free_get)
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_mem_slab_cleanup, struct k_mem_slab *slab){
		SKADI_INTERFACE_WRAPPER_REMOVE(SKADI_MEM_SLAB, slab);
	}
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_mem_slab_cleanup)
#endif
...
__boot_func static int mem_init_trampolines(void){
    bool init_ok = true;


	init_ok &= __skadi_mem_slab_init_register_init_function();
	init_ok &= __skadi_mem_slab_alloc_register_init_function();
	init_ok &= __skadi_mem_slab_alloc_alt_register_init_function();
	init_ok &= __skadi_mem_slab_free_register_init_function();
	
    return init_ok == true ? 0 : -ENOMEM;
}

SYS_INIT(mem_init_trampolines, PRE_KERNEL_1, CONFIG_LOADER_SKADI_TRAMPOLINE_INIT_PRIO);
