...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_ARGS(skadi_timer_wrapper, struct k_timer *timer);
	SKADI_SUBSYSTEM_CALLER_TRAMPOLINE_FN_PTR_VOID_IMPL(skadi_timer_wrapper, 1, struct k_timer *timer);
#endif
...
#ifdef CONFIG_SKADI_LOADER
	if(!skadi_token_is_in_our_text(timer->expiry_fn)){
			skadi_subsystem_check_function_pointer(timer->expiry_fn, false, false);
				/* subsystem call needed, we probably jump into other subsystem - provide original timer as back reference */
			skadi_timer_wrapper(timer->user_data, timer->expiry_fn);
		}
	else{
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_INTERFACE_WRAPPER_INIT(SKADI_TIMER);
	#define INIT_FN(TIMER) k_timer_init(TIMER, expiry_fn, stop_fn); TIMER->user_data=timer
	#define FREE_FN(TIMER) (void)(TIMER);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_timer_init, struct k_timer *timer, k_timer_expiry_t expiry_fn, k_timer_stop_t stop_fn)
		SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_TIMER, struct k_timer, INIT_FN, FREE_FN, timer);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_init)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_timer_start, struct k_timer *timer, k_timeout_t duration, k_timeout_t period)
		z_impl_k_timer_start(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_TIMER, struct k_timer, timer), duration, period);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_start)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	if(!skadi_token_is_in_our_text(timer->stop_fn)){
			/* subsystem call needed, we probably jump into other subsystem */
			skadi_timer_wrapper(timer, timer->stop_fn);
		}
	else{
			timer->stop_fn(timer);
		}
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_timer_stop, struct k_timer *timer)
		z_impl_k_timer_stop(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_TIMER, struct k_timer, timer));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_stop)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(uint32_t, __skadi_timer_status_get, struct k_timer *timer)
		return z_impl_k_timer_status_get(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_TIMER, struct k_timer, timer));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_status_get)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(uint32_t, __skadi_timer_status_sync, struct k_timer *timer)
		return z_impl_k_timer_status_sync(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_TIMER, struct k_timer, timer));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_status_sync)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(k_ticks_t, __skadi_timer_expires_ticks, const struct k_timer *timer)
		return k_timer_expires_ticks(SKADI_INTERFACE_WRAPPER_TRANSLATE_CONST(SKADI_TIMER, struct k_timer, timer));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_expires_ticks)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(k_ticks_t, __skadi_timer_remaining_ticks, const struct k_timer *timer)
		return k_timer_remaining_ticks(SKADI_INTERFACE_WRAPPER_TRANSLATE_CONST(SKADI_TIMER, struct k_timer, timer));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_remaining_ticks)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(k_ticks_t, __skadi_timer_remaining_get, const struct k_timer *timer)
		return k_ticks_to_ms_floor32(z_timeout_remaining(&SKADI_INTERFACE_WRAPPER_TRANSLATE_CONST(SKADI_TIMER, struct k_timer, timer)->timeout));;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_remaining_get)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_timer_cleanup, struct k_timer *timer)
		SKADI_INTERFACE_WRAPPER_REMOVE(SKADI_TIMER, timer);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_timer_cleanup)
...
__boot_func static int timer_init_trampolines(void){
    bool init_ok = true;

	init_ok &= __skadi_timer_init_register_init_function();
	init_ok &= __skadi_timer_start_register_init_function();
	init_ok &= __skadi_timer_stop_register_init_function();
	init_ok &= __skadi_timer_status_get_register_init_function();
	init_ok &= __skadi_timer_status_sync_register_init_function();
	
    return init_ok == true ? 0 : -ENOMEM;
}

SYS_INIT(timer_init_trampolines, PRE_KERNEL_1, CONFIG_LOADER_SKADI_TRAMPOLINE_INIT_PRIO);
