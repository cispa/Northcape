...
#ifdef CONFIG_SKADI_LOADER
	SKADI_INTERFACE_WRAPPER_INIT_GLOBAL(SKADI_SEM);
	#define INIT_FN(SEM) ret = z_impl_k_sem_init(SEM, initial_count, limit)
	#define FREE_FN(SEM) (void)(SEM);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_sem_init, struct k_sem *sem, unsigned int initial_count, unsigned int limit)
		int ret;
		SKADI_INTERFACE_WRAPPER_REGISTER(SKADI_SEM, struct k_sem, INIT_FN, FREE_FN, sem);
		return ret;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sem_init)
#endif
...
void z_impl_k_sem_give(struct k_sem *sem)
{
	...
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(void, __skadi_sem_give, struct k_sem *sem)
		return z_impl_k_sem_give(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_SEM, struct k_sem, sem));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sem_give)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_sem_take, struct k_sem *sem, k_timeout_t timeout)
		return z_impl_k_sem_take(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_SEM, struct k_sem, sem), timeout);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sem_take)
#endif
...
#ifdef CONFIG_SKADI_LOADER
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_sem_reset, struct k_sem *sem)
		z_impl_k_sem_reset(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_SEM, struct k_sem, sem));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sem_reset)
	
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(int, __skadi_sem_count_get, struct k_sem *sem)
		return k_sem_count_get(SKADI_INTERFACE_WRAPPER_TRANSLATE(SKADI_SEM, struct k_sem, sem));
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sem_count_get)


	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_sem_cleanup, struct k_sem *sem)
		SKADI_INTERFACE_WRAPPER_REMOVE(SKADI_SEM, sem);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_sem_cleanup)
#endif
...
__boot_func static int sem_init_trampolines(void){
    bool init_ok = true;


	init_ok &= __skadi_sem_init_register_init_function();
	init_ok &= __skadi_sem_give_register_init_function();
	init_ok &= __skadi_sem_take_register_init_function();
	init_ok &= __skadi_sem_reset_register_init_function();
	
    return init_ok == true ? 0 : -ENOMEM;
}

SYS_INIT(sem_init_trampolines, PRE_KERNEL_1, CONFIG_LOADER_SKADI_TRAMPOLINE_INIT_PRIO);
