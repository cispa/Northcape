...
{
	__ASSERT_NO_MSG(thread_1);
	__ASSERT_NO_MSG(thread_2);
...
static inline void set_current(struct k_thread *new_thread)
{
	...
	__ASSERT_NO_MSG(new_thread->thread_id);
	if(!new_thread->thread_id){
		__asm__ volatile("ebreak");
	}
	_current_cpu->current_id = new_thread->thread_id;
#endif
}
...
#ifdef CONFIG_SKADI_OS
	static inline bool skadi_sched_current_thread_query_register_init_function(void);
#endif
...
void z_sched_init(void)
{
	...
	(void)skadi_sched_current_thread_query_register_init_function();
#endif
}
...
#ifdef CONFIG_SKADI_OS
	void __skadi_k_thread_priority_set(k_tid_t thread, int prio){
...
#ifdef CONFIG_SKADI_OS
	/* alias prevents us from using k_sleep in subsystems, where it will not work due to task ID */
	int32_t __skadi_k_sleep(k_timeout_t timeout){
...
#ifdef CONFIG_SKADI_OS
	/* alias prevents us from using k_sleep in subsystems, where it will not work due to task ID */
	int32_t __skadi_k_usleep(int32_t ns){
		return z_impl_k_usleep(ns);
	}
#endif
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_NOIRQ(k_tid_t, skadi_sched_current_thread_query, void)
	return z_impl_k_sched_current_thread_query();
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(skadi_sched_current_thread_query)
#endif
...
#if defined(SKADI_SUBSYSTEM) && defined(CONFIG_SKADI_MARK_THREAD_IN_TRAMPOLINE)
	extern void skadi_handle_thread_cancellation(k_tid_t thread);
	skadi_handle_thread_cancellation(thread);
#endif
}
...
#ifdef CONFIG_SKADI_OS
	void __skadi_k_thread_abort(k_tid_t thread){
...
#ifdef CONFIG_SKADI_OS
	int __skadi_k_thread_join(k_tid_t thread, k_timeout_t timeout){
