...
#include <zephyr/skadi/sys/skadi_bitarray.h>
#include <zephyr/skadi/skadi_sched.h>

#define sys_bitarray_test_bit skadi_sys_bitarray_test_bit
#define sys_bitarray_alloc skadi_sys_bitarray_alloc
#define sys_bitarray_free skadi_sys_bitarray_free
...
static struct fd_op_vtable zvfs_eventfd_fd_vtable, *zvfs_eventfd_fd_vtable_ptr;
...
static int zvfs_eventfd_read_locked(struct zvfs_eventfd *efd, zvfs_eventfd_t *value)
{
	...
	if (efd->cnt == 0) {
		skadi_poll_signal_reset(efd->read_sig);
	}

	skadi_poll_signal_raise(efd->write_sig, 0);
...
static int zvfs_eventfd_write_locked(struct zvfs_eventfd *efd, zvfs_eventfd_t *value)
{
	...
	if (efd->cnt == (UINT64_MAX - 1)) {
		skadi_poll_signal_reset(efd->write_sig);
	}

	skadi_poll_signal_raise(efd->read_sig, 0);

	return 0;
}

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, zvfs_eventfd_read_op, void *obj, void *buf, size_t sz)
{
	return zvfs_eventfd_rw_op(obj, buf, sz, zvfs_eventfd_read_locked);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(zvfs_eventfd_read_op)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, zvfs_eventfd_write_op, void *obj, const void *buf, size_t sz)
{
	return zvfs_eventfd_rw_op(obj, (zvfs_eventfd_t *)buf, sz, zvfs_eventfd_write_locked);
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(zvfs_eventfd_write_op)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, zvfs_eventfd_close_op, void *obj)
{
	int ret;
	int err;
	...
	err = (int)zvfs_get_obj_lock_and_cond(obj, zvfs_eventfd_fd_vtable_ptr, &lock, &cond);
	__ASSERT((bool)err, "zvfs_get_obj_lock_and_cond() failed");
	__ASSERT_NO_MSG(lock != NULL);
	__ASSERT_NO_MSG(cond != NULL);

	err = skadi_mutex_lock(lock, K_FOREVER);
	__ASSERT(err == 0, "skadi_mutex_lock() failed: %d", err);
...
unlock:
	...
	err = skadi_condvar_broadcast(cond);
	__ASSERT(err == 0, "skadi_condvar_broadcast() failed: %d", err);
	err = skadi_mutex_unlock(lock);
	__ASSERT(err == 0, "skadi_mutex_unlock() failed: %d", err);

	skadi_poll_signal_cleanup(efd->write_sig);
	skadi_poll_signal_cleanup(efd->read_sig);

	return ret;
}
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(zvfs_eventfd_close_op)

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_ALLOW_SELF(int, zvfs_eventfd_ioctl_op, void *obj, unsigned int request, va_list args)
{
	int ret;
	k_spinlock_key_t key;
	struct zvfs_eventfd *efd = (struct zvfs_eventfd *)obj;
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(zvfs_eventfd_ioctl_op)

static struct fd_op_vtable zvfs_eventfd_fd_vtable = {
	0
};
			     ...
			     int (*op)(struct zvfs_eventfd *efd, zvfs_eventfd_t *value))
{
	...
	while (true) {
		...
		err = skadi_condvar_wait(cond, lock, K_FOREVER);
...
unlock_mutex:
	...
	err = skadi_condvar_signal(cond);
	__ASSERT(err == 0, "skadi_condvar_signal() failed: %d", err);
	err = skadi_mutex_unlock(lock);
	__ASSERT(err == 0, "skadi_mutex_unlock() failed: %d", err);
	goto out;
...
int zvfs_eventfd(unsigned int initval, int flags)
{
	...
	efd->write_sig = skadi_allocator_alloc_rw(sizeof(*efd->write_sig));
	efd->read_sig = skadi_allocator_alloc_rw(sizeof(*efd->read_sig));

	if(!efd->write_sig || !efd->read_sig){
		errno = ENOMEM;
		return -1;
	}

	skadi_poll_signal_init(efd->write_sig);
	skadi_poll_signal_init(efd->read_sig);
	...
	zvfs_finalize_fd(fd, efd, zvfs_eventfd_fd_vtable_ptr);

	return fd;
}

SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zvfs_eventfd, unsigned int initval, int flags)
	return zvfs_eventfd(initval, flags);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zvfs_eventfd)
...
int zvfs_eventfd_read(int fd, zvfs_eventfd_t *value)
{
	...
	obj = zvfs_get_fd_obj(fd, zvfs_eventfd_fd_vtable_ptr, EBADF);
	if (obj == NULL) {
		return -1;
	}
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zvfs_eventfd_read, int fd, zvfs_eventfd_t *value)
	return zvfs_eventfd_read(fd, value);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zvfs_eventfd_read)
...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_zvfs_eventfd_write, int fd, zvfs_eventfd_t value)
	return zvfs_eventfd_write(fd, value);
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_zvfs_eventfd_write)

static bool skadi_zvfs_eventfd_init(void){
	zvfs_eventfd_fd_vtable.read = SKADI_SUBSYSTEM_FUNCTION_POINTER(zvfs_eventfd_read_op);
	zvfs_eventfd_fd_vtable.write = SKADI_SUBSYSTEM_FUNCTION_POINTER(zvfs_eventfd_write_op);
	zvfs_eventfd_fd_vtable.close = SKADI_SUBSYSTEM_FUNCTION_POINTER(zvfs_eventfd_close_op);
	zvfs_eventfd_fd_vtable.ioctl = SKADI_SUBSYSTEM_FUNCTION_POINTER(zvfs_eventfd_ioctl_op);

	zvfs_eventfd_fd_vtable_ptr = skadi_cap_ops_derive_arg(&zvfs_eventfd_fd_vtable, sizeof(zvfs_eventfd_fd_vtable));


	__ASSERT_NO_MSG(zvfs_eventfd_fd_vtable.read);
	__ASSERT_NO_MSG(zvfs_eventfd_fd_vtable.write);
	__ASSERT_NO_MSG(zvfs_eventfd_fd_vtable.close);
	__ASSERT_NO_MSG(zvfs_eventfd_fd_vtable.ioctl);
	__ASSERT_NO_MSG(zvfs_eventfd_fd_vtable_ptr);

	return zvfs_eventfd_fd_vtable.read && zvfs_eventfd_fd_vtable.write && zvfs_eventfd_fd_vtable.close && zvfs_eventfd_fd_vtable.ioctl && zvfs_eventfd_fd_vtable_ptr;
}
SKADI_SUBSYSTEM_INIT_FUNCTIONS(skadi_zvfs_eventfd_init);
