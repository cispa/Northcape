...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_opterr, void)
		return opterr;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_opterr)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_optind, void)
		return optind;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_optind)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, __skadi_optind_set, int new_optind)
		optind = new_optind;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_optind_set)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_optopt, void)
		return optopt;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_optopt)

	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(char *, __skadi_optarg, void)
		return optarg;
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_optarg)
#endif
