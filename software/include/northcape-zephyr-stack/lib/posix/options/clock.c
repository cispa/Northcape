...
#ifdef CONFIG_SKADI_OS
	uint64_t ticks = skadi_uptime_ticks();
#else
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_clock_gettime, clockid_t clock_id, struct timespec *ts)
		return clock_gettime(clock_id, ts);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_clock_gettime)
#endif
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_clock_getres, clockid_t clock_id, struct timespec *ts)
		return clock_getres(clock_id, ts);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_clock_getres)
#endif
...
#ifdef CONFIG_SKADI_OS
	uint64_t elapsed_nsecs = k_ticks_to_ns_floor64(skadi_uptime_ticks());
#else
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_clock_settime, clockid_t clock_id, const struct timespec *ts)
		return clock_settime(clock_id, ts);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_clock_settime)
#endif
			       ...
			       struct timespec *rmtp)
{
	...
	if ((flags & TIMER_ABSTIME) == 0 &&
		...
		ns = rqtp->tv_nsec + NSEC_PER_SEC
			+ skadi_sleep(K_SECONDS(rqtp->tv_sec - 1)) * NSEC_PER_MSEC;
#else
		ns = rqtp->tv_nsec + NSEC_PER_SEC
...
#ifdef CONFIG_SKADI_OS
	uptime_ns = k_ticks_to_ns_ceil64(skadi_sys_clock_tick_get());
#else
	...
	do {
#ifdef CONFIG_SKADI_OS
		us = skadi_sleep(K_TIMEOUT_ABS_US(us)) * 1000;
#else
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, __skadi_gettimeofday, struct timeval *tv, void *tz)
		__ASSERT(!tz, "tz param is not interpreted, only exists for historical reasons!");
		return gettimeofday(tv, NULL);
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(__skadi_gettimeofday)
#endif
