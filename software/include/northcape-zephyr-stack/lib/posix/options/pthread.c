...
#ifdef CONFIG_SKADI_OS
	t = (struct posix_thread *)CONTAINER_OF(skadi_current_get(), struct posix_thread, thread);
#else
...
int pthread_attr_setstack(pthread_attr_t *_attr, void *stackaddr, size_t stacksize)
{
	...
	if (attr->stack != NULL) {
#ifdef CONFIG_SKADI_OS
		ret = skadi_thread_stack_free(attr->stack);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, posix_thread_recycle_work_handler, struct k_work *work)
#else
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(posix_thread_recycle_work_handler)
#endif

#ifdef CONFIG_SKADI_OS
	static struct k_work_delayable posix_thread_recycle_work;
	static bool posix_pthread_init_work(void){
		skadi_work_init_delayable(&posix_thread_recycle_work, SKADI_SUBSYSTEM_FUNCTION_POINTER(posix_thread_recycle_work_handler));
		return true;
	}
	SKADI_SUBSYSTEM_INIT_FUNCTIONS(posix_pthread_init_work);
#else
...
#ifdef CONFIG_SKADI_OS
	(void)skadi_work_schedule(&posix_thread_recycle_work, K_MSEC(CONFIG_PTHREAD_RECYCLER_DELAY_MS));
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, zephyr_thread_wrapper, void *arg1, void *arg2, void *arg3)
#else
FUNC_NORETURN
...
#ifdef CONFIG_SKADI_OS
	struct posix_thread *t = CONTAINER_OF(skadi_current_get(), struct posix_thread, thread);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(zephyr_thread_wrapper)
#endif
		   ...
		   void *arg)
{
...
#ifdef CONFIG_SKADI_OS
		t->attr.priority =
			zephyr_to_posix_priority(skadi_thread_priority_get(skadi_current_get()), &pol);
#else
		t->attr.priority =
...
#ifdef CONFIG_SKADI_OS
	{
		...
		params.new_thread = &t->thread;
		params.stack = t->attr.stack;
		params.stack_size = __get_attr_stacksize(&t->attr) + t->attr.guardsize;
		params.entry = SKADI_SUBSYSTEM_FUNCTION_POINTER(zephyr_thread_wrapper);
		params.p1 = (void *)arg;
		params.p2 = threadroutine;
		params.p3 = IS_ENABLED(CONFIG_PTHREAD_CREATE_BARRIER) ? UINT_TO_POINTER(barrier) : NULL;
		params.prio = posix_to_zephyr_priority(t->attr.priority, t->attr.schedpolicy);
		params.options = 0;
...
int pthread_setschedparam(pthread_t pthread, int policy, const struct sched_param *param)
{
	...
	if (ret == 0) {
#ifdef CONFIG_SKADI_OS
		skadi_thread_priority_set(&t->thread, new_prio);
#else
...
#ifdef CONFIG_SKADI_OS
		attr->stack = skadi_thread_stack_alloc(DYNAMIC_STACK_SIZE + attr->guardsize,
...
int pthread_getschedparam(pthread_t pthread, int *policy, struct sched_param *param)
{
	...
	K_SPINLOCK(&pthread_pool_lock) {
		...
		param->sched_priority =
			zephyr_to_posix_priority(skadi_thread_priority_get(&t->thread), policy);
#else
		param->sched_priority =
...
#ifdef CONFIG_SKADI_OS
		LOG_DBG("Aborting non-pthread %p", skadi_current_get());
		skadi_thread_abort(skadi_current_get());
#else
...
#ifdef CONFIG_SKADI_OS
	ret = skadi_thread_join(&t->thread, K_FOREVER);
#else
...
#ifdef CONFIG_SKADI_OS
	new_stack =
		skadi_thread_stack_alloc(stacksize + attr->guardsize, k_is_user_context() ? K_USER : 0);
#else
	new_stack =
...
#ifdef CONFIG_SKADI_OS
	return skadi_thread_name_set(kthread, name);
#else
...
int pthread_getname_np(pthread_t thread, char *name, size_t len)
{
	...
	kthread = &posix_thread_pool[thread].thread;
#ifdef CONFIG_SKADI_OS
	return skadi_thread_name_copy(kthread, name, len - 1);	
#else
