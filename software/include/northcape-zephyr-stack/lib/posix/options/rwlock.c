...
#ifdef CONFIG_SKADI_OS
	skadi_sem_init(&rwl->rd_sem, CONCURRENT_READER_LIMIT, CONCURRENT_READER_LIMIT);
	skadi_sem_init(&rwl->wr_sem, 1, 1);
	skadi_sem_init(&rwl->reader_active, 1, 1);
#else
...
#ifdef CONFIG_SKADI_OS
	skadi_sem_cleanup(&rwl->rd_sem);
	skadi_sem_cleanup(&rwl->wr_sem);
	skadi_sem_cleanup(&rwl->reader_active);
#endif
...
#ifdef CONFIG_SKADI_OS
	if (skadi_current_get() == rwl->wr_owner) {
#else
	if (k_current_get() == rwl->wr_owner) {
#endif
		/* Write unlock */
		rwl->wr_owner = NULL;
		skadi_sem_give(&rwl->reader_active);
		skadi_sem_give(&rwl->wr_sem);
	} else {
		/* Read unlock */
		skadi_sem_give(&rwl->rd_sem);

		if (skadi_sem_count_get(&rwl->rd_sem) == CONCURRENT_READER_LIMIT) {
...
static uint32_t read_lock_acquire(struct posix_rwlock *rwl, int32_t timeout)
{
	...
	if (skadi_sem_take(&rwl->wr_sem, SYS_TIMEOUT_MS(timeout)) == 0) {
		skadi_sem_take(&rwl->reader_active, K_NO_WAIT);
		skadi_sem_take(&rwl->rd_sem, K_NO_WAIT);
		skadi_sem_give(&rwl->wr_sem);
	} else {
		ret = EBUSY;
	}
...
static uint32_t write_lock_acquire(struct posix_rwlock *rwl, int32_t timeout)
{
	uint32_t ret = 0U;
#ifdef CONFIG_SKADI_OS
	int64_t elapsed_time, st_time = skadi_uptime_get();
#else
	...
	if (skadi_sem_take(&rwl->wr_sem, k_timeout) == 0) {
		/* update remaining timeout time for 2nd sem */
		if (timeout != SYS_FOREVER_MS) {
#ifdef CONFIG_SKADI_OS
			elapsed_time = skadi_uptime_get() - st_time;
#else
		...
		if (skadi_sem_take(&rwl->reader_active, k_timeout) == 0) {
#ifdef CONFIG_SKADI_OS
			rwl->wr_owner = skadi_current_get();
#else
