...
int pthread_barrier_wait(pthread_barrier_t *b)
{
...
#ifdef CONFIG_SKADI_OS
	err = skadi_mutex_lock(&bar->mutex, K_FOREVER);
#else
	...
	while (bar->count != 0) {
#ifdef CONFIG_SKADI_OS
		err = skadi_condvar_wait(&bar->cond, &bar->mutex, K_FOREVER);
#else
...
unlock:
#ifdef CONFIG_SKADI_OS
	err = skadi_condvar_signal(&bar->cond);
	__ASSERT_NO_MSG(err == 0);
	err = skadi_mutex_unlock(&bar->mutex);
	__ASSERT_NO_MSG(err == 0);
#else
...
#ifdef CONFIG_SKADI_OS
	err = skadi_condvar_broadcast(&bar->cond);
	__ASSERT_NO_MSG(err == 0);
...
#ifdef CONFIG_SKADI_OS
		err = skadi_mutex_init(&posix_barrier_pool[i].mutex);
		__ASSERT_NO_MSG(err == 0);
		err = skadi_condvar_init(&posix_barrier_pool[i].cond);
		__ASSERT_NO_MSG(err == 0);
#else
...
SYS_INIT(pthread_barrier_pool_init, POST_KERNEL, 0);
