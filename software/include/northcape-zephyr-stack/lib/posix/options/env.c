...
int setenv(const char *name, const char *val, int overwrite)
{
	...
	K_SPINLOCK(&environ_lock)
	{
		...
		if (ret >= 0) {
			...
			esize = environ_size();
#ifdef CONFIG_SKADI_OS
			envp = skadi_allocator_realloc(environ, sizeof(char **) *
...
#ifdef CONFIG_SKADI_OS
			env = skadi_allocator_realloc(environ[ret], tsize);
#else
...
#ifdef CONFIG_SKADI_OS
		skadi_allocator_free(environ[ret]);
#else
		free(environ[ret]);
#endif
...
#ifdef CONFIG_SKADI_OS
			envp = skadi_allocator_realloc(environ, (esize + 1 /* NULL */) * sizeof(char **));
#else
