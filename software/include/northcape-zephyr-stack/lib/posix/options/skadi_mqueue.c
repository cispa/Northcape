...
static bool posix_mqueue_init_sem(void){
	return skadi_sem_init(&mq_sem, 1, 1) == 0;
}

SKADI_SUBSYSTEM_INIT_FUNCTIONS(posix_mqueue_init_sem);
...
mqd_t mq_open(const char *name, int oflags, ...)
{
	...
	skadi_sem_take(&mq_sem, K_FOREVER);
	msg_queue = find_in_list(name);
	skadi_sem_give(&mq_sem);
	...
	mq_desc_ptr = skadi_allocator_alloc_rw(sizeof(struct mqueue_desc));
	if (mq_desc_ptr != NULL) {
	...
	if (msg_queue == NULL) {
		...
		mq_obj_ptr = skadi_allocator_alloc_rw(sizeof(mqueue_object));
		if (mq_obj_ptr != NULL) {
		...
		mq_name_ptr = skadi_allocator_alloc_rw(strlen(name) + 1);
		if (mq_name_ptr != NULL) {
		...
		mq_buf_ptr = skadi_allocator_alloc_rw(msg_size * max_msgs * sizeof(uint8_t));
		if (mq_buf_ptr != NULL) {
		...
		skadi_msgq_init(&msg_queue->queue, msg_queue->mem_buffer, msg_size,
			    max_msgs);
		skadi_sem_take(&mq_sem, K_FOREVER);
...
free_mq_buffer:
	skadi_allocator_free(mq_name_ptr);
free_mq_name:
	skadi_allocator_free(mq_obj_ptr);
free_mq_object:
	skadi_allocator_free(mq_desc_ptr);
free_mq_desc:
	errno = ENOSPC;
	return (mqd_t)mqd;
}
...
int mq_close(mqd_t mqdes)
{
	...
	skadi_msgq_cleanup(&mqd->mqueue->queue);
	
	skadi_allocator_free(mqd->mem_desc);
	return 0;
}
...
int mq_unlink(const char *name)
{
	...
	skadi_msgq_cleanup(&msg_queue->queue);

	skadi_allocator_free(msg_queue->name);
	msg_queue->name = NULL;
	skadi_sem_give(&mq_sem);
	remove_mq(msg_queue);
	return 0;
...
int mq_getattr(mqd_t mqdes, struct mq_attr *mqstat)
{
	...
	skadi_sem_take(&mq_sem, K_FOREVER);
	skadi_msgq_get_attrs(&mqd->mqueue->queue, &attrs);
			  ...
			  k_timeout_t timeout)
{
	...
	if (skadi_msgq_put(&mqd->mqueue->queue, (void *)msg_ptr, timeout) != 0) {
		errno = K_TIMEOUT_EQ(timeout, K_NO_WAIT) ? EAGAIN : ETIMEDOUT;
		return ret;
	}

	if (skadi_msgq_num_used_get(&mqd->mqueue->queue) - msgq_num > 0) {
			     ...
			     k_timeout_t timeout)
{
	...
	if (skadi_msgq_get(&mqd->mqueue->queue, (void *)msg_ptr, timeout) != 0) {
		errno = K_TIMEOUT_EQ(timeout, K_NO_WAIT) ? EAGAIN : ETIMEDOUT;
	} else {
...
static void remove_mq(mqueue_object *msg_queue)
{
	if (atomic_cas(&msg_queue->ref_count, 0, 0)) {
		...
		skadi_allocator_free(msg_queue->mem_buffer);
		skadi_allocator_free(msg_queue->mem_obj);
	}
}
