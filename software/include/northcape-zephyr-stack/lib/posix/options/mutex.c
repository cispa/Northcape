...
static int acquire_mutex(pthread_mutex_t *mu, k_timeout_t timeout)
{
...
#ifdef CONFIG_SKADI_OS
	if (m->sched_mutex->owner_id == skadi_current_get()) {
#else
	...
	if (ret == 0) {
#ifdef CONFIG_SKADI_OS
		ret = skadi_mutex_lock(m, timeout);
#else
...
#ifdef CONFIG_SKADI_OS
	err = skadi_mutex_init(&posix_mutex_pool[i]);
#else
