...
#ifdef CONFIG_SKADI_OS
	static bool posix_semaphore_init_mutex(void){
		return skadi_mutex_init(&nsem_mutex) == 0;
	}
	SKADI_SUBSYSTEM_INIT_FUNCTIONS(posix_semaphore_init_mutex);

static inline void nsem_list_lock(void)
{
	__unused int ret = skadi_mutex_lock(&nsem_mutex, K_FOREVER);
...
#ifdef CONFIG_SKADI_OS
	if (nsem->name != NULL) {
			skadi_allocator_free(nsem->name);
		}
		skadi_allocator_free(nsem);
#else
		if (nsem->name != NULL) {
			k_free(nsem->name);
		}
...
#ifdef CONFIG_SKADI_OS
	if (skadi_sem_count_get(semaphore)) {
		errno = EBUSY;
		return -1;
	}

	skadi_sem_reset(semaphore);
	skadi_sem_cleanup(semaphore);
#else
...
#ifdef CONFIG_SKADI_OS
	*value = (int) skadi_sem_count_get(semaphore);
#else
...
#ifdef CONFIG_SKADI_OS
	skadi_sem_init(semaphore, value, CONFIG_POSIX_SEM_VALUE_MAX);
#else
...
#ifdef CONFIG_SKADI_OS
	if (skadi_sem_take(semaphore, K_MSEC(timeout))) {
		errno = ETIMEDOUT;
		return -1;
	}
#else
...
{
#ifdef CONFIG_SKADI_OS
	if (skadi_sem_take(semaphore, K_NO_WAIT) == -EBUSY) {
#else
...
{
#ifdef CONFIG_SKADI_OS
	(void)skadi_sem_take(semaphore, K_FOREVER);
#else
...
#ifdef CONFIG_SKADI_OS
	nsem = skadi_allocator_calloc_rw(1, sizeof(struct nsem_obj));
#else
...
#ifdef CONFIG_SKADI_OS
	nsem = skadi_allocator_calloc_rw((namelen + 1), sizeof(uint8_t));
#else
...
#ifdef CONFIG_SKADI_OS
	(void)skadi_sem_init(&nsem->sem, value, CONFIG_POSIX_SEM_VALUE_MAX);
#else
