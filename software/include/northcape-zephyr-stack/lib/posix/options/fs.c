...
#include <zephyr/fs/fs.h>
	...
	#include <zephyr/skadi/fs/skadi_fs.h>
...
#ifdef CONFIG_SKADI_OS
	rc = skadi_fs_open(&ptr->file, name, zmode);
#else
...
#ifdef CONFIG_SKADI_OS
	skadi_zvfs_finalize_fd(fd, ptr, fs_fd_op_vtable);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, fs_close_vmeth, void *obj)
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(fs_close_vmeth)
#endif

#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(int, fs_ioctl_vmeth, void *obj, unsigned int request, va_list args)
#else
...
#ifdef CONFIG_SKADI_OS
		rc = skadi_fs_seek(&ptr->file, offset, whence);
		if (rc == 0) {
			rc = skadi_fs_tell(&ptr->file);
		}
#else
...
#ifdef CONFIG_SKADI_OS
		rc = skadi_fs_truncate(&ptr->file, length);
#else
...
#ifdef CONFIG_SKADI_OS
	SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(fs_ioctl_vmeth);
#endif
 ...
 */
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, fs_write_vmeth, void *obj, const void *buffer, size_t count)
#else
...
#ifdef CONFIG_SKADI_OS
	rc = skadi_fs_write(&ptr->file, buffer, count);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(fs_write_vmeth)
#endif
 ...
 */
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(ssize_t, fs_read_vmeth, void *obj, void *buffer, size_t count)
#else
...
#endif
{
	...
	struct posix_fs_desc *ptr = obj;
#ifdef CONFIG_SKADI_OS
	rc = skadi_fs_read(&ptr->file, buffer, count);
#else
...
#ifdef CONFIG_SKADI_OS
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(fs_read_vmeth)
#endif


#ifdef CONFIG_SKADI_OS
	static bool skadi_posix_fs_init_op_vtable(void){
		fs_fd_op_vtable = skadi_allocator_alloc_rw(sizeof(*fs_fd_op_vtable));

		__ASSERT_NO_MSG(fs_fd_op_vtable);

		if(!fs_fd_op_vtable){
			return false;
		}

		fs_fd_op_vtable->read = SKADI_SUBSYSTEM_FUNCTION_POINTER(fs_read_vmeth);
		fs_fd_op_vtable->write = SKADI_SUBSYSTEM_FUNCTION_POINTER(fs_write_vmeth);
		fs_fd_op_vtable->close = SKADI_SUBSYSTEM_FUNCTION_POINTER(fs_close_vmeth);
		fs_fd_op_vtable->ioctl = SKADI_SUBSYSTEM_FUNCTION_POINTER(fs_ioctl_vmeth);

		return true;
	}

	SKADI_SUBSYSTEM_INIT_FUNCTIONS(skadi_posix_fs_init_op_vtable);
#else
static struct fd_op_vtable fs_fd_op_vtable = {
	.read = fs_read_vmeth,
...
#ifdef CONFIG_SKADI_OS
	skadi_fs_dir_t_init(&ptr->dir);

	rc = skadi_fs_opendir(&ptr->dir, dirname);
#else
...
#ifdef CONFIG_SKADI_OS
	rc = skadi_fs_readdir(&ptr->dir, &fdirent);
#else
...
#ifdef CONFIG_SKADI_OS
	rc = skadi_fs_statvfs(path, &stat_vfs);
#else
...
#ifdef CONFIG_SKADI_OS
	rc = skadi_fs_stat(path, &stat_file);
#else
...
{
#ifdef CONFIG_SKADI_OS
	return skadi_zvfs_fstat(fildes, buf);
#else
