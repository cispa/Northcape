...
#ifdef CONFIG_SKADI_OS
#define NET_BUF_DBG(fmt, ...) LOG_DBG("(%p) " fmt, skadi_current_get(), \
				      ##__VA_ARGS__)
#else
...
#ifdef CONFIG_SKADI_OS
	/* need to be size-aligned and base-aligned to Dcache width -> otherwise, invalidation might destroy co-located data structures! */
	size_t aligned_size = 2*sizeof(void *) + CONFIG_DCACHE_LINE_SIZE + *size;
	aligned_size -= aligned_size % CONFIG_DCACHE_LINE_SIZE;
	aligned_size += CONFIG_DCACHE_LINE_SIZE;
	void *b = skadi_heap_alloc(pool, aligned_size, timeout);
	size_t base_offset;
	size_t original_base;
	bool derived;
	skadi_restriction_t restriction = SKADI_NO_RESTRICTION; 

	/* SHOULD be sized such that this cannot fail */
	__ASSERT_NO_MSG(b);
	buf->heap_allocation = b;

	original_base = skadi_cap_ops_inspect_get_base(b) + skadi_get_capability_offset(b);
	base_offset = original_base % CONFIG_DCACHE_LINE_SIZE;
	base_offset = CONFIG_DCACHE_LINE_SIZE - base_offset;

	b = ((uint8_t*)b) + base_offset;

	if(b){
		derived = skadi_cap_ops_derive(b, restriction, 2*sizeof(void *) + *size, skadi_get_capability_offset(b), SKADI_PERMISSION_READ | SKADI_PERMISSION_WRITE | SKADI_PERMISSION_IRQ_ACCESSIBLE | SKADI_PERMISSION_CACHEABLE_TLB | SKADI_PERMISSION_CACHEABLE_ACCESS, &b);
	}
	else{
		derived = false;
	}

	__ASSERT(skadi_cap_ops_inspect_get_base(b) % CONFIG_DCACHE_LINE_SIZE == 0, "Invalid base %p - base offset was %zu original base %zx", (void*)skadi_cap_ops_inspect_get_base(b), base_offset, original_base);

	__ASSERT_NO_MSG(b);
	__ASSERT_NO_MSG(derived);
	/* we need to ensure that the reference count is not on the same cache line as the data*/
	if(!b){
		b = buf->heap_allocation;
	}
	b = (void*)(((uint8_t*)b)+sizeof(void*));
#else
...
#ifdef CONFIG_SKADI_OS
	/* separately derived */
	bool dropped = skadi_cap_ops_drop(buf->__buf);
	ARG_UNUSED(dropped);
	__ASSERT_NO_MSG(dropped);
	skadi_heap_free(pool, buf->heap_allocation);
#else
...
#ifdef CONFIG_SKADI_OS
	ref_count = skadi_allocator_alloc_rw_lockable(sizeof(void *) + *size);
#else
...
#ifdef CONFIG_SKADI_OS
	skadi_allocator_free(ref_count);
#else
	k_free(ref_count);
#endif
}
...
static uint8_t *data_alloc(struct net_buf *buf, size_t *size, k_timeout_t timeout)
{
	...
	__ASSERT_NO_MSG(buf->data_alloc->cb);
	__ASSERT_NO_MSG(buf->data_alloc->cb->alloc);
	return buf->data_alloc->cb->alloc(buf, size, timeout);
}

static uint8_t *data_ref(struct net_buf *buf, uint8_t *data)
{
	return buf->data_alloc->cb->ref(buf, data);
}
#else /* CONFIG_SKADI_NET_ZEROCOPY */
static uint8_t *data_alloc(struct net_buf *buf, size_t *size, k_timeout_t timeout)
{
	struct net_buf_pool *pool = net_buf_pool_get(buf->pool_id);
...
#endif
{
	...
	NET_BUF_DBG("Calling net_buf_alloc_len with len %zu uninit count is %d!\n", size, pool->uninit_count);
...
#ifdef CONFIG_SKADI_OS
			buf = skadi_lifo_get(pool->free, K_NO_WAIT);
#else
	...
	if (!K_TIMEOUT_EQ(timeout, K_NO_WAIT) &&
	    skadi_current_get() == k_work_queue_thread_get(&k_sys_work_q)) {
#else
	if (!K_TIMEOUT_EQ(timeout, K_NO_WAIT) &&
	    k_current_get() == k_work_queue_thread_get(&k_sys_work_q)) {
#endif
		NET_BUF_DBG("Timeout discarded. No blocking in syswq");
		timeout = K_NO_WAIT;
	}
...
#endif
#ifdef CONFIG_SKADI_OS
			buf = skadi_lifo_get(pool->free, WARN_ALLOC_INTERVAL);
#else
...
#ifdef CONFIG_SKADI_OS
		buf = skadi_lifo_get(pool->free, timeout);
#else
...
#ifdef CONFIG_SKADI_NET_ZEROCOPY
	buf->data_alloc = pool->alloc;
	buf->heap_allocation = NULL;
#endif
...
#ifdef CONFIG_SKADI_OS
	buf = skadi_fifo_get(fifo, timeout);
#else
