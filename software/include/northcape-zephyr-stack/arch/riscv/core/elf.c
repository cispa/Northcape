...
/* CB-type has 8-bit signed immediate with implicit LSB of 0, giving it a 9-bit range */
...
static inline int riscv_relocation_fits(long long jump_target, long long max_distance,
					elf_word reloc_type, const char *symbol, uintptr_t loc)
{
	/* two's complement encoding, e.g., [-128=0b10000000, 127=0b01111111] encodable with 8 bits */
	if(jump_target < 0){
		max_distance++;
	}
	if (llabs(jump_target) > max_distance) {
		LOG_ERR("%lld byte relocation is not possible for symbol %s at %" PRIu64 " type %" PRIu64 " (max %lld)!",
			jump_target, symbol, (uint64_t) loc, (uint64_t)reloc_type, max_distance);
		      ...
		      const char *sym_name, uintptr_t load_bias)
{
	...
	case R_RISCV_32:
		...
		return riscv_relocation_fits(jump_target, INT32_MAX, reloc_type, sym_name, loc_unsigned);
	case R_RISCV_64:
	...
	case R_RISCV_BRANCH:
		...
		return riscv_relocation_fits(jump_target, RISCV_MAX_JUMP_DISTANCE_B_TYPE,
					     reloc_type, sym_name, loc_unsigned);
	case R_RISCV_JAL:
		jump_target = sym_base_addr + rel->r_addend - loc; /* S + A - P */
		modified_operand = UNALIGNED_GET(loc32);
		imm8 = jump_target;
