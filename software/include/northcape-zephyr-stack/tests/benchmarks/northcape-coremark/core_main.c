...
static long perf_ops_write_stall_cycles;
...
static void reset_performance_counters(void){
    ...
    perf_ops_write_stall_cycles = SKADI_PERF_COUNTER_READ_OPS_WRITE_STALL();
}

static void read_performance_counters(void){
	long diff_l1_instr, diff_l1_data, diff_l2_resolver, diff_l2_ops, diff_extra_cycles, diff_missunit_stall_cycles, diff_write_stall_cycles;
    ...
    diff_write_stall_cycles = SKADI_PERF_COUNTER_READ_OPS_WRITE_STALL() - perf_ops_write_stall_cycles;


	printf("Northcape statistics: L1 instruction misses %ld L1 data misses %ld L2 resolver misses %ld L2 ops misses %ld extra icache cycles %ld missunit stall cycles %ld write stall cycles %ld\n", diff_l1_instr, diff_l1_data, diff_l2_resolver, diff_l2_ops, diff_extra_cycles, diff_missunit_stall_cycles, diff_write_stall_cycles);
}
#else
static void reset_performance_counters(void){
...
#else
SKADI_SUBSYSTEM_MAIN_NOIRQ(void){
#endif
    int   argc = 0;
    char *argv[1];
...
main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < TEST_ITERATIONS; i++){
        printf("Repetition %u of %u\n", i+1, TEST_ITERATIONS);
        skadi_perf_start(100);
        (void)do_test(argc, argv);
        skadi_perf_cancel();
    }
    printf("Done :-)");
    return 0;
