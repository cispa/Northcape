...
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, timing_start_thread_entry, void *p1, void *p2, void *p3)
#else
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(timing_start_thread_entry)
#endif
...
void timestamp_overhead_init(uint32_t num_iterations)
{
	...
	params.entry = SKADI_SUBSYSTEM_FUNCTION_POINTER(timing_start_thread_entry);
