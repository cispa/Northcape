...
static struct skadi_benchmark_state thread_samples_1[CONFIG_BENCHMARK_NUM_ITERATIONS],
			   thread_samples_2[CONFIG_BENCHMARK_NUM_ITERATIONS],
			   thread_samples_3[CONFIG_BENCHMARK_NUM_ITERATIONS],
			   thread_samples_4[CONFIG_BENCHMARK_NUM_ITERATIONS],
			   thread_samples_5[CONFIG_BENCHMARK_NUM_ITERATIONS];

#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, thread_alt_thread_entry, void *p1, void *p2, void *p3)
#else
...
#endif
{
...
#ifdef SKADI_SUBSYSTEM
	skadi_sem_take(&pause_sem, K_FOREVER);  /* Let 'start_thread' execute */
#else
	...
	timestamp.sample = timing_timestamp_get();
#ifdef SKADI_SUBSYSTEM
	skadi_thread_suspend(&alt_thread);
#else
...
#ifdef SKADI_SUBSYSTEM
	priority = skadi_thread_priority_get(&alt_thread);
	skadi_thread_priority_set(&alt_thread, priority + 2);
#else
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(thread_alt_thread_entry)
#endif

#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE(void, thread_start_thread_entry, void *p1, void *p2, void *p3)
#else
...
#ifdef SKADI_SUBSYSTEM
	priority = skadi_thread_priority_get(&start_thread);
#else
	priority = k_thread_priority_get(&start_thread);
#endif

	for (uint32_t i = 0; i < num_iterations; i++) {
			skadi_benchmark_prepare_sample(&thread_samples_2[i]);
			skadi_benchmark_prepare_sample(&thread_samples_3[i]);
			skadi_benchmark_prepare_sample(&thread_samples_4[i]);
			skadi_benchmark_prepare_sample(&thread_samples_5[i]);

		/* 1. Measure time to create, but not start <alt_thread> */

		if ((bit_options & START_ALT) == START_ALT) {
			skadi_benchmark_prepare_sample(&thread_samples_1[i]);
			...
			params.entry = SKADI_SUBSYSTEM_FUNCTION_POINTER(thread_alt_thread_entry);
		
			params.p1 = NULL;
			params.p2 = NULL;
			params.p3 = NULL;
			params.prio = priority;
			params.options = (bit_options & ALT_USER) == ALT_USER ? K_USER : 0;
			...
			skadi_benchmark_add_sample(&thread_samples_1[i], timing_cycles_to_ns(timing_cycles_get(&start, &finish)));
		} else {
		...
		skadi_benchmark_add_sample(&thread_samples_2[i], timing_cycles_to_ns(timing_cycles_get(&start, &finish)));
		...
		skadi_benchmark_add_sample(&thread_samples_3[i], timing_cycles_to_ns(timing_cycles_get(&start, &finish)));

		/* 9. Being measuring time to resume <alt_thread> */

		start = timing_timestamp_get();
#ifdef SKADI_SUBSYSTEM
		skadi_thread_resume(&alt_thread);
#else
		...
		skadi_benchmark_add_sample(&thread_samples_4[i], timing_cycles_to_ns(timing_cycles_get(&start, &finish)));

		/* 13. Process the time to terminate <alt_thread> */

		start = timing_timestamp_get();
#ifdef SKADI_SUBSYSTEM
		skadi_thread_abort(&alt_thread);
		...
		skadi_benchmark_add_sample(&thread_samples_5[i], timing_cycles_to_ns(timing_cycles_get(&start, &finish)));
	}
...
#ifdef SKADI_SUBSYSTEM
SKADI_SUBSYSTEM_CALLEE_TRAMPOLINE_END(thread_start_thread_entry)
#endif
...
int thread_ops(uint32_t num_iterations, uint32_t start_options, uint32_t alt_options)
{
	...
	params.entry = SKADI_SUBSYSTEM_FUNCTION_POINTER(thread_start_thread_entry);

	params.p1 = (void *)(uintptr_t)num_iterations;
	params.p2 = (void *)(uintptr_t)bit_options;
	params.p3 = NULL;
	...
	if ((bit_options & START_ALT) == START_ALT) {
		...
		skadi_benchmark_evaluate_samples(thread_samples_1, CONFIG_BENCHMARK_NUM_ITERATIONS, 0, description);
	}
	...
	PRINT_STATS_AVG(description, (uint32_t)cycles,
			num_iterations, false, "");
	skadi_benchmark_evaluate_samples(thread_samples_2, CONFIG_BENCHMARK_NUM_ITERATIONS, 0, description);
	...
	PRINT_STATS_AVG(description, (uint32_t)cycles,
			num_iterations, false, "");
	skadi_benchmark_evaluate_samples(thread_samples_3, CONFIG_BENCHMARK_NUM_ITERATIONS, 0, description);
	...
	PRINT_STATS_AVG(description, (uint32_t)cycles,
			num_iterations, false, "");
	skadi_benchmark_evaluate_samples(thread_samples_4, CONFIG_BENCHMARK_NUM_ITERATIONS, 0, description);
	...
	PRINT_STATS_AVG(description, (uint32_t)cycles,
			num_iterations, false, "");
	skadi_benchmark_evaluate_samples(thread_samples_5, CONFIG_BENCHMARK_NUM_ITERATIONS, 0, description);
#ifdef SKADI_SUBSYSTEM
